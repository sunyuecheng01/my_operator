...
#include "nn_other.h"
#include "../op_graph/assert_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                     \
    ...
    assert.set_input_##inputName(placeholder##inputIndex);                                                           \
    assert.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                            \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    assert.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto assert = op::Assert("assert");
    std::vector<int64_t> xShape = {};
    ADD_INPUT(1, input_condition, DT_BOOL, xShape);

    outputs.push_back(assert);
    // 添加完毕
    return SUCCESS;
}
