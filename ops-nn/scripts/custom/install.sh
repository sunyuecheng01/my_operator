...
export LD_LIBRARY_PATH=${ASCEND_CANN_PACKAGE_PATH}/opp/built-in/op_impl/ai_core/tbe/op_api/lib/linux/${architecture}/:${LD_LIBRARY_PATH}
export LD_LIBRARY_PATH=${ASCEND_CANN_PACKAGE_PATH}/opp/built-in/op_impl/ai_core/tbe/op_tiling/lib/linux/${architecture}/:${LD_LIBRARY_PATH}
...
if [ ! -d ${targetdir}/${vendordir} ];then
        ...
        mkdir -p ${targetdir}/${vendordir}
        if [ $? -ne 0 ];then
            log "[ERROR] create ${targetdir}/${vendordir} failed"
            exit 1
        fi
fi
chmod u+w ${targetdir}/${vendordir}
