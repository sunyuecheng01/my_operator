...
/* !
 * \file non_zero_big_mask_dim8.h
 * \brief
 */

#ifndef CANN_NON_ZERO_BIG_MASK_DIM8_H
#define CANN_NON_ZERO_BIG_MASK_DIM8_H
...
public:
    __aicore__ inline NonZeroBigMaskDim8(){};
...
template <typename T1, typename T2>
__aicore__ inline void NonZeroBigMaskDim8<T1, T2>::ComputeOutput(LocalTensor<int32_t>& yUb)
{
    // Scalar
    ...
    uint32_t dim3SValue = multipDim.GetValue(IDX_NUM_3);
    uint32_t dim3MValue = multipDim.GetValue(IDX_NUM_19);
    int16_t dim3KValue = (int16_t)multipDim.GetValue(IDX_NUM_11);

    uint32_t dim4SValue = multipDim.GetValue(IDX_NUM_4);
    uint32_t dim4MValue = multipDim.GetValue(IDX_NUM_20);
    int16_t dim4KValue = (int16_t)multipDim.GetValue(IDX_NUM_12);

    uint32_t dim5SValue = multipDim.GetValue(IDX_NUM_5);
    uint32_t dim5MValue = multipDim.GetValue(IDX_NUM_21);
    int16_t dim5KValue = (int16_t)multipDim.GetValue(IDX_NUM_13);

    uint32_t dim6SValue = multipDim.GetValue(IDX_NUM_6);
    uint32_t dim6MValue = multipDim.GetValue(IDX_NUM_22);
    int16_t dim6KValue = (int16_t)multipDim.GetValue(IDX_NUM_14);

    // address
    __ubuf__ uint32_t* dstPtr0 = nullptr;
    __ubuf__ uint32_t* dstPtr1 = nullptr;
    __ubuf__ uint32_t* dstPtr2 = nullptr;
    __ubuf__ uint32_t* dstPtr3 = nullptr;
    __ubuf__ uint32_t* dstPtr4 = nullptr;
    __ubuf__ uint32_t* dstPtr5 = nullptr;
    __ubuf__ uint32_t* dstPtr6 = nullptr;
    __ubuf__ uint32_t* dstPtr7 = nullptr;
    ...
    dstPtr3 = dstPtr2 + this->processSize_;
    dstPtr4 = dstPtr3 + this->processSize_;
    dstPtr5 = dstPtr4 + this->processSize_;
    dstPtr6 = dstPtr5 + this->processSize_;
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTimes; i++) {
            ...
            AscendC::MicroAPI::AddrReg vagReg = AscendC::MicroAPI::CreateAddrReg<uint32_t>(1, offsetI);
            this->ComputeOutputBaseFunc(
                dstLastPtr, dstPtr4, dstLastPtr, preg, vagReg, srcReg, subReg, shapeReg, mulReg, mReg, divReg0, divReg1,
                dim4SValue, dim4MValue, dim4KValue);
        }
        mem_bar(VST_VLD);
        // 处理7维
        sreg = num;
        for (uint16_t i = 0; i < repeatTimes; i++) {
            ...
            AscendC::MicroAPI::AddrReg vagReg = AscendC::MicroAPI::CreateAddrReg<uint32_t>(1, offsetI);
            this->ComputeOutputBaseFunc(
                dstLastPtr, dstPtr5, dstLastPtr, preg, vagReg, srcReg, subReg, shapeReg, mulReg, mReg, divReg0, divReg1,
                dim5SValue, dim5MValue, dim5KValue);
        }
        mem_bar(VST_VLD);
        // 处理8维
        sreg = num;
        for (uint16_t i = 0; i < repeatTimes; i++) {
            ...
            AscendC::MicroAPI::AddrReg vagReg = AscendC::MicroAPI::CreateAddrReg<uint32_t>(1, offsetI);
            this->ComputeOutputBaseFunc(
                dstLastPtr, dstPtr6, dstLastPtr, preg, vagReg, srcReg, subReg, shapeReg, mulReg, mReg, divReg0, divReg1,
                dim6SValue, dim6MValue, dim6KValue);
        }
        mem_bar(VST_VLD);
    }
...
template <typename T1, typename T2>
__aicore__ inline void NonZeroBigMaskDim8<T1, T2>::Init(
    GM_ADDR x, GM_ADDR y, GM_ADDR outShape, GM_ADDR workspace, const NonZeroTilingData* tilingData)
{
...
template <typename T1, typename T2>
template <typename CLS_NAME, void (CLS_NAME::*ComputeOutputPtr)(LocalTensor<int32_t>& yUb)>
__aicore__ inline void NonZeroBigMaskDim8<T1, T2>::Process(CLS_NAME* objPtr)
{
    this->template ProcessBase<CLS_NAME, ComputeOutputPtr>(objPtr);
}
} // namespace NonZero

#endif // CANN_NON_ZERO_BIG_MASK_DIM8_H