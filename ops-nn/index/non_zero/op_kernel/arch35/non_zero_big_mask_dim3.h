...
/* !
 * \file non_zero_big_mask_dim3.h
 * \brief
 */

#ifndef CANN_NON_ZERO_BIG_MASK_DIM3_H
#define CANN_NON_ZERO_BIG_MASK_DIM3_H
...
class NonZeroBigMaskDim3 : public NonZeroBigMask<T1, T2> {
public:
    __aicore__ inline NonZeroBigMaskDim3(){};
...
template <typename T1, typename T2>
__aicore__ inline void NonZeroBigMaskDim3<T1, T2>::ComputeOutput(LocalTensor<int32_t>& yUb)
{
    // Scalar
...
template <typename T1, typename T2>
__aicore__ inline void NonZeroBigMaskDim3<T1, T2>::Init(
    GM_ADDR x, GM_ADDR y, GM_ADDR outShape, GM_ADDR workspace, const NonZeroTilingData* tilingData)
{
...
template <typename T1, typename T2>
template <typename CLS_NAME, void (CLS_NAME::*ComputeOutputPtr)(LocalTensor<int32_t>& yUb)>
__aicore__ inline void NonZeroBigMaskDim3<T1, T2>::Process(CLS_NAME* objPtr)
{
