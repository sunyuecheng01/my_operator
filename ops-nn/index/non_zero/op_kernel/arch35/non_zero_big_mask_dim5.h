...
/* !
 * \file non_zero_big_mask_dim5.h
 * \brief
 */

#ifndef CANN_NON_ZERO_BIG_MASK_DIM5_H
#define CANN_NON_ZERO_BIG_MASK_DIM5_H
...
class NonZeroBigMaskDim5 : public NonZeroBigMask<T1, T2> {
public:
...
template <typename T1, typename T2>
__aicore__ inline void NonZeroBigMaskDim5<T1, T2>::ComputeOutput(LocalTensor<int32_t>& yUb)
{
    // Scalar
...
template <typename T1, typename T2>
template <typename CLS_NAME, void (CLS_NAME::*ComputeOutputPtr)(LocalTensor<int32_t>& yUb)>
__aicore__ inline void NonZeroBigMaskDim5<T1, T2>::Process(CLS_NAME* objPtr)
{
    this->template ProcessBase<CLS_NAME, ComputeOutputPtr>(objPtr);
}
} // namespace NonZero

#endif // CANN_NON_ZERO_BIG_MASK_DIM5_H