...
template <typename T1, typename T2>
__aicore__ inline void ScatterListRLBLEPad<T1, T2>::Process()
{
...
template <typename T1, typename T2>
__aicore__ inline void ScatterListRLBLEPad<T1, T2>::ProcessEqLen(const int64_t& eachCoreBatchNum)
{
...
template <typename T1, typename T2>
__aicore__ inline void ScatterListRLBLEPad<T1, T2>::ProcessNotEqLen(const int64_t& eachCoreBatchNum)
{
    for (int64_t eachCoreBatchIdx = 0; eachCoreBatchIdx < eachCoreBatchNum; eachCoreBatchIdx++) {
        for (int64_t eachBatchInnerLoopIdx = 0; eachBatchInnerLoopIdx < m_tilingData.eachLoopNum - 1;
             eachBatchInnerLoopIdx++) {
        ...
        CopyIn(eachCoreBatchIdx, m_tilingData.eachLoopNum - 1);
        CopyOutNotEqLen(eachCoreBatchIdx, m_tilingData.eachLoopNum - 1, m_tilingData.eachLastLoopEle);
    }
}
    ...
    const int64_t& eachCoreBatchIdx, const int64_t& eachBatchInnerLoopIdx, const int64_t& eachBatchInnerLoopEle)
{
    ...
    if (maskIsNull || maskUb.GetValue(dim0Idx) == 1) {
        ...
        dstGmOffset = dim1Idx * m_tilingData.dstBatchStride + dim2OffsetIdx * m_tilingData.dim3Count +
                      eachBatchInnerLoopIdx * m_tilingData.eachPreLoopEle;
        if (eachBatchInnerLoopEle == m_tilingData.eachPreLoopEle ||
            eachBatchInnerLoopEle == m_tilingData.eachLastLoopEleAlign) {
            DataCopy(varGm[dstGmOffset], updatesUb, eachBatchInnerLoopEle);
        } else {
            CopyOutPad(eachBatchInnerLoopEle, m_tilingData.eachLastLoopEleAlign, updatesUb);
        }
        this->Mte3ToMte2();
    }
...
#endif // SCATTER_LIST_RLBLE_PAD_H_
