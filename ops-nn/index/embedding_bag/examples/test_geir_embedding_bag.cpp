...
/*!
 * \file test_geir_embedding_bag.cpp
 * \brief
 */
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::EmbeddingBag("add1");
    std::vector<int64_t> xShape = {4, 2};
    std::vector<int64_t> indicesShape = {4};
    std::vector<int64_t> offsetsShape = {4};
    std::vector<int64_t> perSampleWeightsShape = {4};
    ADD_INPUT(1, weight, inDtype, xShape);
    ADD_INPUT(1, indices, DT_INT32, xShape);
    ADD_INPUT(1, offsets, DT_INT32, xShape);
    ADD_INPUT(1, per_sample_weights, inDtype, xShape);
