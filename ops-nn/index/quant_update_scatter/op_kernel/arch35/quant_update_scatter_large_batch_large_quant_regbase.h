...
/*!
 * \file quant_update_scatter_large_batch_large_quant_regbase.h
 * \brief quant_update_scatter for large_batch_large_quant template
 */
#ifndef QUANT_UPDATE_SCATTER_LARGE_BATCH_LARGE_QUANT_REGBASE_H_
#define QUANT_UPDATE_SCATTER_LARGE_BATCH_LARGE_QUANT_REGBASE_H_
...
class QuantUpdateScatterLargeBatchLargeQuantRegbase
    : public QuantUpdateScatterBase<VarType, IndicesType, UpdatesType, ScalesType, OffsetsType, DivMode, CastRoundMode>
{
public:
    __aicore__ inline QuantUpdateScatterLargeBatchLargeQuantRegbase(){};
    using Base =
    ...
    __aicore__ inline void Process()
    {
        ...
        int64_t quantLoop = tilingData_.updateDim2 * tilingData_.updateDim3 / tilingData_.updateOriLastDim;
        for (int64_t bsIdx = 0; bsIdx < coreBsNum_; bsIdx++) {
            ...
            for (int64_t quantLoopIdx = 0; quantLoopIdx < quantLoop; quantLoopIdx++) {
                for (int64_t innerIdx = 0; innerIdx < tilingData_.innerLoopTimes; innerIdx++) {
                    CopyInUpdate(bsIdx, quantLoopIdx, innerIdx, tilingData_.innerLoopEle);
                    ...
                    CalcDstOffset(bsIdx, quantLoopIdx, innerIdx, iLocal);
                    CopyOutVar(tilingData_.innerLoopEle);
                }
                if (tilingData_.innerLoopTail != 0) {
                    CopyInUpdate(bsIdx, quantLoopIdx, tilingData_.innerLoopTimes, tilingData_.innerLoopTail);
                    ...
                    CalcDstOffset(bsIdx, quantLoopIdx, tilingData_.innerLoopTimes, iLocal);
    ...
    __aicore__ inline void CopyInUpdate(int64_t bsIdx, int64_t quantLoopIdx, int64_t innerIdx, int64_t len)
    {
        gmUpdatesOffset_ = (blockIdx_ * tilingData_.eachCoreBsNum + bsIdx) * tilingData_.srcBsStride +
                           quantLoopIdx * tilingData_.updateOriLastDim + innerIdx * tilingData_.innerLoopEle;
    ...
    __aicore__ inline void CopyInIndices(int64_t bsIdx)
    {
    ...
    __aicore__ inline void CalcDstOffset(
        int64_t bsIdx, int64_t quantLoopIdx, int64_t innerIdx, LocalTensor<IndicesType> iLocal)
    {
        ...
        if (tilingData_.indicesShapeRank == this->INDICES_SHAPE_RANK_2) {
            ...
            actualBsIdx = varDim0Idx * tilingData_.varDim1 + updateDim1Idx;
        } else {
...
#endif // QUANT_UPDATE_SCATTER_LARGE_BATCH_LARGE_QUANT_REGBASE_H_