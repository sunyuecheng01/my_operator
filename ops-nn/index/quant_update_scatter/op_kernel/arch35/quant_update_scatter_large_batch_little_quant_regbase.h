...
/*!
 * \file quant_update_scatter_large_batch_little_quant_regbase.h
 * \brief quant_update_scatter for large_batch_little_quant template
 */
#ifndef QUANT_UPDATE_SCATTER_LARGE_BATCH_LITTLE_QUANT_REGBASE_H_
...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        int64_t fullLoop_ = tilingData_.innerLoopTimes;
        int64_t fullBlockCount_ = tilingData_.innerLoopFullRpt;
        int64_t tailBlockCount_ = tilingData_.innerLoopTailRpt;
        ...
        for (int64_t bsIdx = 0; bsIdx < coreBsNum_; bsIdx++) {
            ...
            for (int64_t innerLoopIdx = 0; innerLoopIdx < fullLoop_; innerLoopIdx++) {
                ...
                CopyInUpdate(fullBlockCount_);
                QuantUpdate(fullBlockCount_, scaleLocal, offsetLocal);
                CalcDstOffset(bsIdx, innerLoopIdx, iLocal);
                CopyOutVar(fullBlockCount_);
            }
            if (tailBlockCount_ != 0) {
                gmUpdatesOffset_ = (blockIdx_ * tilingData_.eachCoreBsNum + bsIdx) * tilingData_.srcBsStride +
                                   fullLoop_ * tilingData_.innerLoopEle;
                CopyInUpdate(tailBlockCount_);
                QuantUpdate(tailBlockCount_, scaleLocal, offsetLocal);
                CalcDstOffset(bsIdx, fullLoop_, iLocal);
                CopyOutVar(tailBlockCount_);
            }
    ...
    __aicore__ inline void CalcDstOffset(int64_t bsIdx, int64_t innerLoopIdx, LocalTensor<IndicesType> iLocal)
    {
        ...
        gmVarOffset_ = actualBsIdx * tilingData_.dstBsStride + (axisOffset + innerLoopIdx) * tilingData_.varDim3;
...
#endif // QUANT_UPDATE_SCATTER_LARGE_BATCH_LITTLE_QUANT_REGBASE_H_