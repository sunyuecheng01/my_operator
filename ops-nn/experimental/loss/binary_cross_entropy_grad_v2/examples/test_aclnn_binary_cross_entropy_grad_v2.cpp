...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_binary_cross_entropy_grad_v2.h"
...
int main() {
  ...
  //std::vector<int64_t> gradOutputShape = {1};
  ...
  //std::vector<float> gradOutputHostData = {0.1};
  ...
  ret = aclnnBinaryCrossEntropyGradV2GetWorkspaceSize(gradOutput, self, target, weight, reduction, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnBinaryCrossEntropGradGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  ret = aclnnBinaryCrossEntropyGradV2(workspaceAddr, workspaceSize, executor, stream);
