...
#include "../op_kernel/hard_swish_v2_tiling_data.h"
#include "../op_kernel/hard_swish_v2_tiling_key.h"
namespace optiling {
...
// tiling 分发入口
static ge::graphStatus HardSwishV2TilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    if (totalIdx <= 0) {
        ...
        memset_s(tiling, sizeof(HardSwishV2TilingData), 0, sizeof(HardSwishV2TilingData));
...
static ge::graphStatus TilingParseForHardSwishV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(HardSwishV2)
    .Tiling(HardSwishV2TilingFunc)
    .TilingParse<HardSwishV2CompileInfo>(TilingParseForHardSwishV2);
} // namespace optiling