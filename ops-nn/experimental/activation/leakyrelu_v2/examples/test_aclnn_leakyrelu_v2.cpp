...
int main()
{
    ...
    std::vector<DataType> selfXHostData(2048, 2);
    ...
    std::vector<DataType> outHostData(2048, 0);
    ...
    ret = aclnnLeakyreluV2GetWorkspaceSize(selfX, negativeSlope, out, &workspaceSize, &executor);
    LOG_PRINT("aclnnLeakyreluV2GetWorkspaceSize returned %d, workspaceSize=%llu, executor=%p\n",
          ret, (unsigned long long)workspaceSize, (void*)executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLeakyreluV2ExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    ret = aclnnLeakyreluV2(workspaceAddr, workspaceSize, executor, stream);
