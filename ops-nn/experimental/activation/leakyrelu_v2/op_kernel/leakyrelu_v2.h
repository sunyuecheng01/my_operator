 ...
 * \brief
 */
#ifndef LEAKYRELU_V2_H
#define LEAKYRELU_V2_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "leakyrelu_v2_tiling_data.h"
#include "leakyrelu_v2_tiling_key.h"

namespace NsLeakyreluV2 {

using namespace AscendC;

constexpr int32_t BUFFER_NUM = 2;
constexpr float inputVal = 0.0;

template <typename T>
class LeakyreluV2 {
public:
    __aicore__ inline LeakyreluV2(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, const LeakyreluV2TilingData* tilingData);
...
template <typename T>
__aicore__ inline void LeakyreluV2<T>::Init(GM_ADDR x, GM_ADDR y, const LeakyreluV2TilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void LeakyreluV2<T>::CopyIn(int32_t progress)
{
...
template <typename T>
__aicore__ inline void LeakyreluV2<T>::CopyOut(int32_t progress)
{
...
template <typename T>
__aicore__ inline void LeakyreluV2<T>::Compute(int32_t progress)
{
    ...
    AscendC::Maxs(tmpTensor1, xLocal, static_cast<T>(inputVal), this->tileDataNum);
    AscendC::Mins(tmpTensor2, xLocal, static_cast<T>(inputVal), this->tileDataNum);
    AscendC::PipeBarrier<PIPE_V>();
    AscendC::Muls(tmpTensor2, tmpTensor2, static_cast<T>(this->negativeSlope), this->tileDataNum);
    AscendC::Add(yLocal, tmpTensor1, tmpTensor2, this->tileDataNum);
    outputQueueY.EnQue<T>(yLocal);
    inputQueueX.FreeTensor(xLocal);
}

template <typename T>
__aicore__ inline void LeakyreluV2<T>::Process()
{
