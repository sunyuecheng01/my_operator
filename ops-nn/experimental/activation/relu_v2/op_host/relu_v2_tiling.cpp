...
// tiling 分发入口
// 可直接替换你的 ReluV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus ReluV2TilingFunc(gert::TilingContext* context)
{
    ...
    // handle empty input
    if (totalIdx <= 0) {
        ReluV2TilingData* tiling = context->GetTilingData<ReluV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(ReluV2TilingData), 0, sizeof(ReluV2TilingData));
