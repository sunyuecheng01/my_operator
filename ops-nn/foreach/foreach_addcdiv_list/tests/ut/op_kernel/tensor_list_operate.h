...
template <typename T>
uint8_t* CreateTensorListForeachAddcdivList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type, char* tensor_name) {
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./addcdiv_list_data/"<< d_type << "_" << tensor_name << "_t_foreach_addcdiv" << i <<".bin";
...
template <typename T>
void FreeTensorListForeachAddcdivList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./addcdiv_list_data/"<< d_type << "_output_t_foreach_addcdiv" << i <<".bin";
