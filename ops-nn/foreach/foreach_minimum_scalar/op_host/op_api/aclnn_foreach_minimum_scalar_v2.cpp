...
static aclnnStatus ExecForeachMinimumScalarV2GetWorkspaceSize(const aclTensorList *x,  const aclScalar *scalar, const aclTensorList *out, uint64_t *workspaceSize,
    ...
    // 调用l0算子ForeachMinimumScalarV2进行计算
    auto result = l0op::ForeachMinimumScalarV2(contiguousTensors, otherTensor, out, uniqueExecutor.get());
...
aclnnStatus aclnnForeachMinimumScalarV2GetWorkspaceSize(
    ...
    aclOpExecutor **executor)
{
    L2_DFX_PHASE_1(aclnnForeachMinimumScalarV2, DFX_IN(x, scalar), DFX_OUT(out));
    return ExecForeachMinimumScalarV2GetWorkspaceSize(x, scalar, out, workspaceSize, executor);
}

aclnnStatus aclnnForeachMinimumScalarV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, const aclrtStream stream) {
