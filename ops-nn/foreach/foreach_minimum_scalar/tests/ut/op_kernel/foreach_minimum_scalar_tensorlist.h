...
/*!
 * \file foreach_minimum_scalar_tensorlist.h
 * \brief
 */
#ifndef FOREACH_MINIMUM_SCALAR_TENSORLIST_H
#define FOREACH_MINIMUM_SCALAR_TENSORLIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateTensorListForeachMinimumScalar(
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos != 0) {
            ...
            fileName << "./minimum_scalar_data/" << d_type << "_input_t_foreach_minimum" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachMinimumScalar(
    uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type, int arg_pos)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos == 0) {
            std::stringstream fileName;
            fileName << "./minimum_scalar_data/" << d_type << "_output_t_foreach_minimum" << i << ".bin";
...
#endif // FOREACH_MINIMUM_SCALAR_TENSORLIST_H
