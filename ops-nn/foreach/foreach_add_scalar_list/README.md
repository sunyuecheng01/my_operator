...
<table style="undefined;table-layout: fixed; width: 1005px"><colgroup>
  ...
  <tbody>
    <tr>
      <td>x</td>
      <td>输入</td>
      <td>表示进行加法运算的输入张量列表，对应公式中的`x`。该参数中所有Tensor的数据类型保持一致。</td>
    ...
    <tr>
      <td>scalars</td>
      <td>输入</td>
      <td>表示进行加法运算的输入标量列表，对应公式中的`scalars`。元素个数与`x`中Tensor的个数相等。数据类型与输入参数的数据类型存在一定的对应关系：当入参`x`的数据类型为FLOAT32、FLOAT16、BFLOAT16时，`scalars`的数据类型仅支持FLOAT32；当入参`x`的数据类型为INT32时，`scalars`的数据类型仅支持INT64。</td>
      <td>FLOAT32、INT64</td>
      <td>ND</td>
    </tr>
    <tr>
