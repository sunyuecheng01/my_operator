...
TEST_F(foreach_expm1_test, test_case_float16_2)
{
    ...
    uint8_t* x1 = CreateTensorListForeachExpm1<half>(shapeInfos, "float16"); // input tensor
    uint8_t* x2 = CreateTensorListForeachExpm1<half>(shapeInfos, "float16"); // output tensor

    ICPU_SET_TILING_KEY(1);
    ICPU_RUN_KF(foreach_expm1, blockDim, x1, x2, workspace, tiling);

    FreeTensorListForeachExpm1<half>(x2, shapeInfos, "float16");
    AscendC::GmFree((void*)x1);
    AscendC::GmFree((void*)workspace);
    AscendC::GmFree((void*)tiling);

    system("cd ./expm1_data/ && python3 compare_data.py 'float16'");
}
