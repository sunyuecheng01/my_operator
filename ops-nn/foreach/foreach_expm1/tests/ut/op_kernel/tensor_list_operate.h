...
template <typename T>
uint8_t* CreateTensorListForeachExpm1(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./expm1_data/" << d_type << "_input_t_foreach_expm1_" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachExpm1(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./expm1_data/" << d_type << "_output_t_foreach_expm1_" << i << ".bin";
