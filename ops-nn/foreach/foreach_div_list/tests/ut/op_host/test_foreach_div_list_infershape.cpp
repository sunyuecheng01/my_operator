...
#include "log/log.h"
#include "../../../op_graph/foreach_div_list_proto.h"

class ForeachDivListTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachDivList SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachDivList TearDown" << std::endl;
  }
};

TEST_F(ForeachDivListTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachDivList")->infer_shape;
...
TEST_F(ForeachDivListTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachDivList")->infer_datatype;
