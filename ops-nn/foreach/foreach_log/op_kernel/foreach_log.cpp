...
template <typename T>
__aicore__ void LogAdapter(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const int32_t& uValue)
{
    Log<T>(dstLocal, srcLocal);
}

extern "C" __global__ __aicore__ void foreach_log(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
