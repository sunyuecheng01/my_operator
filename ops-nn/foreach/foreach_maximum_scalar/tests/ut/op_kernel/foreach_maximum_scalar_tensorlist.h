...
/*!
 * \file foreach_maximum_scalar_tensorlist.h
 * \brief
 */
#ifndef FOREACH_MAXIMUM_SCALAR_TENSORLIST_H
#define FOREACH_MAXIMUM_SCALAR_TENSORLIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos != 0) {
            ...
            fileName << "./maximum_scalar_data/" << d_type << "_input_t_foreach_maximum" << i << ".bin";
    ...
    uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type, int arg_pos)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos == 0) {
            std::stringstream fileName;
            fileName << "./maximum_scalar_data/" << d_type << "_output_t_foreach_maximum" << i << ".bin";
...
#endif // FOREACH_MAXIMUM_SCALAR_TENSORLIST_H
