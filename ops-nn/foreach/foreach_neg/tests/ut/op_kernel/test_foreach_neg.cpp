...
TEST_F(foreach_neg_test, test_case_float_1)
{
    ...
    uint8_t* x1 = CreateTensorListForeachNeg<float>(shapeInfos, "float32"); // input tensor
    uint8_t* x2 = CreateTensorListForeachNeg<float>(shapeInfos, "float32"); // output tensor

    ICPU_SET_TILING_KEY(2);
    ICPU_RUN_KF(foreach_neg, blockDim, x1, x2, workspace, tiling);

    FreeTensorListForeachNeg<float>(x2, shapeInfos, "float32");
    AscendC::GmFree((void*)x1);
    AscendC::GmFree((void*)workspace);
    AscendC::GmFree((void*)tiling);

    system("cd ./neg_data/ && python3 compare_data.py 'float32'");
}
...
TEST_F(foreach_neg_test, test_case_float16_2)
{
    ...
    uint8_t* x1 = CreateTensorListForeachNeg<half>(shapeInfos, "float16"); // input tensor
    uint8_t* x2 = CreateTensorListForeachNeg<half>(shapeInfos, "float16"); // output tensor

    ICPU_SET_TILING_KEY(1);
    ICPU_RUN_KF(foreach_neg, blockDim, x1, x2, workspace, tiling);

    FreeTensorListForeachNeg<half>(x2, shapeInfos, "float16");
    AscendC::GmFree((void*)x1);
    AscendC::GmFree((void*)workspace);
    AscendC::GmFree((void*)tiling);

    system("cd ./neg_data/ && python3 compare_data.py 'float16'");
}
...
TEST_F(foreach_neg_test, test_case_int32_3)
{
    ...
    uint8_t* x1 = CreateTensorListForeachNeg<int32_t>(shapeInfos, "int32");
    uint8_t* x2 = CreateTensorListForeachNeg<int32_t>(shapeInfos, "int32");

    ICPU_SET_TILING_KEY(3); // int32
    ICPU_RUN_KF(foreach_neg, blockDim, x1, x2, workspace, tiling);

    FreeTensorListForeachNeg<int32_t>(x2, shapeInfos, "int32");
    AscendC::GmFree((void*)workspace);
    AscendC::GmFree((void*)tiling);

    system("cd ./neg_data/ && python3 compare_data.py 'int32'");
}
...
TEST_F(foreach_neg_test, test_case_bfloat16_4)
{
    ...
    uint8_t* x1 = CreateTensorListForeachNeg<bfloat16_t>(shapeInfos, "bfloat16_t");
    uint8_t* x2 = CreateTensorListForeachNeg<bfloat16_t>(shapeInfos, "bfloat16_t");

    ICPU_SET_TILING_KEY(4); // bfloat16_t
    ICPU_RUN_KF(foreach_neg, blockDim, x1, x2, workspace, tiling);

    FreeTensorListForeachNeg<bfloat16_t>(x2, shapeInfos, "bfloat16_t");
    AscendC::GmFree((void*)workspace);
    AscendC::GmFree((void*)tiling);

    system("cd ./neg_data/ && python3 compare_data.py 'bfloat16_t'");
}
