...
def gen_data_and_golden(shape_str, d_type="float32"):
    ...
    for index, shape in enumerate(shape_list):
        ...
        tmp_input.astype(np_type).tofile(f"{d_type}_input_t_foreach_addcmul{index}.bin")
        tmp_tensor1.astype(np_type).tofile(f"{d_type}_tensor1_t_foreach_addcmul{index}.bin")
        tmp_tensor2.astype(np_type).tofile(f"{d_type}_tensor2_t_foreach_addcmul{index}.bin")
        scalar.astype(np_type).tofile(f"{d_type}_scalar_t_foreach_addcmul{index}.bin")
        tmp_golden.astype(np_type).tofile(f"{d_type}_golden_t_foreach_addcmul{index}.bin")
