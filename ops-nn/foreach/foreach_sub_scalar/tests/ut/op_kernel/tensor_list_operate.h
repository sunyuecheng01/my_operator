...
#ifndef FOREACH_SUB_SCALAR_TENSOR_LIST_OPERATE_H
#define FOREACH_SUB_SCALAR_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateTensorListForeachSubScalar(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    uint64_t tensorListDescCount = 1 + shapeInfos.size() * 2;

    std::cout << "shapeInfos.size() = " << shapeInfos.size() << "\n" << std::endl;
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./sub_scalar_data/" << d_type << "_input_t_foreach_sub" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachSubScalar(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./sub_scalar_data/" << d_type << "_output_t_foreach_sub" << i << ".bin";
...
#endif // FOREACH_SUB_SCALAR_TENSOR_LIST_OPERATE_H
