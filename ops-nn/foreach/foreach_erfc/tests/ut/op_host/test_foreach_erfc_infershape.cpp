...
{
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachErfc")->infer_shape;
...
TEST_F(ForeachErfcTest, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachErfc")->infer_datatype;
