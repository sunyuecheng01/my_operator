...
#ifndef FOREACH_ERFC_TENSOR_LIST_OPERATE_H
#define FOREACH_ERFC_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateErfcTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./erfc_data/" << d_type << "_input_t_foreach_erfc_" << i << ".bin";
...
template <typename T>
void FreeErfcTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./erfc_data/" << d_type << "_output_t_foreach_erfc_" << i << ".bin";
...
#endif // FOREACH_ERFC_INPLACE_TENSOR_LIST_OPERATE_H