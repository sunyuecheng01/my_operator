...
def gen_data_and_golden(shape_str, d_type="float32"):
    ...
    for index, shape in enumerate(shape_list):
        tmp_input = np.random.rand(*shape)*10
        tmp_input = tmp_input.astype(np_type)
        tmp_golden = np.random.rand(*shape)*10
        tmp_golden = tmp_input.astype(np_type)

        for i in range(len(tmp_golden)):
            tmp_golden[i] = special.erfc(tmp_input[i])

        tmp_golden = tmp_golden.astype(np_type)

        tmp_input.astype(np_type).tofile(f"{d_type}_input_t_foreach_erfc_{index}.bin")
        tmp_golden.astype(np_type).tofile(f"{d_type}_golden_t_foreach_erfc_{index}.bin")
