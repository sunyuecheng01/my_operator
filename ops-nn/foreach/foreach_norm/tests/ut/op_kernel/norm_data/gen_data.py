...
def gen_data_and_golden(shape_str, scalar=1, d_type="float32"):
    d_type_dict = {
        "float32": np.float32,
        "float16": np.float16,
        "int32": np.int32,
    ...
    for index, shape in enumerate(shape_list):
        tmp_input = (np.random.rand(*shape) * 2 - 1) * 100
        tmp_golden = np.linalg.norm(tmp_input.astype(np.float32), ord = int(scalar), axis = -1)

        tmp_input.astype(np_type).tofile(f"{d_type}_input_t_foreach_norm_{index}.bin")
        tmp_golden.astype(np_type).tofile(f"{d_type}_golden_t_foreach_norm_{index}.bin")
