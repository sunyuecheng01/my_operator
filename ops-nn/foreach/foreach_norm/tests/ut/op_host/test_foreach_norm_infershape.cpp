...
{
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachNorm")->infer_shape;
    ...
    EXPECT_EQ(Ops::Base::ToString(*output_shape_1), "[1]");

    auto output_shape_2 = context->GetOutputShape(2);
    EXPECT_EQ(Ops::Base::ToString(*output_shape_2), "[1]");
}

TEST_F(ForeachNormTest, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachNorm")->infer_datatype;
