...
<table style="undefined;table-layout: fixed; width: 1005px"><colgroup>
  ...
  <tbody>
    <tr>
      <td>x</td>
      <td>输入</td>
      <td>表示进行双曲正切函数运算的输入张量列表，对应公式中的`x`。该参数中所有Tensor的数据类型保持一致。</td>
    ...
    <tr>
      <td>y</td>
      <td>输出</td>
      <td>表示进行双曲正切函数运算的输出张量列表，对应公式中的`y`。该参数中所有Tensor的数据类型保持一致。数据类型和数据格式与入参`x`的数据类型和数据格式一致，shape size大于等于入参`x`的shape size。</td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
    </tr>
  </tbody></table>
