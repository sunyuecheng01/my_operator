...
#ifndef FOREACH_TANH_TENSOR_LIST_OPERATE_H
#define FOREACH_TANH_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateTanhTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./tanh_data/" << d_type << "_input_t_foreach_tanh_" << i << ".bin";
...
template <typename T>
void FreeTanhTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./tanh_data/" << d_type << "_output_t_foreach_tanh_" << i << ".bin";
...
#endif // FOREACH_TANH_INPLACE_TENSOR_LIST_OPERATE_H
