...
/*!
 * \file foreach_pow_list_tensorlist.h
 * \brief
 */
#ifndef FOREACH_POW_LIST_TENSORLIST_H
#define FOREACH_POW_LIST_TENSORLIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos != 0) {
            ...
            fileName << "./pow_list_data/" << d_type << "_input_t_foreach_pow" << i << "_" << arg_pos << ".bin";
...
void FreeTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type, int arg_pos)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos == 0) {
            std::stringstream fileName;
            fileName << "./pow_list_data/" << d_type << "_output_t_foreach_pow" << i << ".bin";
...
#endif // FOREACH_POW_LIST_TENSORLIST_H
