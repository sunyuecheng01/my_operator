...
#ifndef TENSOR_LIST_OPERATE_H
#define TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateTensorListForeachAddcdivScalarList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type, char* tensor_name) {
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./addcdiv_scalar_list_data/"<< d_type << "_" << tensor_name << "_t_foreach_addcdiv" << i <<".bin";
...
template <typename T>
void FreeTensorListForeachAddcdivScalarList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./addcdiv_scalar_list_data/"<< d_type << "_output_t_foreach_addcdiv" << i <<".bin";
