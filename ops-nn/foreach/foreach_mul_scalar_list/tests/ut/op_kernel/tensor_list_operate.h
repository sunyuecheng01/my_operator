...
template <typename T>
uint8_t* CreateTensorListForeachMulScalarList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./mul_scalar_list_data/" << d_type << "_input_t_foreach_mul" << i << ".bin";
    ...
    uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./mul_scalar_list_data/" << d_type << "_output_t_foreach_mul" << i << ".bin";
