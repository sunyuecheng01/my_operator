...
template <typename T>
uint8_t* CreateTensorListForeachLog1p(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./log1p_data/" << d_type << "_input_t_foreach_log1p_" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachLog1p(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./log1p_data/" << d_type << "_output_t_foreach_log1p_" << i << ".bin";
