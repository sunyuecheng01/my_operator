...
template <typename T>
__aicore__ void Log1pAdapter(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const int32_t& uValue)
{
    T scalarVal = T(1);
...
extern "C" __global__ __aicore__ void foreach_log1p(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
