...
/*!
 * \file aclnn_foreach_round_off_number_v2.cpp
 * \brief
 */
...
static inline bool CheckDtypeValid(const aclTensorList* self, const aclScalar* scalar, const aclTensorList* out)
{
    ...
    OP_CHECK_DTYPE_NOT_SUPPORT(scalar, FOREACH_ROUND_SCALAR_SUPPORT_LIST, return false);
    return true;
}
    ...
    aclOpExecutor** executor)
{
    ...
    // sclar to tensor
    const aclTensor* otherTensor = uniqueExecutor.get()->ConvertToTensor(scalar, DataType::DT_INT8);

    // 调用l0算子ForeachRoundOffNumberV2进行计算
    auto result = l0op::ForeachRoundOffNumberV2(contiguousTensors, otherTensor, out, uniqueExecutor.get());
