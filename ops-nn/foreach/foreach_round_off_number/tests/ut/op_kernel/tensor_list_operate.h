...
template <typename T>
uint8_t* CreateForeachRoundOffNumberTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./round_off_number_data/" << d_type << "_input_t_foreach_round_off_number" << i << ".bin";
...
template <typename T>
void FreeForeachRoundOffNumberTensorList(
    uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./round_off_number_data/" << d_type << "_output_t_foreach_round_off_number" << i << ".bin";
