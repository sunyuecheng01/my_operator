...
template <typename T>
uint8_t* CreateTensorListForeachMinimumScalarList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./minimum_scalar_list_data/" << d_type << "_input_t_foreach_minimum" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachMinimumScalarList(
    uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./minimum_scalar_list_data/" << d_type << "_output_t_foreach_minimum" << i << ".bin";
