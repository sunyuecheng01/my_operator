...
{
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSqrt")->infer_shape;
...
TEST_F(ForeachSqrtTest, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSqrt")->infer_datatype;
