...
def gen_data_and_golden(shape_str, d_type="float32"):
    ...
    for index, shape in enumerate(shape_list):
        ...
        tmp_golden = np.tan(tmp_input.astype(np_type))

        tmp_input.astype(np_type).tofile(f"{d_type}_input_t_foreach_tan_{index}.bin")
        tmp_golden.astype(np_type).tofile(f"{d_type}_golden_t_foreach_tan_{index}.bin")
