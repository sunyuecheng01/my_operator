...
#include "log/log.h"
#include "../../../op_graph/foreach_cosh_proto.h"

class ForeachCoshTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachCosh SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachCosh TearDown" << std::endl;
  }
};

TEST_F(ForeachCoshTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachCosh")->infer_shape;
...
TEST_F(ForeachCoshTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachCosh")->infer_datatype;
