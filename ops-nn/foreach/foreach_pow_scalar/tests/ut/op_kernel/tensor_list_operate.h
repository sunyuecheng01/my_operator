...
#ifndef FOREACH_POW_SCALAR_TENSOR_LIST_OPERATE_H
#define FOREACH_POW_SCALAR_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateTensorListForeachPowScalar(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./pow_scalar_data/" << d_type << "_input_t_foreach_pow" << i << ".bin";
...
template <typename T>
void FreeTensorListForeachPowScalar(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./pow_scalar_data/" << d_type << "_output_t_foreach_pow" << i << ".bin";
...
#endif // FOREACH_POW_SCALAR_TENSOR_LIST_OPERATE_H
