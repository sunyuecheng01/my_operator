...
#ifndef FOREACH_SIN_TENSOR_LIST_OPERATE_H
#define FOREACH_SIN_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateSinTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./sin_data/" << d_type << "_input_t_foreach_sin_" << i << ".bin";
...
template <typename T>
void FreeSinTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./sin_data/" << d_type << "_output_t_foreach_sin_" << i << ".bin";
...
#endif // FOREACH_SIN_INPLACE_TENSOR_LIST_OPERATE_H