...
{
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSin")->infer_shape;
...
TEST_F(ForeachSinTest, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSin")->infer_datatype;
