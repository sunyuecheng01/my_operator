...
/*!
 * \file foreach_minimum_list_tensorlist.h
 * \brief
 */
#ifndef FOREACH_MINIMUM_LLST_TENSORLIST_H
#define FOREACH_MINIMUM_LLST_TENSORLIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos != 0) {
            ...
            fileName << "./minimum_list_data/" << d_type << "_input_t_foreach_minimum" << i << "_" << arg_pos << ".bin";
...
void FreeTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./minimum_list_data/" << d_type << "_output_t_foreach_minimum" << i << ".bin";
...
#endif // FOREACH_MINIMUM_LLST_TENSORLIST_H
