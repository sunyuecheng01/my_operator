...
private:
        ...
        uint32_t index, int64_t dataCount, LocalTensor<float>& float32Tensor, bool isRemainder)
    {
        ...
        computer.Compute(inLocal_1, inLocal_2, outLocal, float32Tensor, scalarVal, Base::maxCastDataCount, dataCount);
...
__aicore__ inline void ForeachOneScalarTernary<T, P, op, bufferNum, paramsCount, needCopyOut>::Init(
    GM_ADDR x1, GM_ADDR x2, GM_ADDR scalar, GM_ADDR y, GM_ADDR workspace, const ForeachCommonTilingData* tilingData)
{
