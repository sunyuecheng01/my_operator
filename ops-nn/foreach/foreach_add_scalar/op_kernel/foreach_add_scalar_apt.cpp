...
/*!
 * \file foreach_add_scalar_apt.cpp
 * \brief
 */
#include "./arch35/foreach_add_scalar_regbase.h"

using namespace ForeachAddScalar;

template <typename T, typename ScalarT>
__aicore__ inline void ForeachAddScalarImpl(
    GM_ADDR inputs, GM_ADDR scalar, GM_ADDR outputs, GM_ADDR workspace, GM_ADDR tiling, TPipe* tPipe)
{
    ...
    ForeachAddScalarRegbase<T, ScalarT, ForeachSoloTilingDataRegbase> op;
    ...
    GM_ADDR inputs, GM_ADDR scalar, GM_ADDR outputs, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(FOREACH_TILING_KEY_HALF)) {
        ForeachAddScalarImpl<half, DTYPE_SCALAR>(inputs, scalar, outputs, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_FLOAT)) {
        ForeachAddScalarImpl<float, float>(inputs, scalar, outputs, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_INT)) {
        ForeachAddScalarImpl<int, int>(inputs, scalar, outputs, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_BF16)) {
        ForeachAddScalarImpl<bfloat16_t, float>(inputs, scalar, outputs, workspace, tiling, &pipeOp);
    }
    pipeOp.Destroy();
}