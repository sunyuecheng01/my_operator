...
#include "log/log.h"
#include "../../../op_graph/foreach_add_scalar_proto.h"

class ForeachAddScalarTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachAddScalar SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachAddScalar TearDown" << std::endl;
  }
};

TEST_F(ForeachAddScalarTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAddScalar")->infer_shape;
...
TEST_F(ForeachAddScalarTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAddScalar")->infer_datatype;
