...
{
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAbs")->infer_shape;
...
TEST_F(ForeachAbsTest, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAbs")->infer_datatype;
