...
#include "log/log.h"
#include "../../../op_graph/foreach_atan_proto.h"

class ForeachAtanTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachAtan SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachAtan TearDown" << std::endl;
  }
};

TEST_F(ForeachAtanTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAtan")->infer_shape;
...
TEST_F(ForeachAtanTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAtan")->infer_datatype;
