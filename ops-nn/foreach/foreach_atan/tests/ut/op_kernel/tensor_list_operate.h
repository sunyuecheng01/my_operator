...
#ifndef FOREACH_ATAN_TENSOR_LIST_OPERATE_H
#define FOREACH_ATAN_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateAtanTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./atan_data/"<< d_type << "_input_t_foreach_atan_" << i <<".bin";
...
template <typename T>
void FreeAtanTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./atan_data/"<< d_type << "_output_t_foreach_atan_" << i <<".bin";
...
#endif  // FOREACH_ATAN_INPLACE_TENSOR_LIST_OPERATE_H