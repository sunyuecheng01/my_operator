...
/*!
 * \file foreach_maximum_list_tensorlist.h
 * \brief
 */
#ifndef FOREACH_MAXIMUM_LLST_TENSORLIST_H
#define FOREACH_MAXIMUM_LLST_TENSORLIST_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        if (arg_pos != 0) {
            ...
            fileName << "./maximum_list_data/" << d_type << "_input_t_foreach_maximum" << i << "_" << arg_pos << ".bin";
...
void FreeTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type)
{
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./maximum_list_data/" << d_type << "_output_t_foreach_maximum" << i << ".bin";
...
#endif // FOREACH_MAXIMUM_LLST_TENSORLIST_H
