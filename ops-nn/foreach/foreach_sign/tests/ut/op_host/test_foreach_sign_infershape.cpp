...
{
    // 确保获得的 OpImpl 指针不为空
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSign")->infer_shape;
...
TEST_F(ForeachSign, infer_dtype_test_1)
{
    auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachSign")->infer_datatype;
