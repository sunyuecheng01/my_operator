...
template <typename T>
__aicore__ inline void ForeachSignND<T>::ParseTilingData(const ForeachCommonTilingData* tilingData)
{
    ...
    uint16_t index, int64_t dataCount, struct TensorInfo& tensorInfo, bool isRemainder)
{
    ...
    computer.Compute(dataLocal, outLocal, tensorInfo, dataCount);
