...
#ifndef FOREACH_ACOS_TENSOR_LIST_OPERATE_H
#define FOREACH_ACOS_TENSOR_LIST_OPERATE_H
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
template <typename T>
uint8_t* CreateAcosTensorList(const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    *tensorListDesc = (tensorListDescCount - shapeInfos.size()) * sizeof(uint64_t);
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        fileName << "./acos_data/"<< d_type << "_input_t_foreach_acos_" << i <<".bin";
...
template <typename T>
void FreeAcosTensorList(uint8_t* addr, const std::vector<std::vector<uint64_t>>& shapeInfos, char* d_type) {
    ...
    for (size_t i = 0; i < shapeInfos.size(); i++) {
        ...
        std::stringstream fileName;
        fileName << "./acos_data/"<< d_type << "_output_t_foreach_acos_" << i <<".bin";
...
#endif  // FOREACH_ACOS_INPLACE_TENSOR_LIST_OPERATE_H