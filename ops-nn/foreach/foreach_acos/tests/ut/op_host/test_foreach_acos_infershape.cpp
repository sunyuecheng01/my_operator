...
#include "log/log.h"
#include "../../../op_graph/foreach_acos_proto.h"

class ForeachAcosTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachAcos SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachAcos TearDown" << std::endl;
  }
};

TEST_F(ForeachAcosTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAcos")->infer_shape;
...
TEST_F(ForeachAcosTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachAcos")->infer_datatype;
