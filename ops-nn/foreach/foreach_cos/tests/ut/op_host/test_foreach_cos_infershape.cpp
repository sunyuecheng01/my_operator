...
#include "log/log.h"
#include "../../../op_graph/foreach_cos_proto.h"

class ForeachCosTest : public testing::Test {
 protected:
  static void SetUpTestCase() {
    std::cout << "ForeachCos SetUp" << std::endl;
  }

  static void TearDownTestCase() {
    std::cout << "ForeachCos TearDown" << std::endl;
  }
};

TEST_F(ForeachCosTest, infer_shape_known_success) {
  auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachCos")->infer_shape;
...
TEST_F(ForeachCosTest, infer_dtype_test_1) {
  auto infer_datatype_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ForeachCos")->infer_datatype;
