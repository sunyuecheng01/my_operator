...
def gen_data_and_golden(shape_str, d_type="float32"):
    ...
    for index, shape in enumerate(shape_list):
        tmp_input = np.random.rand(*shape) * 2
        tmp_golden = np.log10(tmp_input.astype(np_type))

        tmp_input.astype(np_type).tofile(f"{d_type}_input_t_foreach_log10_{index}.bin")
        tmp_golden.astype(np_type).tofile(f"{d_type}_golden_t_foreach_log10_{index}.bin")
