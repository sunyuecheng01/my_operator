...
/*!
 * \file apply_adam_w_quant_fp16.h
 * \brief
 */
#ifndef APPLY_ADAM_W_QUANT_FP16_H_
#define APPLY_ADAM_W_QUANT_FP16_H_
...
template <typename T, typename U, typename T_VAR_GRAD>
...
public:
        ...
        GM_ADDR absmax_v_ref, const ApplyAdamWQuantTilingData* tilingData)
    {
        ...
        varGm.SetGlobalBuffer((__gm__ T_VAR_GRAD*)var + gmOffset);
        gradGm.SetGlobalBuffer((__gm__ T_VAR_GRAD*)grad + gmOffset);
        ...
        varRefGm.SetGlobalBuffer((__gm__ T_VAR_GRAD*)var_ref + gmOffset);
        ...
        pipe.InitBuffer(varBuf, singleSize * sizeof(T_VAR_GRAD));
        pipe.InitBuffer(gradBuf, singleSize * sizeof(T_VAR_GRAD));
        pipe.InitBuffer(varFp32Buf, singleSize * sizeof(T));
        pipe.InitBuffer(gradFp32Buf, singleSize * sizeof(T));
        ...
        this->var = varBuf.Get<T_VAR_GRAD>();
        this->grad = gradBuf.Get<T_VAR_GRAD>();
        this->varFp32 = varFp32Buf.Get<T>();
        this->gradFp32 = gradFp32Buf.Get<T>();
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t n = 0; n < perCoreRowWork; n++) {
            ...
            DeQuant(stateM, qMapM, absMaxM, calcBuf, singleBlockNum);
            DeQuant(stateV, qMapV, absMaxV, calcBuf, singleBlockNum);
            ...
            QuantM(stateM.template ReinterpretCast<T>(), qMapM, stateM, absMaxM, calcBuf, singleBlockNum);
            QuantV(stateV.template ReinterpretCast<T>(), qMapV, stateV, absMaxV, calcBuf, singleBlockNum);
            PipeSync<AscendC::HardEvent::V_MTE3>();
...
private:
    __aicore__ inline void ParseTilingData(const ApplyAdamWQuantTilingData* tilingData)
    {
...
private:
    ...
    AscendC::TBuf<AscendC::QuePosition::VECCALC> varFp32Buf;
    AscendC::TBuf<AscendC::QuePosition::VECCALC> gradFp32Buf;
    ...
    AscendC::GlobalTensor<T_VAR_GRAD> varGm;
    AscendC::GlobalTensor<T_VAR_GRAD> varRefGm;
    AscendC::GlobalTensor<T_VAR_GRAD> gradGm;
    ...
    AscendC::LocalTensor<T_VAR_GRAD> var;
    AscendC::LocalTensor<T_VAR_GRAD> grad;
    AscendC::LocalTensor<T> varFp32;
    AscendC::LocalTensor<T> gradFp32;
...
#endif // APPLY_ADAM_W_QUANT_FP16_H_