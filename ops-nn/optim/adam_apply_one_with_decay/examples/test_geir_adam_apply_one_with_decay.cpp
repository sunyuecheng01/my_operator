/**
	 ...
	 /*!
	  * \file test_geir_adam_apply_one_with_decay.cpp
	  * \brief
	  */
	 ...
	 #include "../op_graph/adam_apply_one_with_decay_proto.h"
	 ...
	 #define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
	     ...
	     adamApplyOneWithDecay1.set_input_##intputName(placeholder##intputIndex);                                    \
	     inputs.push_back(placeholder##intputIndex);

	 #define ADD_INPUT_ATTR(attrName, attrValue) adamApplyOneWithDecay1.set_attr_##attrName(attrValue);
	 ...
	 #define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
	     ...
	     graph.AddOp(placeholder##intputIndex);                                                                  \
	     adamApplyOneWithDecay1.set_input_##intputName(placeholder##intputIndex);                                \
	     adamApplyOneWithDecay1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                 \
	     inputs.push_back(placeholder##intputIndex);

	 #define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
	     TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
	     adamApplyOneWithDecay1.update_output_desc_##outputName(outputName##outputIndex##_desc);
	     ...
	     Graph& graph)
	 {
	     Status ret = SUCCESS;
	     // 自定义代码：添加单算子定义到图中
	     auto adamApplyOneWithDecay1 = op::AdamApplyOneWithDecay("adamApplyOneWithDecay1");
	     std::vector<int64_t> xShape = {(32, 4, 4, 4)};
	     ADD_INPUT(1, input0, inDtype, xShape);
	     ADD_INPUT(2, input1, inDtype, xShape);
	     ADD_INPUT(3, input2, inDtype, xShape);
	     ADD_INPUT(4, input3, inDtype, xShape);
	     ADD_INPUT(5, input4, inDtype, xShape);
	     ADD_INPUT(6, mul0_x, inDtype, xShape);
	     ADD_INPUT(7, mul1_x, inDtype, xShape);
	     ADD_INPUT(8, mul2_x, inDtype, xShape);
	     ADD_INPUT(9, mul3_x, inDtype, xShape);
	     ADD_INPUT(10, mul4_x, inDtype, xShape);
	     ADD_INPUT(11, add2_y, inDtype, xShape);

	     outputs.push_back(adamApplyOneWithDecay1);
	     // 添加完毕
	     return SUCCESS;
	 }
