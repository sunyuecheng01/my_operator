...
/*!
 * \file adam_apply_one_with_decay_assign.cpp
 * \brief adam_apply_one_with_decay_assign.cpp
 */
...
__global__ __aicore__ void adam_apply_one_with_decay_assign(
    ...
    GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (std::is_same<DTYPE_INPUT0, bfloat16_t>::value) {
        using OpDag = AdamApplyOneWithDecayAssignOp::AdamApplyOneWithDecayAssignCompute<bfloat16_t, float>::OpDag;
        BroadcastSch<schMode, OpDag> sch(tiling);
        sch.Process(
            input0, input1, input2, input3, input4, mul0_x, mul1_x, mul2_x, mul3_x, mul4_x, add2_y, output0, output1,
            output2);
    } else {
        using OpDag = AdamApplyOneWithDecayAssignOp::AdamApplyOneWithDecayAssignCompute<DTYPE_INPUT0, float>::OpDag;
        BroadcastSch<schMode, OpDag> sch(tiling);
        sch.Process(
            input0, input1, input2, input3, input4, mul0_x, mul1_x, mul2_x, mul3_x, mul4_x, add2_y, output0, output1,
            output2);
