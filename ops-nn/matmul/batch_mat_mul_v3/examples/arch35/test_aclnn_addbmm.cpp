...
int AclnnAddbmmTest(int32_t deviceId, aclrtStream& stream)
{
...
int main()
{
    ...
    int32_t deviceId = 0;
    aclrtStream stream;
    auto ret = AclnnAddbmmTest(deviceId, stream);
    CHECK_FREE_RET(ret == ACL_SUCCESS, LOG_PRINT("AclnnAddbmmTest failed. ERROR: %d\n", ret); return ret);
    Finalize(deviceId, stream);
    return 0;
}