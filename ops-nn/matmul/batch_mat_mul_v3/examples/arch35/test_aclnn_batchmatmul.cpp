...
int AclnnBatchMatmulTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<uint16_t> selfHostData = {0x0000, 0x3f80, 0x4000, 0x4040, 0x4080, 0x40a0};
...
int main()
{
    ...
    int32_t deviceId = 0;
    aclrtStream stream;
    auto ret = AclnnBatchMatmulTest(deviceId, stream);
    CHECK_FREE_RET(ret == ACL_SUCCESS, LOG_PRINT("AclnnBatchMatmulTest failed. ERROR: %d\n", ret); return ret);
    Finalize(deviceId, stream);
    return 0;
}