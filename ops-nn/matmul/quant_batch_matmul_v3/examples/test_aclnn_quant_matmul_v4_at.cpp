...
int aclnnQuantMatmulV4Test(int32_t deviceId, aclrtStream& stream)
{
    ...
    ret = CreateAclTensor(
        pertokenScaleHostData, pertokenScaleShape, &pertokenScaleDeviceAddr, aclDataType::ACL_FLOAT, &pertokenScale);
    std::unique_ptr<aclTensor, aclnnStatus (*)(const aclTensor*)> pertokenScaleTensorPtr(
        pertokenScale, aclDestroyTensor);
    ...
    workspaceSize = 0;
    ret = aclnnQuantMatmulV4GetWorkspaceSize(
        x1, x2Transposed, quantParam, nullptr, nullptr, bias, transposeX1, transposeX2, out, &workspaceSize, &executor);
