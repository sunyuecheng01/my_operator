...
#include "matmul_v3_asw_loadbalance_table.h"
...
MM_REGISTER_TILING_TEMPLATE(MatMulV3, MatMulV3AswTiling, RESERVED_VERSION, BASE); // supportMmadS8S4平台
...
void MatMulV3AswTiling::GetOuterAxisTailCnt(bool nLoadBalance, uint64_t& baseTailSplitCnt, uint64_t& tailMain)
{
...
void MatMulV3AswTiling::CalcSingleX(uint64_t& higherSingleX, uint64_t& lowerSingleX)
{
...
void MatMulV3AswTiling::CalcBasicBlock()
{
    uint64_t mCore = MathUtil::CeilDivision(args_.mValue, runInfo_.baseM);
    uint64_t nCore = MathUtil::CeilDivision(args_.nValue, runInfo_.baseN);
...
void MatMulV3AswTiling::FormulateLoadBalanceBlock()
{
    runInfo_.baseM = std::min(ops::CeilAlign(args_.mValue, BASIC_BLOCK_SIZE_16), runInfo_.baseM);
    runInfo_.baseN = std::min(ops::CeilAlign(args_.nValue, BASIC_BLOCK_SIZE_16), runInfo_.baseN);
...
// 单边大场景处理
uint64_t MatMulV3AswTiling::UpdateBaseBlock(uint64_t baseBlock, bool isMLarger)
{
    if (!isMLarger) {
...
void MatMulV3AswTiling::HandleLargeSingleSide(uint64_t minMN, uint64_t maxMN, bool isMLarger)
{
    if (isMLarger) {
...
// 两边都比较大场景处理
bool MatMulV3AswTiling::UpdateBothBaseBlock(
...
bool MatMulV3AswTiling::CalcBestBalance(CalcParams& params, bool isMLarger)
{
    uint64_t startIndex;
    uint64_t count;
...
void MatMulV3AswTiling::HandleLargeBothSides(
...
ge::graphStatus MatMulV3AswTiling::DoNormOpTiling()
{
