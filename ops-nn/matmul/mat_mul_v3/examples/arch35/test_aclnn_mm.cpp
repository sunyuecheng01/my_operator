...
int AclnnMmTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<uint16_t> selfHostData(16 * 32, 0b0011111110000000);
    std::vector<uint16_t> mat2HostData(32 * 16, 0b0011111110000000);
    std::vector<uint16_t> outHostData(16 * 16, 0);
    // 创建self aclTensor
    ...
    float resultDataFloat = 0;
    for (int64_t i = 0; i < size; i++) {
        resultDataFloat = bf16_to_float(resultData[i]);
...
int main()
{
    ...
    int32_t deviceId = 0;
    aclrtStream stream;
    auto ret = AclnnMmTest(deviceId, stream);
    CHECK_FREE_RET(ret == ACL_SUCCESS, LOG_PRINT("AclnnMmTest failed. ERROR: %d\n", ret); return ret);
    Finalize(deviceId, stream);
    return 0;
}