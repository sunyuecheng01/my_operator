...
int AclnnAddmmInplaceTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<uint16_t> mat1HostData = {0x3f80, 0x3f80, 0x3f80, 0x4000, 0x4000, 0x4000};
    std::vector<uint16_t> mat2HostData = {0x3f80, 0x3f80, 0x3f80, 0x4000, 0x4000, 0x4000,
                                          0x4040, 0x4040, 0x4040, 0x4080, 0x4080, 0x4080};
    std::vector<uint16_t> outHostData(8, 0x0000);
    ...
    // 创建mat1 aclTensor
    ret = CreateAclTensor(mat1HostData, mat1Shape, &mat1DeviceAddr, aclDataType::ACL_BF16, &mat1);
...
int main()
{
    ...
    int32_t deviceId = 0;
    aclrtStream stream;
    auto ret = AclnnAddmmInplaceTest(deviceId, stream);
    CHECK_FREE_RET(ret == ACL_SUCCESS, LOG_PRINT("AclnnAddmmInplaceTest failed. ERROR: %d\n", ret); return ret);
    Finalize(deviceId, stream);
