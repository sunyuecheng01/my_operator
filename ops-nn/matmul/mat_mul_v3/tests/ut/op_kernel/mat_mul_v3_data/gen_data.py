...
def gen_matmul_v3_data(matrix_m, matrix_k, matrix_n):
    shape_a = np.random.uniform(-1, 1, [int(matrix_m), int(matrix_k)]).astype(np.float16)
    shape_b = np.random.uniform(-1, 1, [int(matrix_k), int(matrix_n)]).astype(np.float16)
    shape_output = np.random.uniform(-1, 1, [int(matrix_m), int(matrix_n)]).astype(np.float16)
