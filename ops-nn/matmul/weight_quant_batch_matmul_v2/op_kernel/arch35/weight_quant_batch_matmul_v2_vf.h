...
__aicore__ inline void AntiquantW8PerGroupNK(ParamsW8NK<XType>& param)
{
    ...
    for (uint16_t gIdx = 0; gIdx < static_cast<uint16_t>(param.groupNum); ++gIdx) {
        for (uint16_t nIdx = 0; nIdx < static_cast<uint16_t>(param.nBub); ++nIdx) {
            ...
            for (uint16_t kIdx = 0; kIdx < static_cast<uint16_t>(param.vlNum); ++kIdx) {
    ...
    for (uint16_t nIdx = 0; nIdx < static_cast<uint16_t>(param.nBub); ++nIdx) {
        ...
        for (uint16_t kIdx = 0; kIdx < static_cast<uint16_t>(param.tailVlNum); ++kIdx) {
            maskWeight = MicroAPI::UpdateMask<XType>(maskWeightValue);
...
__aicore__ inline void AntiquantPerGroupKN(ParamsKN<XType>& param)
{
    ...
    for (uint16_t nIdx = 0; nIdx < static_cast<uint16_t>(param.nLoop); ++nIdx) {
        maskWeight = MicroAPI::UpdateMask<XType>(maskWeightValue);
        for (uint16_t gIdx = 0; gIdx < static_cast<uint16_t>(param.groupNum); ++gIdx) {
            ...
            for (uint16_t kIdx = 0; kIdx < static_cast<uint16_t>(param.groupSize); ++kIdx) {
        ...
        for (uint16_t kIdx = 0; kIdx < static_cast<uint16_t>(param.groupTail); ++kIdx) {
            if constexpr (AscendC::IsSameType<WType, int8_t>::value) {
                MicroAPI::AddrReg addRegWeightTail = MicroAPI::CreateAddrReg<typename RegTensorActualT<WType>::T>(
...
__aicore__ inline void AntiquantW4Pergroup128NK(ParamsGroupSize128And256<XType, WType>& p)
{
    ...
    for (uint16_t groupIdx = 0; groupIdx < static_cast<uint16_t>(p.groupNum); ++groupIdx) {
        for (uint16_t nBubIdx = 0; nBubIdx < static_cast<uint16_t>(p.bubNLen); ++nBubIdx) {
            if constexpr (useVag) {
...
__aicore__ inline void AntiquantW4PergroupGt128NKCase1(ParamsGroupSizeGt128<XType, WType>& p)
{
    ...
    for (uint16_t nBubIdx = 0; nBubIdx < static_cast<uint16_t>(p.bubNLen); ++nBubIdx) {
        for (uint16_t groupIdx = 0; groupIdx < static_cast<uint16_t>(p.mainGroupNum); ++groupIdx) {
            if constexpr (hasAntiquantOffset) {
                MicroAPI::DataCopy<XType, MicroAPI::LoadDist::DIST_BRC_B16>(
            ...
            for (uint16_t vlIdx = 0; vlIdx < static_cast<uint16_t>(p.numVLInGroup); ++vlIdx) {
                MicroAPI::DataCopy<typename RegTensorActualT<WType>::T, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                    weightB4,
        ...
        for (uint16_t tailGroup = 0; tailGroup < static_cast<uint16_t>(p.tailGroupInBubKLen); ++tailGroup) {
            if constexpr (hasAntiquantOffset) {
                MicroAPI::DataCopy<XType, MicroAPI::LoadDist::DIST_BRC_B16>(
            ...
            for (uint16_t vlIdx = 0; vlIdx < static_cast<uint16_t>(p.numVLInRemainGroup); ++vlIdx) {
                MicroAPI::DataCopy<typename RegTensorActualT<WType>::T, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                    weightB4,
                    (__local_mem__ typename RegTensorActualT<WType>::T*)(p.weightInBaseAddr + nBubIdx * p.innerExtend +
...
__aicore__ inline void AntiquantW4PergroupGt128NKCase2(ParamsGroupSizeGt128<XType, WType>& p)
{
    ...
    for (uint16_t nBubIdx = 0; nBubIdx < static_cast<uint16_t>(p.bubNLen); ++nBubIdx) {
        ...
        for (uint16_t vlIdx = 0; vlIdx < static_cast<uint16_t>(p.numVLInKLen); ++vlIdx) {
            MicroAPI::DataCopy<typename RegTensorActualT<WType>::T, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                weightB4,
                (__local_mem__ typename RegTensorActualT<WType>::T*)(p.weightInBaseAddr + nBubIdx * p.innerExtend +
                                                                     vlIdx * p.vlB4SizeInByte));
        ...
        for (uint16_t tailVL = 0; tailVL < static_cast<uint16_t>(p.tailVLInKLen); ++tailVL) {
            MicroAPI::DataCopy<typename RegTensorActualT<WType>::T, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                weightB4,
                (__local_mem__ typename RegTensorActualT<WType>::T*)(p.weightInBaseAddr + nBubIdx * p.innerExtend +
...
__aicore__ inline void AntiquantW4Pergroup32OddNK(ParamsGroupSize32OddNK<XType>& p)
{
    ...
    for (uint16_t groupIdx = 0; groupIdx < static_cast<uint16_t>(p.groupPairNum); ++groupIdx) {
        for (uint16_t nIdx = 0; nIdx < static_cast<uint16_t>(p.bubNLen); ++nIdx) {
            // odd group part
            MicroAPI::AddrReg addRegScale =
            ...
            for (uint16_t oddIdx = 0; oddIdx < static_cast<uint16_t>(p.oddGroupVLNum); ++oddIdx) {
                MicroAPI::AddrReg addRegWeight = MicroAPI::CreateAddrReg<typename RegTensorActualT<WType>::T>(
            ...
            if constexpr (crossBorder) {
                ...
                for (uint16_t evenIdx = 0; evenIdx < static_cast<uint16_t>(p.evenGroupVLNum); ++evenIdx) {
                    if constexpr (useVag) {
                        MicroAPI::AddrReg addRegWeight = MicroAPI::CreateAddrReg<typename RegTensorActualT<WType>::T>(
