...
#include "../op_graph/gemm_v2_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::GemmV2("add1");
    std::vector<int64_t> aShape = {4, 8};
    std::vector<int64_t> bShape = {8, 12};
    std::vector<int64_t> alphaShape = {4};
    std::vector<int64_t> betaShape = {4};
    std::vector<int64_t> cShape = {4, 12};

    ADD_INPUT(1, a, inDtype, aShape);
    ADD_INPUT(2, b, inDtype, bShape);
    ADD_INPUT(3, alpha, inDtype, alphaShape);
    ADD_INPUT(4, beta, inDtype, betaShape);
    ADD_INPUT(5, c, DT_FLOAT, cShape);
