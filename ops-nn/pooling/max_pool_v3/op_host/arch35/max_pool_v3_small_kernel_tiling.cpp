...
/*!
 * \file max_pool_v3_small_kernel_tiling.cpp
 * \brief
 */
#include "max_pool_v3_small_kernel_tiling.h"
...
void MaxPoolV3SmallKernelTiling::InitializationVars()
{
    isPadding_ = false;
...
bool MaxPoolV3SmallKernelTiling::IsCapable()
{
...
uint64_t MaxPoolV3SmallKernelTiling::GetTilingKey() const
{
...
bool MaxPoolV3SmallKernelTiling::IsBufferCapable()
{
...
void MaxPoolV3SmallKernelTiling::CalcSplitMaxRows(int64_t maxInCols)
{
...
void MaxPoolV3SmallKernelTiling::CalcSplitMaxCols(int64_t minInRows)
{
    if (minInRows <= 0) {
...
void MaxPoolV3SmallKernelTiling::CalcSplitMaxBatch(int64_t oneBacthBuffer, int64_t oneBatchInputSize)
{
...
void MaxPoolV3SmallKernelTiling::DoUBTilingSingle()
{
    int64_t maxInCols = std::max(
        (inputData.outShape[W_DIM] - 1) * inputData.stride[W_DIM] + inputData.kernelSize[W_DIM],
        inputData.inputShape[W_DIM] + inputData.pad[LEFT_PAD_INDEX] + inputData.pad[RIGHT_PAD_INDEX]);
...
void MaxPoolV3SmallKernelTiling::DoUBTiling()
{
...
void MaxPoolV3SmallKernelTiling::DoBlockTiling()
{
...
void MaxPoolV3SmallKernelTiling::CalcGatherMode()
{
    int64_t max_reg_num = (dtypeSize == B64) ? SINGLE_KERNEL_MAX_REG_NUM / DIGIT_TWO : SINGLE_KERNEL_MAX_REG_NUM;
    if (inputData.kernelSize[H_DIM] * inputData.kernelSize[W_DIM] >= maxGatherScatterElm_ / DIGIT_TWO &&
        inputData.kernelSize[H_DIM] * inputData.kernelSize[W_DIM] <= max_reg_num * maxGatherScatterElm_) {
...
void MaxPoolV3SmallKernelTiling::CalcCopyMode()
{
...
void MaxPoolV3SmallKernelTiling::SetTilingData()
{
    ...
    tilingData.set_indiceUbSize(indiceUbSize_);
...
ge::graphStatus MaxPoolV3SmallKernelTiling::DoOpTiling()
{
    DoUBTiling();
    DoBlockTiling();
    CalcCopyMode();
...
ge::graphStatus MaxPoolV3SmallKernelTiling::PostTiling()
{
...
void MaxPoolV3SmallKernelTiling::DumpTilingInfo()
{
    std::string str;
    ...
    str += " indiceUbSize:" + std::to_string(tilingData.get_indiceUbSize());
