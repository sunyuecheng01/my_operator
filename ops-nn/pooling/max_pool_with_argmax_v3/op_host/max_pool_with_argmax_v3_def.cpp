...
/*!
 * \file max_pool_with_argmax_v3_def.cpp
 * \brief imply for max_pool_with_argmax
 */
...
public:
    explicit MaxPoolWithArgmaxV3(const char* name) : OpDef(name)
    {
        ...
        this->Attr("dtype").AttrType(OPTIONAL).Int(DTYPE_INT32);
        this->Attr("dilation").AttrType(OPTIONAL).ListInt({1, 1});
