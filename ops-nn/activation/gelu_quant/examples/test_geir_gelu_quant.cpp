...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    operatorObj.set_input_##intputName(placeholder##intputIndex);                                               \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) operatorObj.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    operatorObj.set_input_##intputName(placeholder##intputIndex);                                           \
    operatorObj.update_input_desc_##intputName(placeholder##intputIndex##_desc);                            \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    operatorObj.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto operatorObj = op::GeluQuant("operatorObj");
    std::vector<int64_t> x1Shape = {4, 2};
    std::vector<int64_t> x2Shape = {1};
    std::vector<int64_t> x3Shape = {1};
    std::vector<int64_t> yOutShape = {4, 2};
    std::vector<int64_t> emptyShape = {4, 0};
    ADD_INPUT(1, x, inDtype, x1Shape);
    ADD_INPUT(2, input_scale, inDtype, x2Shape);
    ADD_INPUT(3, input_offset, inDtype, x3Shape);
    ADD_INPUT_ATTR(approximate, "tanh");
    ADD_INPUT_ATTR(quant_mode, "static");
    ADD_INPUT_ATTR(round_mode, "rint");
    ADD_INPUT_ATTR(dst_type, DT_INT8);

    outputs.push_back(operatorObj);
    // 添加完毕
    return SUCCESS;
}
