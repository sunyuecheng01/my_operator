...
/* !
 * \file gelu_static_quant_block_regbase.h
 * \brief
 */

#ifndef GELU_STATIC_QUANT_BLOCK_REGBASE_H
#define GELU_STATIC_QUANT_BLOCK_REGBASE_H
...
public:
    ...
    __aicore__ inline void ComputeCast(LocalTensor<float>& geluRes, LocalTensor<int8_t>& outLocal);
    ...
    LocalTensor<float>& geluRes, LocalTensor<float>& scaleLocalFp32, LocalTensor<float>& offsetLocalFp32)
{
    ...
    uint16_t loopNum = (colActual_ + vl - 1) / vl;
    uint32_t vlSize = vl;

    if (inputScaleType_ == NORMAL_TENSOR) {
        ComputeScale(geluRes, scaleLocalFp32);
    }

    if (inputOffsetType_ == NORMAL_TENSOR) {
        ComputeOffset(geluRes, offsetLocalFp32);
    }

    if (dstType_ == DT_INT8) {
        ComputeCast<int8_t, AscendC::RoundMode::CAST_RINT>(geluRes, outLocal);
    } else if (dstType_ == DT_FLOAT8_E4M3FN) {
        ComputeCast<fp8_e4m3fn_t, AscendC::RoundMode::CAST_RINT>(geluRes, outLocal);
    } else if (dstType_ == DT_FLOAT8_E5M2) {
        ComputeCast<fp8_e5m2_t, AscendC::RoundMode::CAST_RINT>(geluRes, outLocal);
    } else if (roundMode_ == AscendC::RoundMode::CAST_ROUND) { // hifloat8 roundMode hi8_round
        ComputeCast<hifloat8_t, AscendC::RoundMode::CAST_ROUND>(geluRes, outLocal);
    } else { // hifloat8 roundMode hi8_hybrid
        ComputeCast<hifloat8_t, AscendC::RoundMode::CAST_HYBRID>(geluRes, outLocal);
    }
    ...
    LocalTensor<float>& geluRes, LocalTensor<float>& scaleLocalFp32)
{
    ...
    __ubuf__ float* scaleAddr = (__ubuf__ float*)scaleLocalFp32.GetPhyAddr();

    int32_t dtypeSize = sizeof(float);
    uint32_t vl = VECTOR_REG_WIDTH / dtypeSize;
    uint16_t loopNum = (colActual_ + vl - 1) / vl;
    uint32_t vlSize = vl;
    uint16_t rowActualVF = static_cast<uint16_t>(rowActual_);
...
#ifdef __CCE_AICORE__
    __ubuf__ float* geluResAddr = (__ubuf__ float*)geluRes.GetPhyAddr();
    __ubuf__ float* offsetAddr = (__ubuf__ float*)offsetLocalFp32.GetPhyAddr();
...
template <typename T1, typename T2>
template <typename dstType, AscendC::RoundMode roundMode>
__aicore__ inline void StaticQuantBlock<T1, T2>::ComputeCast(LocalTensor<float>& geluRes, LocalTensor<int8_t>& outLocal)
{
#ifdef __CCE_AICORE__
    __ubuf__ float* geluResAddr = (__ubuf__ float*)geluRes.GetPhyAddr();
    __ubuf__ dstType* yAddr = (__ubuf__ dstType*)outLocal.GetPhyAddr();
        ...
        for (uint16_t i = 0; i < rowActualVF; i++) {
            ...
            for (uint16_t j = 0; j < loopNum; j++) {
                ...
                if constexpr (IsSameType<dstType, int8_t>::value) {
                    ...
                    AscendC::MicroAPI::Cast<dstType, half, castTraitF16ToI8>(vregRes, vregHalf, preg0);
                } else if constexpr (
