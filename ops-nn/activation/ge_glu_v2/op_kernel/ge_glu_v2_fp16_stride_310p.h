...
template <typename T>
__aicore__ inline void GeGluV2Fp16Stride310P<T>::Init(
    GM_ADDR x, GM_ADDR y, GM_ADDR gelu, GM_ADDR workspace, const GeGluV2TilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void GeGluV2Fp16Stride310P<T>::CopyOutGelu(
    const int64_t& index, const int64_t& length, const int64_t& group)
{
    LocalTensor<T> outLocal = m_outQueue.DeQue<T>();
    LocalTensor<T> tmpGeluLocal = m_tmpBuf.Get<T>();
    this->CopyOutBase(index, length, group, outLocal, this->yGeluGm, tmpGeluLocal, 0);
    m_outQueue.EnQue(outLocal);
}

template <typename T>
__aicore__ inline void GeGluV2Fp16Stride310P<T>::CopyOutMul(
    const int64_t& index, const int64_t& length, const int64_t& group)
{
    LocalTensor<T> outLocal = m_outQueue.DeQue<T>();
    LocalTensor<T> yLocal = outLocal[BUFFER_SIZE];
    LocalTensor<T> tmpGeluLocal = m_tmpBuf.Get<T>();
    LocalTensor<T> tmpYLocal = tmpGeluLocal[BUFFER_SIZE];
    this->CopyOutBase(index, length, group, yLocal, this->yMulGm, tmpYLocal, 1);
