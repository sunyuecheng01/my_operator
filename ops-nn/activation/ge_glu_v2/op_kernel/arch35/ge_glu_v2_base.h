...
public:
    ...
    constexpr static float negativeOne = -1.0f;
    ...
    constexpr static float ERF_SQRT_TWO_REC = 0.70710678118654752440;
    constexpr static float ERF_DOT_FIVE = 0.5;
    constexpr static float ERF_ONE = 1.0;
...
protected:
    ...
    __aicore__ inline void ComputeGeluErf(
        LocalTensor<float>& gelu_out, U& computeOutQue, LocalTensor<float>& ubx2_fp32, LocalTensor<float>& tmpBuf1,
...
protected:
    GlobalTensor<T> xGm;
    GlobalTensor<T> yGeluGm;
    GlobalTensor<T> yMulGm;
    int32_t blockIdx = 0;
    int64_t gmXOffset = 0;
    ...
    LocalTensor<float>& tmpBuf2, const int64_t length)
{
    ...
    if constexpr (std::is_same<T, float>::value) {
        gelu_out = computeOutQue.template AllocTensor<T>();
    }
