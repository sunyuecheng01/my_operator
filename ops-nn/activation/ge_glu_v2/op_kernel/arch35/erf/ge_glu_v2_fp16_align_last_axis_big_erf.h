...
template <typename T>
__aicore__ inline void GeGluV2Fp16AlignLastAxisBigErf<T>::ComputeGeluAndMul(const int64_t& ub_num)
{
    ...
    this->ComputeGeluErf(tmpBuf, outQueueGelu, ubx2_fp32, tmpBuf1, tmpBuf2, ub_num);
    ...
    Cast(tmpBuf1, ubX1, RoundMode::CAST_NONE, ub_num);
    inQueueX1.FreeTensor(ubX1);

    Mul(tmpBuf2, tmpBuf, tmpBuf1, ub_num);
    LocalTensor<T> mul_out = outQueueMul.AllocTensor<T>();
    Cast(mul_out, tmpBuf2, RoundMode::CAST_RINT, ub_num);
