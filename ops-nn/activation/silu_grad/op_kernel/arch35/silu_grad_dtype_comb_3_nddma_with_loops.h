...
/*!
 * \file silu_grad_dtype_comb_3_nddma_with_loops.h
 * \brief
 */
#ifndef ASCENDC_SILU_GRAD_DTYPE_COMB_3_NDDMA_WITH_LOOPS_H_
#define ASCENDC_SILU_GRAD_DTYPE_COMB_3_NDDMA_WITH_LOOPS_H_
...
class SiluGradDtypeComb3NddmaWithLoops {
public:
    __aicore__ inline SiluGradDtypeComb3NddmaWithLoops() {};
...
private:
    __aicore__ inline void CopyIn0(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS], int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input0Strides[tilingDataPtr_->ubSplitAxis] != 0) || (ubLoopIdx <= 1 || (AscendC::GetBlockIdx() * tilingDataPtr_->blockFormer + ubLoopIdx) % tilingDataPtr_->ubOuter <= 1)) {
            BroadcastNddmaWithLoop(inputGmDy_, bufferIn0_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides, tilingDataPtr_->input0Strides, axesIndices, tilingDataPtr_->ubSplitAxis, tilingDataPtr_->shapeLen, ubSplitSize, tilingDataPtr_->ubFormer);
        }
    ...
    __aicore__ inline void CopyIn1(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS], int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input1Strides[tilingDataPtr_->ubSplitAxis] != 0) || (ubLoopIdx <= 1 || (AscendC::GetBlockIdx() * tilingDataPtr_->blockFormer + ubLoopIdx) % tilingDataPtr_->ubOuter <= 1)) {
            BroadcastNddmaWithLoop(inputGmX_, bufferIn1_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides, tilingDataPtr_->input1Strides, axesIndices, tilingDataPtr_->ubSplitAxis, tilingDataPtr_->shapeLen, ubSplitSize, tilingDataPtr_->ubFormer);
        }
    ...
    __aicore__ inline void Compute2(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS], int64_t ubLoopIdx)
    {
        ...
        __VEC_SCOPE__ {
            ...
            for (uint16_t i = 0; i < vfLoopNum; i++) {
                ...
                AscendC::MicroAPI::DataCopy<bfloat16_t, AscendC::MicroAPI::LoadDist::DIST_UNPACK_B16>(vreg0, bufferIn0Addr + i * (AscendC::VECTOR_REG_WIDTH / 4));
                AscendC::MicroAPI::Cast<float, bfloat16_t, castTrait0>(vreg1, vreg0, preg0);
                AscendC::MicroAPI::DataCopy<float, AscendC::MicroAPI::LoadDist::DIST_NORM>(vreg3, bufferIn1Addr + i * (AscendC::VECTOR_REG_WIDTH / 4));
                AscendC::MicroAPI::Muls<float, float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg4, vreg3, static_cast<float>(-1), preg0);
                AscendC::MicroAPI::Exp<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg5, vreg4, preg0);
                AscendC::MicroAPI::Adds<float, float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg6, vreg5, static_cast<float>(1), preg0);
                AscendC::MicroAPI::Div<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg7, vreg2, vreg6, preg0);
                AscendC::MicroAPI::Sub<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg8, vreg2, vreg7, preg0);
                AscendC::MicroAPI::Mul<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg9, vreg8, vreg3, preg0);
                AscendC::MicroAPI::Add<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg10, vreg2, vreg9, preg0);
                AscendC::MicroAPI::Mul<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg11, vreg7, vreg10, preg0);
                AscendC::MicroAPI::Mul<float, AscendC::MicroAPI::MaskMergeMode::ZEROING>(vreg12, vreg1, vreg11, preg0);
                AscendC::MicroAPI::DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_NORM_B32>(bufferOut0Addr + i * (AscendC::VECTOR_REG_WIDTH / 4), vreg12, preg0);
            }
        }
...
#endif // ASCENDC_SILU_GRAD_DTYPE_COMB_3_NDDMA_WITH_LOOPS_H_
