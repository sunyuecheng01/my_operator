...
#include "nn_other.h"
#include "../op_graph/elu_grad_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    eluGrad1.set_input_##intputName(placeholder##intputIndex);                                   \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    eluGrad1.set_input_##intputName(placeholder##intputIndex);                                   \
    eluGrad1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                    \
    inputs.push_back(placeholder##intputIndex);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto eluGrad1 = op::EluGrad("eluGrad1");
    std::vector<int64_t> xShape = {4, 2};
    ADD_INPUT(1, grads, inDtype, xShape);
    ADD_INPUT(2, activations, inDtype, xShape);

    outputs.push_back(eluGrad1);
    // 添加完毕
    return SUCCESS;
}
