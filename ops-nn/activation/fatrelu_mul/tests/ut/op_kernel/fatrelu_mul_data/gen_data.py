...
def gen_data_and_golden(input_shape_str, output_size_str, d_type="float32"):
    ...
    x1 = x_tensor[..., :d]
    x2 = x_tensor[..., d:]
    m = torch.nn.Threshold(1.5, 0.0)
    x1 = m(x1)
    y_golden = x1 * x2
    tmp_golden = np.array(y_golden).astype(np_type)

    tmp_input.astype(np_type).tofile(f"{d_type}_input_fatrelu_mul.bin")
    tmp_golden.astype(np_type).tofile(f"{d_type}_golden_fatrelu_mul.bin")
