...
/*!
 * \file test_geir_dynamic_rnnv2.cpp
 * \brief
 */
#include <iostream>
#include <fstream>
...
const int T = 1; // Sequence Length
const int B = 16; // Batch size
const int INPUT_SIZE = 16; // Input size
const int HIDDEN_SIZE = 16; // Hidden size
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    dynamic_rnn_op.set_input_##intputName(placeholder##intputIndex);                       \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue)                                                  \
    dynamic_rnn_op.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    dynamic_rnn_op.set_input_##intputName(placeholder##intputIndex);                       \
    dynamic_rnn_op.update_input_desc_##intputName(placeholder##intputIndex##_desc);        \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);                          \
    dynamic_rnn_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto dynamic_rnn_op = op::DynamicRNNV2("test_geir_dynamic_rnnv2");

    // shape定义
    std::vector<int64_t> x_shape = {T, B, INPUT_SIZE};
    std::vector<int64_t> w_input_shape = {INPUT_SIZE, HIDDEN_SIZE * 4};
    std::vector<int64_t> w_hidden_shape = {HIDDEN_SIZE, HIDDEN_SIZE * 4};
    // 可选
    std::vector<int64_t> init_h_shape = {1, B, HIDDEN_SIZE};
    std::vector<int64_t> init_c_shape = {1, B, HIDDEN_SIZE};
    std::vector<int64_t> output_shape = {T, B, HIDDEN_SIZE};


    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, x, inDtype, x_shape);
    ADD_INPUT(2, weight_input, inDtype, w_input_shape);
    ADD_INPUT(3, weight_hidden, inDtype, w_hidden_shape);
    ADD_INPUT(6, init_h, inDtype, init_h_shape);
    ADD_INPUT(7, init_c, inDtype, init_c_shape);


    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, y, inDtype, output_shape);
    ADD_OUTPUT(2, output_h, inDtype, output_shape);
    ADD_OUTPUT(3, output_c, inDtype, output_shape);
    ADD_OUTPUT(4, i, inDtype, output_shape);
    ADD_OUTPUT(5, j, inDtype, output_shape);
    ADD_OUTPUT(6, f, inDtype, output_shape);
    ADD_OUTPUT(7, o, inDtype, output_shape);
    ADD_OUTPUT(8, tanhc, inDtype, output_shape);


    outputs.push_back(dynamic_rnn_op);
    // 添加完毕
    return SUCCESS;
}
