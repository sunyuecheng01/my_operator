...
/*!
 * \file test_geir_dynamic_rnn.cpp
 * \brief
 */
#include <iostream>
#include <fstream>
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto dynamic_rnn_op = op::DynamicRNN("test_geir_dynamic_rnn");

    // shape定义
    std::vector<int64_t> x_shape = {T, B, INPUT_SIZE};
    std::vector<int64_t> w_shape = {INPUT_SIZE + HIDDEN_SIZE, HIDDEN_SIZE * 4};
    std::vector<int64_t> b_shape = {HIDDEN_SIZE * 4};
    // 可选
    ...
    ADD_INPUT(2, w, inDtype, w_shape);
    ADD_INPUT(3, b, inDtype, b_shape);
