...
TEST_F(dynamic_r_n_n_test, test_case_1) {
    ...
    uint8_t* bGM = (uint8_t*)AscendC::GmAlloc(bGMByteSize);
    ...
    ReadFile(path + "/rnn_data/x.bin", aGMByteSize, aGM, aGMByteSize);
    ReadFile(path + "/rnn_data/w.bin", bGMByteSize, bGM, bGMByteSize);
    ReadFile(path + "/rnn_data/b.bin", biasGMByteSize, biasGM, biasGMByteSize);
    ReadFile(path + "/rnn_data/out.bin", cGMByteSize, goldenGM, cGMByteSize);
    ReadFile(path + "/rnn_data/inth.bin", initHCGMByteSize, initHGM, initHCGMByteSize);
    ReadFile(path + "/rnn_data/intc.bin", initHCGMByteSize, initCGM, initHCGMByteSize);
...
TEST_F(dynamic_r_n_n_test, test_case_2) {
    ...
    size_t aGMByteSize = time * batch * input_size * sizeof(float);
    size_t bGMByteSize = (input_size + hidden_size) * 4 * hidden_size * sizeof(float);
    size_t seqGMByteSize = time * batch * hidden_size * sizeof(float);
    size_t initHCGMByteSize = batch * hidden_size * sizeof(float);
    size_t tiling_data_size = sizeof(DynamicRNNTilingData);
    size_t cGMByteSize = time * batch * 4 * hidden_size * sizeof(float);
    size_t workspaceGMByteSize = 96*1024*1024;
    ...
    uint8_t* initHGM = (uint8_t*)AscendC::GmAlloc(initHCGMByteSize);
    uint8_t* initCGM = (uint8_t*)AscendC::GmAlloc(initHCGMByteSize);
    uint8_t* seqGM = (uint8_t*)AscendC::GmAlloc(seqGMByteSize);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tiling_data_size);
    ...
    tilingDatafromBin->isTraining = 1;
    tilingDatafromBin->cellClip = 1.0;
    tilingDatafromBin->forgetBias = 1.0;
