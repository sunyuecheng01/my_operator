...
int aclnnConvolutionTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<int64_t> shapeInput = {2, 2, 2, 2, 2};
    std::vector<int64_t> shapeWeight = {1, 2, 1, 1, 1};
    std::vector<int64_t> shapeResult = {2, 1, 2, 2, 2};
    ...
    ret = aclnnConvolutionGetWorkspaceSize(
        input, weight, nullptr, strides, pads, dilations, false, outPads, 1, result, 1, &workspaceSize, &executor);
    CHECK_FREE_RET(
