    ...
    const aclTensor* totalWeight, const aclTensor* out)
{
    ...
    OP_CHECK_DTYPE_NOT_MATCH(self, totalWeight->GetDataType(), return false);
...
{
    if (reduction > MAX_REDUCTION || reduction < 0) {
    ...
    const aclTensor* totalWeight, const aclTensor* out, int64_t reduction)
{
    ...
    OP_CHECK(selfDimNum == 4, OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Input tensor should be 4D."), return false);
    ...
    aclOpExecutor** executor)
{
    ...
    // 将输入weight转换成连续的tensor
    auto weightInput = l0op::Contiguous(weight, uniqueExecutor.get());
    CHECK_RET(weightInput != nullptr, ACLNN_ERR_INNER_NULLPTR);

    auto weightCast = l0op::Cast(weightInput, promoteType, uniqueExecutor.get());
    ...
    if (GetCurrentPlatformInfo().GetSocVersion() != SocVersion::ASCEND910_95){
        ...
        CHECK_RET(gradOutputIn != nullptr, ACLNN_ERR_INNER_NULLPTR);
        ...
        OP_LOGD("Transfer target type to int32");
        targetCasted = l0op::Cast(targetReshaped, op::DataType::DT_INT32, uniqueExecutor.get());
        ...
        // 将计算结果转换成输出out的数据类型
        auto castOut = l0op::Cast(gradInputTransposed, out->GetDataType(), uniqueExecutor.get());
        ...
        // 将计算结果转换成输出out的数据类型
        auto castOut = l0op::Cast(gradInput, out->GetDataType(), uniqueExecutor.get());
        CHECK_RET(castOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
...
{
    L2_DFX_PHASE_2(aclnnNLLLoss2dBackward);
    // 固定写法，调用框架能力，完成计算
