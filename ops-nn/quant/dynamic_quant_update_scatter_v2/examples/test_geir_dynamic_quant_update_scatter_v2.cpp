...
#include "nn_other.h"
#include "../op_graph/dynamic_quant_update_scatter_v2_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto dynamic_quant_op = op::DynamicQuantUpdateScatterV2("test_geir_dynamic_quant_update_scatter_v2");

    // shape定义
    std::vector<int64_t> x_shape = {B, 1, H};
    std::vector<int64_t> indices_shape = {B};
    std::vector<int64_t> var_shape = {B, S, 1, H};
    std::vector<int64_t> scale_in_shape = {B, S};
    std::vector<int64_t> offset_in_shape = {B, S};

    std::vector<int64_t> var_out_shape = {B, S, H};
    std::vector<int64_t> scale_out_shape = {1, B, S};
    std::vector<int64_t> offset_out_shape = {1, B, S};

    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, x, inDtype, x_shape);
    ADD_INPUT(2, indices, DT_INT32, indices_shape);
    ADD_INPUT(3, var, DT_INT4, var_shape);
    ADD_INPUT(4, var_scale, DT_FLOAT, scale_in_shape);
    ADD_INPUT(5, var_offset, DT_FLOAT, offset_in_shape);

    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, var, DT_INT4, var_out_shape);
    ADD_OUTPUT(2, var_scale, DT_FLOAT, scale_out_shape);
    ADD_OUTPUT(3, var_offset, DT_FLOAT, offset_out_shape);
