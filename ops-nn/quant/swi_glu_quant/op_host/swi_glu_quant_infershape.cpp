...
/*!
 * \file swi_glu_quant_infershape.cpp
 * \brief
 */
...
static ge::graphStatus InferShapeForSwiGluQuant(gert::InferShapeContext* context) {
    ...
    *y_shape = *x_shape;
    ...
    if (x_shape->GetDim(split_dim) < 0 || x_shape->GetDim(split_dim) % SPLIT_NUM != 0) {
        D_OP_LOGE("SwiGluQuant", "The shape [%s] is not divisible by 2.", Ops::Base::ToString(*x_shape).c_str());
...
static ge::graphStatus InferDataTypeForSwiGluQuant(gert::InferDataTypeContext *context) {
    ...
    OP_LOGD(context, "====Enter SwiGluQuant inferDataType impl end.=====");
    return ge::GRAPH_SUCCESS;
}
