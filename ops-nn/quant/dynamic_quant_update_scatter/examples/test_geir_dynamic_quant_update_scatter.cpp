...
const int B = 24; // Batch size
const int S = 1; // Sequence length
const int H = 128; // Hidden size
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    dynamic_quant_op.set_input_##intputName(placeholder##intputIndex);                       \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue)                                                  \
    dynamic_quant_op.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    dynamic_quant_op.set_input_##intputName(placeholder##intputIndex);                       \
    dynamic_quant_op.update_input_desc_##intputName(placeholder##intputIndex##_desc);        \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);                          \
    dynamic_quant_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto dynamic_quant_op = op::DynamicQuantUpdateScatter("test_geir_dynamic_quant_update_scatter");

    // shape定义
    std::vector<int64_t> x_shape = {B, 1, H};
    std::vector<int64_t> var_scale_shape = {B, 1, 1};
    std::vector<int64_t> indices_shape = {B};
    std::vector<int64_t> updates_shape = {B, 1, H};
    std::vector<int64_t> smooth_scales_shape = {H};

    std::vector<int64_t> var_out_shape = {B, 1, H};
    std::vector<int64_t> scale_out_shape = {B, 1, 1};

    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, var, DT_INT8, x_shape);
    ADD_INPUT(2, var_scale, DT_FLOAT, var_scale_shape);
    ADD_INPUT(3, indices, DT_INT32, indices_shape);
    ADD_INPUT(4, updates, DT_BF16, updates_shape);
    ADD_INPUT(5, smooth_scales, DT_BF16, smooth_scales_shape);

    // 添加必选属性（顺序严格匹配 proto.h）
    ADD_INPUT_ATTR(reduce, "update");
    ADD_INPUT_ATTR(axis, -2);
    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, var, DT_INT8, var_out_shape);
    ADD_OUTPUT(2, var_scale, DT_FLOAT, scale_out_shape);

    outputs.push_back(dynamic_quant_op);
    // 添加完毕
    return SUCCESS;
}
