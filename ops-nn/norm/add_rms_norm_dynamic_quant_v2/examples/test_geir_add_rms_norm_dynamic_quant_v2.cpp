    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::AddRmsNormDynamicQuantV2("add1");
    std::vector<int64_t> x1Shape = {4, 1, 8};
    std::vector<int64_t> x2Shape = {4, 1, 8};
    std::vector<int64_t> gammaShape = {8};
    std::vector<int64_t> scale1Shape = {8};
    std::vector<int64_t> scale2Shape = {8};

    ADD_INPUT(1, x1, inDtype, x1Shape);
    ADD_INPUT(2, x2, inDtype, x2Shape);
    ADD_INPUT(3, gamma, inDtype, gammaShape);
    ADD_INPUT(4, smooth_scale1, inDtype, scale1Shape);
    ADD_INPUT(5, smooth_scale2, inDtype, scale2Shape);
