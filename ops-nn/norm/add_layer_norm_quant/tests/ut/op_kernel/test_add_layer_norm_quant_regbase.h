...
#ifndef ADD_LAYER_NORM_NORM_QUANT_REGBASE_TILING_H_
#define ADD_LAYER_NORM_NORM_QUANT_REGBASE_TILING_H_
...
struct AddLayerNormQuantRegbaseTilingData {
    uint32_t blockSize;
...
#define GET_TILING_DATA(tilingData, tilingPointer)                                          \
    AddLayerNormQuantRegbaseTilingData tilingData;                                          \
    INIT_TILING_DATA(AddLayerNormQuantRegbaseTilingData, tilingDataPointer, tilingPointer); \
...
inline[aicore] void InitTilingData(const __gm__ uint8_t* tiling, AddLayerNormQuantRegbaseTilingData* constData)
{
    const __gm__ int64_t* src = (const __gm__ int64_t*)tiling;
    int64_t* dst = (int64_t*)constData;
    for (auto i = 0; i < sizeof(AddLayerNormQuantRegbaseTilingData) / sizeof(int64_t); i++)
        *(dst + i) = *(src + i);
}
#else
inline void InitTilingData(uint8_t* tiling, AddLayerNormQuantRegbaseTilingData* constData)
{
    memcpy(constData, tiling, sizeof(AddLayerNormQuantRegbaseTilingData));
}
#endif
...
#endif // _ADD_LAYER_NORM_NORM_QUANT_REGBASE_TILING_H_
