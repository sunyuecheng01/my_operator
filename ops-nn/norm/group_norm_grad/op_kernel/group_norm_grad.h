...
public:
        ...
        GM_ADDR workspace, const GroupNormGradTilingData* tiling_data)
    {
        ...
        this->dx_is_require = tiling_data->dx_is_require;
        this->dgamma_is_require = tiling_data->dgamma_is_require;
        this->dbeta_is_require = tiling_data->dbeta_is_require;
...
private:
    ...
    __aicore__ inline void CopyIn_Mode_0(int32_t task_idx)
    {
        ...
        Adds_Muls_template(
            x_local, x_local, (float(-1.0) * this->mean_scalar * this->rstd_scalar), this->rstd_scalar,
        ...
        for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
            ...
            if (is_align == 0) {
                float temp1I = ReduceSum_template(
                    ...
                    this->ele_num_per_channel, true);
                temp_1_local.SetValue(C_G_idx, temp1I);
                event_t eventIDSToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_V));
                SetFlag<HardEvent::S_V>(eventIDSToV);
                WaitFlag<HardEvent::S_V>(eventIDSToV);
                Mul(temp_hxw_local, x_local[C_G_idx * this->ele_num_per_channel],
                    dy_local[C_G_idx * this->ele_num_per_channel], this->ele_num_per_channel);
                float temp2I =
                ...
                WaitFlag<HardEvent::MTE2_V>(eventIDMTE2ToV);
                Adds_Muls_template(
                    unalign_channel_x, unalign_channel_x, (float(-1.0) * this->mean_scalar * this->rstd_scalar),
        ...
        if (dx_is_require) {
            ...
            for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
                ...
                if (is_align == 0) {
                    ...
                    WaitFlag<HardEvent::S_V>(eventIDSToV);
                    Muls(dy_local[offset], dy_local[offset], (this->rstd_scalar * gamma), this->ele_num_per_channel);
                    LocalTensor<float> temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
                    ...
                    Adds_Muls_template(
                        x_local[offset], x_local[offset],
                        (this->rstd_scalar * c1 - this->mean_scalar * this->rstd_scalar * this->rstd_scalar * c2),
                        (this->rstd_scalar * this->rstd_scalar * c2), this->ele_num_per_channel);
                    WaitFlag<HardEvent::S_V>(eventIDSToV);
                    Muls(dy_local[offset], dy_local[offset], (this->rstd_scalar * gamma), this->ele_num_per_channel);
                    LocalTensor<float> temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
                    Sub(temp_hxw_local, dy_local[offset], x_local[offset], this->ele_num_per_channel);
    ...
    __aicore__ inline void Compute_Mode_1(int32_t task_idx)
    {
        ...
        for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
            ...
            Adds_Muls_template(
                x_local, x_local, (-1.0f * this->mean_scalar * this->rstd_scalar), (this->rstd_scalar),
                this->ele_num_per_channel);
        ...
        if (dx_is_require) {
            ...
            for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
                ...
                Adds_Muls_template(
                    x_local, x_local, (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                    (this->rstd_scalar * c2), this->ele_num_per_channel);
                Sub(dy_local, dy_local, x_local, this->ele_num_per_channel);
                in_queue_x.FreeTensor(x_local);
                temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
                Muls(temp_hxw_local, dy_local, (this->rstd_scalar), this->ele_num_per_channel);
                in_queue_dy.FreeTensor(dy_local);
                event_t eventIDVToMTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
                SetFlag<HardEvent::V_MTE3>(eventIDVToMTE3);
                WaitFlag<HardEvent::V_MTE3>(eventIDVToMTE3);
    ...
    __aicore__ inline void Compute_Mode_2(int32_t task_idx)
    {
        ...
        for (int32_t C_G_idx = 0; C_G_idx < this->C_G / DOUBLE_BUFFER_; C_G_idx++) {
            ...
            for (int i = 0; i < DOUBLE_BUFFER_; i++) {
                Adds_Muls_template(
                    x_local[i], x_local[i], (-1.0f * this->mean_scalar * this->rstd_scalar), this->rstd_scalar,
                    this->ele_num_per_channel);
            }
            for (int i = 0; i < DOUBLE_BUFFER_; i++) {
        ...
        if (C_G != restChannelStartIdx) {
            ...
            Adds_Muls_template(
                x_local[0], x_local[0], (-1.0f * this->mean_scalar * this->rstd_scalar), this->rstd_scalar,
                this->ele_num_per_channel);
        ...
        if (dx_is_require) {
            ...
            for (int32_t C_G_idx = 0; C_G_idx < this->C_G / DOUBLE_BUFFER_; C_G_idx++) {
                ...
                for (int i = 0; i < DOUBLE_BUFFER_; i++) {
                    ...
                    Adds_Muls_template(
                        x_local[i], x_local[i], (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                        (this->rstd_scalar * c2), this->ele_num_per_channel);
                    ...
                    Muls(temp_hxw_local[i], dy_local[i], this->rstd_scalar, this->ele_num_per_channel);
            ...
            if (C_G != restChannelStartIdx) {
                ...
                Adds_Muls_template(
                    x_local[0], x_local[0], (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                    (this->rstd_scalar * c2), this->ele_num_per_channel);
                Sub(dy_local[0], dy_local[0], x_local[0], this->ele_num_per_channel);
                in_queue_x.FreeTensor(x_local[0]);
                temp_hxw_local[0] = temp_queue_HXW.AllocTensor<float>();
                Muls(temp_hxw_local[0], dy_local[0], this->rstd_scalar, this->ele_num_per_channel);
                in_queue_dy.FreeTensor(dy_local[0]);
        ...
        if (dx_is_require) {
            ...
            for (int32_t iter_C_G_idx = 0; iter_C_G_idx < this->C_G * this->mode2_ub_iteration_num; iter_C_G_idx++) {
                ...
                if (iter_C_G_idx % this->mode2_ub_iteration_num != this->mode2_ub_iteration_num - 1) {
                    ...
                    Adds_Muls_template(
                        x_local, x_local, (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                        (this->rstd_scalar * c2), this->mode2_ub_capacity_ele);
                    Sub(dy_local, dy_local, x_local, this->mode2_ub_capacity_ele);
                    LocalTensor<float> temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
                    Muls(temp_hxw_local, dy_local, this->rstd_scalar, this->mode2_ub_capacity_ele);
                    in_queue_x.FreeTensor(x_local);
                    in_queue_dy.FreeTensor(dy_local);
                    event_t eventIDVToMTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
                    SetFlag<HardEvent::V_MTE3>(eventIDVToMTE3);
                    ...
                    Adds_Muls_template(
                        x_local, x_local, (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                        (this->rstd_scalar * c2), this->mode2_ub_tail_num);
                    Sub(dy_local, dy_local, x_local, this->mode2_ub_tail_num);
                    LocalTensor<float> temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
                    Muls(temp_hxw_local, dy_local, this->rstd_scalar, this->mode2_ub_tail_num);
                    in_queue_x.FreeTensor(x_local);
                    in_queue_dy.FreeTensor(dy_local);
                    event_t eventIDVToMTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
                    SetFlag<HardEvent::V_MTE3>(eventIDVToMTE3);
    ...
    __aicore__ inline void Compute_Mode_5(int32_t task_idx)
    {
        ...
        if (dx_is_require) {
            ...
            out_queue_dgamma.FreeTensor(temp_2_local);
            in_queue_gamma.FreeTensor(gamma_local);
            Adds_Muls_template(x_local, x_local, this->rstd_scalar * sumC1, this->rstd_scalar * sumC2, this->C_G);
            Muls(dy_local, dy_local, this->rstd_scalar, this->C_G);
            LocalTensor<float> temp_hxw_local = temp_queue_HXW.AllocTensor<float>();
    ...
    __aicore__ inline void Init_Unified_Buffer_FP32_MODE_0(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        ...
        if (!(this->ele_num_per_channel % float_per_block == 0 || this->C_G == 1)) {
            pipe.InitBuffer(in_temp_x, HxW_alloc_space);
            pipe.InitBuffer(in_temp_dy, HxW_alloc_space);
        }
    }

    __aicore__ inline void Init_Unified_Buffer_FP32_MODE_1(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
    ...
    __aicore__ inline void Init_Unified_Buffer_FP32_MODE_2(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        uint32_t HxW_alloc_space = ceil(this->ele_num_per_channel, T_per_block) * sizeof(float);
        uint32_t NxG_alloc_space = T_per_block * sizeof(float);
        uint32_t C_G_alloc_space = ceil(this->C_G, T_per_block) * sizeof(float);
        pipe.InitBuffer(in_queue_dy, DOUBLE_BUFFER, HxW_alloc_space);
        pipe.InitBuffer(in_queue_x, DOUBLE_BUFFER, HxW_alloc_space);
        pipe.InitBuffer(in_queue_gamma, 1, C_G_alloc_space);
        // VEC_OUT
        pipe.InitBuffer(out_queue_dgamma, 1, C_G_alloc_space);
        pipe.InitBuffer(out_queue_dbeta, 1, C_G_alloc_space);
        pipe.InitBuffer(temp_queue_HXW, DOUBLE_BUFFER, HxW_alloc_space);
    }

    __aicore__ inline void Init_Unified_Buffer_FP32_MODE_3(const GroupNormGradTilingData* tiling_data)
    {
    ...
    __aicore__ inline void Init_Unified_Buffer_FP32_MODE_5(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        uint32_t C_G_alloc_space = ceil(this->C_G, T_per_block) * sizeof(T);
    ...
    __aicore__ inline void Init_Unified_Buffer_FP16_BF16_MODE_0(const GroupNormGradTilingData* tiling_data)
    {
    ...
    __aicore__ inline void Init_Unified_Buffer_FP16_BF16_MODE_1(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        uint32_t HxW_alloc_space = ceil(this->ele_num_per_channel, T_per_block);
        uint32_t NxG_alloc_space = T_per_block;
    ...
    __aicore__ inline void Init_Unified_Buffer_FP16_BF16_MODE_2(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        uint32_t HxW_alloc_space = ceil(this->ele_num_per_channel, T_per_block);
        uint32_t NxG_alloc_space = T_per_block;
        uint32_t C_G_alloc_space = ceil(this->C_G, T_per_block);
        pipe.InitBuffer(in_queue_dy, DOUBLE_BUFFER, HxW_alloc_space * sizeof(float));
        pipe.InitBuffer(in_tbuf_dy_T, HxW_alloc_space * sizeof(T));
        pipe.InitBuffer(in_queue_x, DOUBLE_BUFFER, HxW_alloc_space * sizeof(float));
        pipe.InitBuffer(in_tbuf_x_T, HxW_alloc_space * sizeof(T));
        pipe.InitBuffer(in_queue_gamma, 1, C_G_alloc_space * sizeof(float));
        pipe.InitBuffer(in_tbuf_gamma_T, C_G_alloc_space * sizeof(T));
        pipe.InitBuffer(out_tbuf_dgamma_T, C_G_alloc_space * sizeof(T));
        ...
        pipe.InitBuffer(out_queue_dgamma, 1, C_G_alloc_space * sizeof(float));
        pipe.InitBuffer(out_queue_dbeta, 1, C_G_alloc_space * sizeof(float));
        pipe.InitBuffer(temp_queue_HXW, DOUBLE_BUFFER, HxW_alloc_space * sizeof(float));
    }

    __aicore__ inline void Init_Unified_Buffer_FP16_BF16_MODE_3(const GroupNormGradTilingData* tiling_data)
    {
        this->mode2_ub_capacity_ele = tiling_data->mode2_ub_capacity_ele;
        this->mode2_ub_iteration_num = tiling_data->mode2_ub_iteration_num;
        this->mode2_ub_tail_num = tiling_data->mode2_ub_tail_num;
    ...
    __aicore__ inline void Init_Unified_Buffer_FP16_BF16_MODE_5(const GroupNormGradTilingData* tiling_data)
    {
        // VEC_IN
        uint32_t C_G_alloc_space = ceil(this->C_G, T_per_block);
    ...
    __aicore__ inline void Init_Unified_Buffer(const GroupNormGradTilingData* tiling_data)
    {
        if constexpr (IsSameType<T, float>::value) {
            if (Tiling_key == MODE_0) {
                Init_Unified_Buffer_FP32_MODE_0(tiling_data);
            } else if (Tiling_key == MODE_1) {
                Init_Unified_Buffer_FP32_MODE_1(tiling_data);
            } else if (Tiling_key == MODE_2) {
                Init_Unified_Buffer_FP32_MODE_2(tiling_data);
            } else if (Tiling_key == MODE_3) {
                Init_Unified_Buffer_FP32_MODE_3(tiling_data);
            } else if (Tiling_key == MODE_5) {
                Init_Unified_Buffer_FP32_MODE_5(tiling_data);
            }
        } else if constexpr (IsSameType<T, half>::value || IsSameType<T, bfloat16_t>::value) {
            if (Tiling_key == MODE_0) {
                Init_Unified_Buffer_FP16_BF16_MODE_0(tiling_data);
            } else if (Tiling_key == MODE_1) {
                Init_Unified_Buffer_FP16_BF16_MODE_1(tiling_data);
            } else if (Tiling_key == MODE_2) {
                Init_Unified_Buffer_FP16_BF16_MODE_2(tiling_data);
            } else if (Tiling_key == MODE_3) {
                Init_Unified_Buffer_FP16_BF16_MODE_3(tiling_data);
            } else if (Tiling_key == MODE_5) {
                Init_Unified_Buffer_FP16_BF16_MODE_5(tiling_data);
            }
        }
    }
    ...
    __aicore__ inline void Adds_Muls_template(
        const LocalTensor<float>& dstLocal, const LocalTensor<float>& srcLocal, float adds_scalarValue,
    ...
    __aicore__ inline void Adds_Muls_template(
        const LocalTensor<float>& mul_dstLocal, const LocalTensor<float>& mul_srcLocal,
        const LocalTensor<float>& add_dstLocal, const LocalTensor<float>& add_srcLocal, float adds_scalarValue,
        float muls_scalarValue, const int32_t calCount)
    {
            ...
            if (calCount > MASK) {
                for (int32_t start = MAX_LEN; start >= 1; start >>= 1) {
                    ...
                    if (calCount > innerOffset && firstAdd) {
                        ...
                        Add<float, false>(
                            dstLocal, dstLocal[innerOffset], dstLocal, MASK, start,
    ...
    __aicore__ inline void Mul_ReduceSum_template(
        ...
        uint32_t repeatTime, uint32_t count, const LocalTensor<float>& dstLocal)
    {
        ...
        DataCopyExtParams copyParams_in{
            (uint16_t)repeatTime, (uint16_t)(count * sizeof(float)), (uint16_t)((C - count) * sizeof(float)),
        ...
        uint32_t start_offset, int64_t channel_idx, uint32_t count, uint32_t reduce_axis_num)
    {
        ...
        for (uint32_t loop_idx = 0; loop_idx < divceil(reduce_axis_num, coreBatchParts); loop_idx++) {
            ...
            if (dgamma_is_require) {
                reduce_axis_n_WSP2UB(
                    in_queue_dgamma_channel, dgamma_workspace, start_offset + loop_offset + channel_idx, repeatTime,
                    count, dgamma_sum_Local);
            }
            if (dbeta_is_require) {
                reduce_axis_n_WSP2UB(
                    in_queue_dbeta_channel, dbeta_workspace, start_offset + loop_offset + channel_idx, repeatTime,
                    count, dbeta_sum_Local);
            }
        }
        event_t eventIDVToMTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
        SetFlag<HardEvent::V_MTE3>(eventIDVToMTE3);
        ...
        int64_t channel_idx, uint32_t count, uint32_t reduce_axis_num)
    {
        ...
        for (uint32_t loop_idx = 0; loop_idx < divceil(reduce_axis_num, coreBatchParts); loop_idx++) {
            ...
            if (dgamma_is_require) {
                reduce_axis_n_WSP2UB(
                    in_queue_dgamma_channel, dgamma_workspace, loop_offset + channel_idx, repeatTime, count,
                    dgamma_sum_Local);
            }
            if (dbeta_is_require) {
                reduce_axis_n_WSP2UB(
                    in_queue_dbeta_channel, dbeta_workspace, loop_offset + channel_idx, repeatTime, count,
                    dbeta_sum_Local);
            }
        }
        PipeBarrier<PIPE_ALL>();
