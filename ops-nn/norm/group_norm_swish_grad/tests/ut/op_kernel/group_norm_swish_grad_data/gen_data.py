...
if __name__ == "__main__":
    ...
    beta = np.zeros(shape=(C), dtype=np_dtype)
    for n_i in range(N):
        for g_i in range(G):
            Var = np.var(x[n_i, g_i*(C//G):(g_i+1)*(C//G)])
            Rstd[n_i * G + g_i] = 1/((Var+1e-5)**0.5)
            Mean[n_i * G + g_i] = np.mean(x[n_i, g_i*(C//G):(g_i+1)*(C//G)])
    inputs_npu = {"dy":dy.astype(np.float32) ,"mean": Mean.astype(np.float32), "rstd":Rstd.astype(np.float32), "x": x.astype(np.float32), "gamma": gamma.astype(np.float32), "beta": beta.astype(np.float32)}
