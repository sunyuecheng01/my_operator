...
extern "C" __global__ __aicore__ void ada_layer_norm_v2(
    GM_ADDR x, GM_ADDR scale, GM_ADDR shift, GM_ADDR weight, GM_ADDR bias, GM_ADDR out, GM_ADDR mean, GM_ADDR rstd,
...
#define INIT_AND_PROCESS             \
    ...
    if (TILING_KEY_IS(1)) {
        if constexpr (std::is_same_v<DTYPE_X, half>) {
            AdaLayerNormND<half, half, BASE_V2_OP_CODE> op;
            INIT_AND_PROCESS;
        }
        if constexpr (std::is_same_v<DTYPE_X, bfloat16_t>) {
            AdaLayerNormND<bfloat16_t, bfloat16_t, BASE_V2_OP_CODE> op;
            INIT_AND_PROCESS;
        }
    } else if (TILING_KEY_IS(2)) {
        if constexpr (std::is_same_v<DTYPE_X, float>) {
            AdaLayerNormND<float, float, BASE_V2_OP_CODE> op;
            INIT_AND_PROCESS;
        }
        if constexpr (std::is_same_v<DTYPE_X, half>) {
            AdaLayerNormND<half, float, BASE_V2_OP_CODE> op;
            INIT_AND_PROCESS;
        }
        if constexpr (std::is_same_v<DTYPE_X, bfloat16_t>) {
            AdaLayerNormND<bfloat16_t, float, BASE_V2_OP_CODE> op;
            INIT_AND_PROCESS;
        }
    }
}