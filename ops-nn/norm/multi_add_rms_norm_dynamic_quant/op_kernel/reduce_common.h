    ...
    const uint8_t repStride)
{
    ...
    for (; elemIndex + MultiAddRNDQ::ELEM_PER_REP_FP32 <= numLastDim; elemIndex += MultiAddRNDQ::ELEM_PER_REP_FP32) {
        Add(tmpLocal, srcLocal[elemIndex], tmpLocal, elemNum, repeat,
            {1, 1, 1, MultiAddRNDQ::ELEM_PER_BLK_FP32, repStride, MultiAddRNDQ::ELEM_PER_BLK_FP32});
        PipeBarrier<PIPE_V>();
    }
    ...
    const uint32_t numRow, const uint32_t numCol, const uint32_t numColAlign)
{
    ...
    const uint8_t repStride = numColAlign / MultiAddRNDQ::ELEM_PER_BLK_FP32;
    Duplicate(tmpLocal, MultiAddRNDQ::ZERO, numRow * MultiAddRNDQ::ELEM_PER_REP_FP32);
    PipeBarrier<PIPE_V>();
...
__aicore__ inline int32_t findPowerTwo(int32_t n)
{
    ...
    n |= n >> MultiAddRNDQ::INDEX_TWO;
    n |= n >> MultiAddRNDQ::INDEX_FOUR;
    n |= n >> MultiAddRNDQ::INDEX_EIGHT;
    n |= n >> MultiAddRNDQ::INDEX_SIXTEEN;
    return (n + 1) >> 1;
}
    ...
    const LocalTensor<float>& dst_local, const LocalTensor<float>& src_local, int32_t count)
{
    if (likely(count > MultiAddRNDQ::ELEM_PER_REP_FP32)) {
        ...
        while (bodyCount > MultiAddRNDQ::ELEM_PER_REP_FP32) {
            bodyCount = bodyCount / MultiAddRNDQ::HALf_INTERVAL;
            Add(src_local, src_local, src_local[bodyCount], bodyCount);
            PipeBarrier<PIPE_V>();
        }

        AscendCUtils::SetMask<float>(MultiAddRNDQ::ELEM_PER_REP_FP32);
    } else {
