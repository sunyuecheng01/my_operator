...
/*!
 * \file multi_add_rms_norm_dynamic_quant_helper.h
 * \brief
 */

#ifndef __multi_add_rms_norm_dynamic_quant_HELPER_H_
#define __multi_add_rms_norm_dynamic_quant_HELPER_H_
...
{
    uint64_t repsFp32 = count >> 6;       // 6 is cound / MultiAddRNDQ::ELEM_PER_REP_FP32
    uint64_t offsetsFp32 = repsFp32 << 6; // 6 is repsFp32 * MultiAddRNDQ::ELEM_PER_REP_FP32
    uint64_t remsFp32 = count & 0x3f;     // 0x3f 63, count % MultiAddRNDQ::ELEM_PER_REP_FP32
    ...
    uint32_t mask = (repsFp32 > 0) ? MultiAddRNDQ::ELEM_PER_REP_FP32 : count;
    // 8 is rep stride
    ...
    float divisorScalar, uint32_t count)
{
    ...
    PipeBarrier<PIPE_V>();
    Div(dstTensor, dividendTensor, tmpTensor, MultiAddRNDQ::ELEM_PER_REP_FP32, repsFp32, {1, 1, 0, 8, 8, 0});
    if ((remsFp32 > 0)) {
...
#endif // __multi_add_rms_norm_dynamic_quant_HELPER_H_
