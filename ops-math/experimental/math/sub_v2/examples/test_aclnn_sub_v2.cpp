...
#include <vector>
#include "acl/acl.h"
#include "aclnn_sub_v2.h"
using DataType = float; // bfloat16_t
...
int main()
{
    ...
    std::vector<DataType> selfXHostData(64, 2);
    ...
    std::vector<int64_t> selfYShape = {8, 8};
    std::vector<DataType> selfYHostData(64, 1);
    ...
    std::vector<DataType> outHostData(64, 0);
    ...
    // 5. 调用aclnnSubExample第二段接口
    ret = aclnnSubV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSubExample failed. ERROR: %d\n", ret); return ret);
