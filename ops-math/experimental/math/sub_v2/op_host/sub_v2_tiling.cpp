...
// tiling 分发入口
static ge::graphStatus SubV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        SubV2TilingData* tiling = context->GetTilingData<SubV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(SubV2TilingData), 0, sizeof(SubV2TilingData));
    ...
    SubV2TilingData* tiling = context->GetTilingData<SubV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(SubV2TilingData), 0, sizeof(SubV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForSubV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}
// tiling注册入口.
IMPL_OP_OPTILING(SubV2).Tiling(SubV2TilingFunc).TilingParse<SubV2CompileInfo>(TilingParseForSubV2);
} // namespace optiling
