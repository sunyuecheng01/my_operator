...
#include "../op_kernel/div_v2_tiling_data.h"
#include "../op_kernel/div_v2_tiling_key.h"
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
   // 获取输入shape信息
    ...
    // dtype校验
    const std::set<ge::DataType> supportedDtype = {ge::DT_FLOAT, ge::DT_INT32, ge::DT_INT16, ge::DT_FLOAT16, ge::DT_BF16};
...
// tiling 分发入口
// 可直接替换你的 DivV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus DivV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        DivV2TilingData* tiling = context->GetTilingData<DivV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(DivV2TilingData), 0, sizeof(DivV2TilingData));
    ...
    DivV2TilingData* tiling = context->GetTilingData<DivV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(DivV2TilingData), 0, sizeof(DivV2TilingData)) != EOK,
    ...
    uint32_t ubDataNumber = (inputBytes == 1ULL) ? 7U : 5U; //5U:3U
...
static ge::graphStatus TilingParseForDivV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(DivV2).Tiling(DivV2TilingFunc).TilingParse<DivV2CompileInfo>(TilingParseForDivV2);
} // namespace optiling
