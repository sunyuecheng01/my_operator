...
static ge::graphStatus TilingParseForSign([[maybe_unused]] gert::TilingParseContext* context)
{
...
static ge::graphStatus SignTilingFunc(gert::TilingContext* context)
{
    // SignTilingData tiling;
    SignTilingData* tiling = context->GetTilingData<SignTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(SignTilingData), 0, sizeof(SignTilingData)) != EOK,
...
// tiling注册入口.
IMPL_OP_OPTILING(Sign).Tiling(SignTilingFunc).TilingParse<SignCompileInfo>(TilingParseForSign);
} // namespace optiling
