...
#include "../op_kernel/minimum_tiling_data.h"
#include "../op_kernel/minimum_tiling_key.h"
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    // 形状不匹配则报错
     OP_CHECK_IF(
        !shapeMatch,
        OP_LOGE(
            context, "Minimum: inputx,outputz shape not match! dim num: x=%zu,  z=%zu",
            inputShapeX.GetDimNum(), outShapeZ.GetDimNum()),
        return ge::GRAPH_FAILED);
    totalIdx = inputX->GetOriginShape().GetShapeSize();
    // dtype校验
    const std::set<ge::DataType> supportedDtype = {ge::DT_FLOAT, ge::DT_FLOAT16, ge::DT_BF16, ge::DT_INT8, ge::DT_INT16, ge::DT_INT32};
...
// tiling 分发入口
// 可直接替换你的 MinimumTilingFunc 内部实现（保留函数签名）
static ge::graphStatus MinimumTilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        MinimumTilingData* tiling = context->GetTilingData<MinimumTilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(MinimumTilingData), 0, sizeof(MinimumTilingData));
    ...
    MinimumTilingData* tiling = context->GetTilingData<MinimumTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(MinimumTilingData), 0, sizeof(MinimumTilingData)) != EOK,
    ...
    uint32_t ubDataNumber = (dataType == ge::DT_INT8 || dataType == ge::DT_BF16) ? 7U : 5U;
...
static ge::graphStatus TilingParseForMinimum([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(Minimum).Tiling(MinimumTilingFunc).TilingParse<MinimumCompileInfo>(TilingParseForMinimum);
} // namespace optiling