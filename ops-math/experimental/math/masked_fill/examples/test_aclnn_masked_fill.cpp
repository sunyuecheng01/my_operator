...
int main()
{
    ...
    std::vector<uint8_t> maskHostData = {0, 0, 0, 0, 1, 1, 1, 1};
    ...
    // 调用aclnnMaskedFill第一段接口
    ret = aclnnMaskedFillGetWorkspaceSize(selfRef, mask, value, out, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
        const char* errMsg = aclGetRecentErrMsg();
        LOG_PRINT("[ERROR] aclnnLinSpaceDGetWorkspaceSize failed: %s", errMsg ? errMsg : "nullptr");
    }
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMaskedFillGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    // 调用aclnnMaskedFill第二段接口
    ret = aclnnMaskedFill(workspaceAddr, workspaceSize, executor, stream);
    if (ret != ACL_SUCCESS) {
        const char* errMsg = aclGetRecentErrMsg();
        LOG_PRINT("[ERROR] aclnnMaskedFill failed: %s", errMsg ? errMsg : "nullptr");
    }
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMaskedFill failed. ERROR: %d\n", ret); return ret);
