...
#include "../op_kernel/tan_v2_tiling_data.h"
#include "../op_kernel/tan_v2_tiling_key.h"
...
// tiling 分发入口
// 可直接替换你的 TanV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus TanV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        TanV2TilingData* tiling = context->GetTilingData<TanV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(TanV2TilingData), 0, sizeof(TanV2TilingData));
    ...
    TanV2TilingData* tiling = context->GetTilingData<TanV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(TanV2TilingData), 0, sizeof(TanV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForTanV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(TanV2).Tiling(TanV2TilingFunc).TilingParse<TanV2CompileInfo>(TilingParseForTanV2);
} // namespace optiling