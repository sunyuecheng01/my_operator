...
int main()
{
    ...
    // 4. 调用aclnnTanV2第一段接口
    ret = aclnnTanV2GetWorkspaceSize(selfX, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTanV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 5. 调用aclnnTanV2第二段接口
    ret = aclnnTanV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnTanV2 failed. ERROR: %d\n", ret); return ret);
