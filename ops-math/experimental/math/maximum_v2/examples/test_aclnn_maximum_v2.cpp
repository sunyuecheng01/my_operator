...
#include "aclnn_maximum_v2.h"

using DataType = u_int16_t;
...
int main()
{
    ...
    std::vector<int64_t> selfXShape = {24, 1, 1};
    std::vector<DataType> selfXHostData(24, 7);
    ...
    std::vector<int64_t> selfYShape = {24, 1, 1};
    std::vector<DataType> selfYHostData(24, 1);
    ret = CreateAclTensor(selfYHostData, selfYShape, &selfYDeviceAddr, aclDataType::ACL_INT16, &selfY);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    aclTensor* out = nullptr;
    void* outDeviceAddr = nullptr;
    std::vector<int64_t> outShape = {24, 1, 1};
    ...
    ret = aclnnMaximumV2GetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    LOG_PRINT("aclnnMaximumV2GetWorkspaceSize returned %d, workspaceSize=%llu, executor=%p\n",
          ret, (unsigned long long)workspaceSize, (void*)executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMaximumV2ExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    ret = aclnnMaximumV2(workspaceAddr, workspaceSize, executor, stream);
