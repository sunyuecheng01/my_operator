...
#include "../op_kernel/invert_v2_tiling_data.h"
#include "../op_kernel/invert_v2_tiling_key.h"
...
// tiling 分发入口
// 可直接替换你的 InvertV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus InvertV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        InvertV2TilingData* tiling = context->GetTilingData<InvertV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(InvertV2TilingData), 0, sizeof(InvertV2TilingData));
    ...
    InvertV2TilingData* tiling = context->GetTilingData<InvertV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(InvertV2TilingData), 0, sizeof(InvertV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForInvertV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(InvertV2).Tiling(InvertV2TilingFunc).TilingParse<InvertV2CompileInfo>(TilingParseForInvertV2);
} // namespace optiling
