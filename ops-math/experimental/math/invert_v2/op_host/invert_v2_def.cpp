...
namespace ops {
class InvertV2 : public OpDef {
public:
    explicit InvertV2(const char* name) : OpDef(name)
    {
        this->Input("x")                                       // 输入x定义
            .ParamType(REQUIRED)                                // 必选输入
            .DataType({ ge::DT_INT16,ge::DT_UINT16})             // 支持数据类型
        ...
        aicoreConfig.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "invert_v2");    // 这里制定的值会对应到kernel入口文件名.cpp
        this->AICore().AddConfig("ascend910b", aicoreConfig); // 其他的soc版本补充部分配置项
    }
};
OP_ADD(InvertV2); // 添加算子信息库
} // namespace ops