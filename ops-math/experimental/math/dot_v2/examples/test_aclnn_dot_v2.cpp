...
int main()
{
    ...
    std::vector<int64_t> selfXShape = {9, 101};
    std::vector<float> selfXHostData(909, 2);
    ...
    std::vector<int64_t> selfYShape = {9, 101};
    std::vector<float> selfYHostData(909, 3);
    ...
    std::vector<float> outHostData(1, 0);
    ...
    // 5. 调用aclnnDotV2第二段接口
    ret = aclnnDotV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnDotV2 failed. ERROR: %d\n", ret); return ret);
