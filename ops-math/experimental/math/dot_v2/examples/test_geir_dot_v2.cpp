...
#include "nn_other.h"
#include "../op_graph/dot_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape,value)                                         \
    ...
    if (intputDtype == DT_FLOAT || intputDtype == DT_FLOAT16) {                                                 \
    ...
    } else if(intputDtype == DT_INT32 || intputDtype == DT_INT16){                                                                       \
        ret = GenOnesData(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex, \
                         placeholder##intputIndex##_desc, intputDtype, static_cast<int>(value)); \
    }                                                                                                \
    if (ret != SUCCESS) {                                                                                   \
    ...
    dot1.set_input_##intputName(placeholder##intputIndex);                                                  \
    inputs.push_back(placeholder##intputIndex);

#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape,value)                                   \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                                          \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    dot1.set_input_##intputName(placeholder##intputIndex);                                                  \
    dot1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                   \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    dot1.update_output_desc_##outputName(outputName##outputIndex##_desc);
    //新加
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto dot1 = op::DotV2("dot1");
    std::vector<int64_t> xShape = {8, 3};
    ADD_INPUT(1, x1, inDtype, xShape,1.0);
    ADD_INPUT(2, x2, inDtype, xShape,1.0);
    //std::vector<int64_t> yShape = {1};
    ADD_OUTPUT(1, y, inDtype, xShape);

    outputs.push_back(dot1);
    // 添加完毕
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    if (ret != SUCCESS) {
        printf("%s - INFO - [XIR]: Run graph failed\n", GetTime().c_str());
         // 获取详细错误信息
         // 方法1: 直接使用AscendString的输出
        ge::AscendString error_msg = ge::GEGetErrorMsgV2();
        if (error_msg.GetString() != nullptr) {
            printf("Detailed error message: %s\n", error_msg.GetString());
        } else {
            printf("No detailed error message available\n");
        }
        delete session;
        GEFinalize();
        return FAILED;
