...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    if (supportedDtype.count(dataType) == 0) {
        OP_LOGE(context, "invalid dtype: %d", dataType);
...
// --- safer numeric types ---
    ...
    // 每个 tile 包含的元素数（至少 1）
    uint32_t tileDataNum = static_cast<uint32_t>((static_cast<uint64_t>(tileBlockNum) * blockSize) / inputBytes);
    if (tileDataNum == 0U) {
        tileDataNum = 1U;
    }
...
static ge::graphStatus CoshTilingFunc(gert::TilingContext* context)
{
    ...
    if (totalIdx <= 0) {
        ...
        memset_s(tiling, sizeof(CoshTilingData), 0, sizeof(CoshTilingData));
