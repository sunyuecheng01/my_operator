...
int main()
{
    ...
    uint64_t XHOSTDATA_X = 2048;
    uint64_t XHOSTDATA_Y = 1;
    uint64_t HOSTDATA_X = 2048;
    uint64_t HOSTDATA_Y = 1;

    std::vector<float> selfXHostData(XHOSTDATA_X, XHOSTDATA_Y);
    ...
    std::vector<float> outHostData(HOSTDATA_X, HOSTDATA_Y);
    ...
    // 4. 调用aclnnAtan第一段接口
    ret = aclnnAtanGetWorkspaceSize(selfX, out, &workspaceSize, &executor);
