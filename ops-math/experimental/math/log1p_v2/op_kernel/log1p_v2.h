...
#include "log1p_v2_tiling_data.h"
#include "log1p_v2_tiling_key.h"
...
class Log1pV2 {
public:
    __aicore__ inline Log1pV2(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR z, const Log1pV2TilingData* tilingData);
...
template <typename T>
__aicore__ inline void Log1pV2<T>::CopyOut(int32_t progress)
{
...
template <typename T>
__aicore__ inline void Log1pV2<T>::Compute(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.DeQue<T>();
    AscendC::LocalTensor<T> zLocal = outputQueueZ.AllocTensor<T>();
    float tmp = 1.0;
    AscendC::Adds(xLocal, xLocal, static_cast<T>(tmp), this->processDataNum);
...
template <typename T>
__aicore__ inline void Log1pV2<T>::Process()
{
