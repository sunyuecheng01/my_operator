...
#include "../op_kernel/log1p_v2_tiling_data.h"
#include "../op_kernel/log1p_v2_tiling_key.h"
...
// tiling 分发入口
static ge::graphStatus Log1pV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        Log1pV2TilingData* tiling = context->GetTilingData<Log1pV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(Log1pV2TilingData), 0, sizeof(Log1pV2TilingData));
    ...
    Log1pV2TilingData* tiling = context->GetTilingData<Log1pV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(Log1pV2TilingData), 0, sizeof(Log1pV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForLog1pV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(Log1pV2).Tiling(Log1pV2TilingFunc).TilingParse<Log1pV2CompileInfo>(TilingParseForLog1pV2);
} // namespace optiling
