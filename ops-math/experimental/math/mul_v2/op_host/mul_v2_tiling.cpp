...
#include "../op_kernel/mul_v2_tiling_data.h"
#include "../op_kernel/mul_v2_tiling_key.h"
...
const uint32_t UBPART = 3;
const uint32_t UBPART_INTE = 5;
...
// tiling 分发入口
// 可直接替换你的MulV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus MulV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        MulV2TilingData* tiling = context->GetTilingData<MulV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(MulV2TilingData), 0, sizeof(MulV2TilingData));
    ...
    MulV2TilingData* tiling = context->GetTilingData<MulV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(MulV2TilingData), 0, sizeof(MulV2TilingData)) != EOK,OP_LOGE(context, "set tiling data error"), return ge::GRAPH_FAILED);
    ...
    uint32_t ubDataNumber = (inputBytes == 1ULL) ? UBPART_INTE : UBPART;
...
// tiling注册入口.
IMPL_OP_OPTILING(MulV2).Tiling(MulV2TilingFunc);
} // namespace optiling