...
namespace ops {
class MulV2 : public OpDef {
public:
    explicit MulV2(const char* name) : OpDef(name)
    {
        this->Input("x1")                                       // 输入x1定义
            .ParamType(REQUIRED)                                // 必选输入
            .DataType({ge::DT_FLOAT, ge::DT_INT32, ge::DT_INT16, ge::DT_FLOAT16})             // 支持数据类型
        ...
        aicoreConfig.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "mul_v2");    // 这里制定的值会对应到kernel入口文件名.cpp
        this->AICore().AddConfig("ascend910b", aicoreConfig); // 其他的soc版本补充部分配置项
    }
};
OP_ADD(MulV2); // 添加算子信息库
} // namespace ops