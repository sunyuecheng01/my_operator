...
namespace optiling {
    ...
    static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t ubSize, ReduceSumV2ShapeInfo& info)
    {
        OP_CHECK_IF(
    ...
    static ge::graphStatus CalculateCoreBlockNums(int64_t coreNum, ReduceSumV2ShapeInfo& info)
    {
    ...
    // tiling 分发入口
    static ge::graphStatus ReduceSumV2TilingFunc(gert::TilingContext* context)
    {
        // ReduceSumV2TilingData tiling;
        ReduceSumV2TilingData* tiling = context->GetTilingData<ReduceSumV2TilingData>();
        ReduceSumV2ShapeInfo shapeInfo;
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        OP_CHECK_IF(
            memset_s(tiling, sizeof(ReduceSumV2TilingData), 0, sizeof(ReduceSumV2TilingData)) != EOK,
    ...
    static ge::graphStatus TilingParseForReduceSumV2([[maybe_unused]] gert::TilingParseContext* context)
    {   
        return ge::GRAPH_SUCCESS;
    }

    // tiling注册入口.
    IMPL_OP_OPTILING(ReduceSumV2).Tiling(ReduceSumV2TilingFunc).TilingParse<ReduceSumV2CompileInfo>(TilingParseForReduceSumV2);
} // namespace optiling