...
#include "nn_other.h"
#include "../op_graph/ger_v2_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::GerV2("add1");
    add1.set_attr_alpha(0.2f);
    std::vector<int64_t> xShape = {8};
    std::vector<int64_t> yShape = {3};
    std::vector<int64_t> AShape = {8,3};
    std::vector<int64_t> zShape = {8,3};
    ADD_INPUT(1, x, inDtype, xShape,7);
    ADD_INPUT(2, y, inDtype, yShape,2);
    ADD_INPUT(3, A, inDtype, AShape,1);

    ADD_OUTPUT(1, z, inDtype, zShape);
