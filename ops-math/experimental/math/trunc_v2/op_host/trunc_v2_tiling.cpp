...
#include "../op_kernel/trunc_v2_tiling_data.h"
#include "../op_kernel/trunc_v2_tiling_key.h"
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    OP_CHECK_IF(
        !shapeMatch,
        OP_LOGE(
            context, "TruncV2: inputx,outputz shape not match! dim num: x=%zu,  z=%zu",
...
// tiling 分发入口
// 可直接替换你的 TruncV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus TruncV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        TruncV2TilingData* tiling = context->GetTilingData<TruncV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(TruncV2TilingData), 0, sizeof(TruncV2TilingData));
    ...
    TruncV2TilingData* tiling = context->GetTilingData<TruncV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(TruncV2TilingData), 0, sizeof(TruncV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForTruncV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(TruncV2).Tiling(TruncV2TilingFunc).TilingParse<TruncV2CompileInfo>(TilingParseForTruncV2);
} // namespace optiling
