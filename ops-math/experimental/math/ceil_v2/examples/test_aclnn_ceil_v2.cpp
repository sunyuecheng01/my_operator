...
int main()
{
    ...
    std::vector<float> selfXHostData(16, 5.0f);
    ...
    // 4. 调用aclnnCeilV2第一段接口
    ret = aclnnCeilV2GetWorkspaceSize(selfX, out, &workspaceSize, &executor);
    // CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCeilV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    if (ret != ACL_SUCCESS) {
    ...
    // 5. 调用aclnnCeilV2第二段接口
    ret = aclnnCeilV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCeilV2 failed. ERROR: %d\n", ret); return ret);
