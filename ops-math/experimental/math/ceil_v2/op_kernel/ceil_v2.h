...
#include "ceil_v2_tiling_data.h"
#include "ceil_v2_tiling_key.h"
...
class CeilV2 {
public:
    __aicore__ inline CeilV2(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR z, const CeilV2TilingData* tilingData);
...
template <typename T>
__aicore__ inline void CeilV2<T>::Init(GM_ADDR x, GM_ADDR z, const CeilV2TilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void CeilV2<T>::CopyIn(int32_t progress)
{
...
template <typename T>
__aicore__ inline void CeilV2<T>::CopyOut(int32_t progress)
{
...
template <typename T>
__aicore__ inline void CeilV2<T>::Compute(int32_t progress)
{
    AscendC::LocalTensor<T> xLocal = inputQueueX.DeQue<T>();
    AscendC::LocalTensor<T> zLocal = outputQueueZ.AllocTensor<T>();
    AscendC::Ceil(zLocal, xLocal, this->processDataNum);
    outputQueueZ.EnQue<T>(zLocal);
    inputQueueX.FreeTensor(xLocal);
}

template <typename T>
__aicore__ inline void CeilV2<T>::Process()
{
