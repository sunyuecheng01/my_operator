 ...
 */
ACLNN_API aclnnStatus aclnnLog2GetWorkspaceSize(
 ...
 * ```
 *
 * @param [in] selfRef(aclTensor*): 数据类型支持FLOAT、FLOAT16、BFLOAT16，数据格式支持ND，selfRef也是输出结果。
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus
aclnnInplaceLog2GetWorkspaceSize(const aclTensor* selfRef, uint64_t* workspaceSize, aclOpExecutor** executor);
 ...
 */
ACLNN_API aclnnStatus
aclnnInplaceLog2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream);
