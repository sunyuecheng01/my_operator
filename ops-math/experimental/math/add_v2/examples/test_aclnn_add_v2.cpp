 ...
 */
 ...
 int main()
 {
     ...
     std::vector<int64_t> selfXShape = {3,4};
     std::vector<DataType> selfXHostData(12, 7);
     ...
     std::vector<int64_t> selfYShape = {3,4};
     ...
     std::vector<DataType> outHostData(12, 0);
     ...
     ret = aclnnAddV2GetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
     LOG_PRINT("aclnnAddV2GetWorkspaceSize returned %d, workspaceSize=%llu, executor=%p\n",
           ret, (unsigned long long)workspaceSize, (void*)executor);
     CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddV2ExampleGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
     ...
     ret = aclnnAddV2(workspaceAddr, workspaceSize, executor, stream);
