...
#include "../op_kernel/add_v2_tiling_data.h"
#include "../op_kernel/add_v2_tiling_key.h"
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
{
    ...
    OP_CHECK_IF(
        !shapeMatch,
        OP_LOGE(
            context, "AddV2: inputx,inputy,outputz shape not match! dim num: x=%zu, y=%zu, z=%zu",
...
// tiling 分发入口
// 可直接替换你的 AddV2TilingFunc 内部实现（保留函数签名）
static ge::graphStatus AddV2TilingFunc(gert::TilingContext* context)
{
    // 1. platform
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    // handle empty input
    if (totalIdx <= 0) {
        AddV2TilingData* tiling = context->GetTilingData<AddV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        memset_s(tiling, sizeof(AddV2TilingData), 0, sizeof(AddV2TilingData));
    ...
    AddV2TilingData* tiling = context->GetTilingData<AddV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(AddV2TilingData), 0, sizeof(AddV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForAddV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(AddV2).Tiling(AddV2TilingFunc).TilingParse<AddV2CompileInfo>(TilingParseForAddV2);
} // namespace optiling
