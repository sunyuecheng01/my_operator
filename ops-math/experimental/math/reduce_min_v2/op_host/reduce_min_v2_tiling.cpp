...
#include "../op_kernel/reduce_min_v2_tiling_data.h"
#include "../op_kernel/reduce_min_v2_tiling_key.h"

namespace optiling {
    const uint32_t BLOCK_SIZE = 32;
    const uint32_t BUFFER_NUM = 2;
    const uint32_t WS_SYS_SIZE =  16U * 1024U* 1024U; // 16MB
    const uint64_t UB_DATA_NUMBER_32 = 6;
    struct ReduceMinV2CompileInfo {};
    struct ReduceMinV2ShapeInfo {
        uint64_t inputNum{0};
    ...
    static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t ubSize, ReduceMinV2ShapeInfo& info)
    {
        OP_CHECK_IF(
    ...
    static ge::graphStatus CalculateCoreBlockNums(int64_t coreNum, ReduceMinV2ShapeInfo& info)
    {
    ...
    // tiling 分发入口
    static ge::graphStatus ReduceMinV2TilingFunc(gert::TilingContext* context)
    {
        // ReduceMinV2TilingData tiling;
        ReduceMinV2TilingData* tiling = context->GetTilingData<ReduceMinV2TilingData>();
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        OP_CHECK_IF(
            memset_s(tiling, sizeof(ReduceMinV2TilingData), 0, sizeof(ReduceMinV2TilingData)) != EOK,
        ...
        //获取输入数据信息
        ReduceMinV2ShapeInfo shapeInfo;
    ...
    static ge::graphStatus TilingParseForReduceMinV2([[maybe_unused]] gert::TilingParseContext* context)
    {   
        return ge::GRAPH_SUCCESS;
    }

    // tiling注册入口.
    IMPL_OP_OPTILING(ReduceMinV2).Tiling(ReduceMinV2TilingFunc).TilingParse<ReduceMinV2CompileInfo>(TilingParseForReduceMinV2);
} // namespace optiling