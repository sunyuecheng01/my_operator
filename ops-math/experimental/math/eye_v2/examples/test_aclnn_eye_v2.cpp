...
int main()
{
    ...
    LOG_PRINT("Before GetWorkspaceSize: out=%p\n", (void*)out);
    LOG_PRINT("Before GetWorkspaceSize: outDeviceAddr=%p\n", outDeviceAddr);
    // 4. 调用aclnnEyeV2第一段接口
    int32_t rows = 4;
    int32_t cols = 4;
    ...
    // 5. 调用aclnnEyeV2第二段接口
    ret = aclnnEyeV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnEyeV2 failed. ERROR: %d\n", ret); return ret);
    ...
    std::vector<int64_t> outShape1 = {16};
