...
__global__ __aicore__ void triu(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    REGISTER_TILING_DEFAULT(TriuTilingData);
    GET_TILING_DATA_WITH_STRUCT(TriuTilingData, tilingData, tiling);
    if constexpr (schMode == static_cast<uint32_t>(AddExampleTilingKey::TILING_KEY_NaivePath)) {
        NsTriu::Triu<DTYPE_X> op; // 算子kernel实例获取
