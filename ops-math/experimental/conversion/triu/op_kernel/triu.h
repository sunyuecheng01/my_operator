...
#include "triu_tiling_data.h"
#include "triu_tiling_key.h"
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, const TriuTilingData* tilingData, uint32_t key);
...
template <typename T>
__aicore__ inline void Triu<T>::Init(GM_ADDR x, GM_ADDR y, const TriuTilingData* tilingData, uint32_t key)
{
...
template <typename T>
__aicore__ inline void Triu<T>::FastPath()
{
    uint32_t GmOffset = 0;
    int32_t init_row = 0;
...
template <typename T>
__aicore__ inline void Triu<T>::AllZero(uint32_t tileLength)
{
    auto yLocal = outQueueY.AllocTensor<DTYPE_X>();
    auto zero = static_cast<DTYPE_X>(0);
    AscendC::Duplicate(yLocal, zero, tileLength);
    outQueueY.EnQue(yLocal);
}

template <typename T>
__aicore__ inline void Triu<T>::CopyIn(uint32_t GmOffset, uint32_t tileLength)
{
...
template <typename T>
__aicore__ inline void Triu<T>::CopyOut(uint32_t GmOffset, uint32_t tileLength)
{
...
template <typename T>
__aicore__ inline void Triu<T>::Compute(int32_t cnt, uint32_t initLength, int32_t adjust)
{
