...
/*!
 * \file pow_u8_nddma_without_loops.h
 * \brief
 */
#ifndef ASCENDC_POW_U8_NDDMA_WITHOUT_LOOPS_H_
#define ASCENDC_POW_U8_NDDMA_WITHOUT_LOOPS_H_
...
class PowU8NddmaWithoutLoops
{
public:
    __aicore__ inline PowU8NddmaWithoutLoops(){};
                                ...
                                const PowTensorTensorTilingData* tilingDataPtr, TPipe* pipePtr)
    {
        ...
        inputGmInputX_.SetGlobalBuffer((__gm__ uint8_t*)inputX);
        inputGmInputY_.SetGlobalBuffer((__gm__ uint8_t*)inputY);
        outputGmOutputZ_.SetGlobalBuffer((__gm__ uint8_t*)outputZ);
        constexpr int64_t DOUBLE_BUFFER = 2;
        int64_t BUFFER_SIZE_0 = tilingDataPtr_->elemNum * sizeof(uint8_t);
...
private:
    __aicore__ inline void CopyIn0(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                   int64_t ubLoopIdx)
    {
        bufferIn0_ = queIn0_.AllocTensor<uint8_t>();
                                   ...
                                   int64_t ubLoopIdx)
    {
        ...
        queIn1_.EnQue<uint8_t>(bufferIn1_);
    }

    __aicore__ inline void Compute2(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        bufferIn0_ = queIn0_.DeQue<uint8_t>();
        bufferIn1_ = queIn1_.DeQue<uint8_t>();
        bufferOut0_ = queOut0_.AllocTensor<uint8_t>();
        Power<uint8_t, false, pConfig_>(bufferOut0_, bufferIn0_, bufferIn1_, 
        ...
        queOut0_.EnQue<uint8_t>(bufferOut0_);
    }

    __aicore__ inline void CopyOut3(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        bufferOut0_ = queOut0_.DeQue<uint8_t>();
        AscendC::DataCopyExtParams dataCopyExtParams;
        dataCopyExtParams.blockCount = 1;
        dataCopyExtParams.blockLen =
            ubSplitSize * tilingDataPtr_->outputStrides[tilingDataPtr_->ubSplitAxis] * sizeof(uint8_t);
...
private:
    ...
    GlobalTensor<uint8_t> inputGmInputX_;
    GlobalTensor<uint8_t> inputGmInputY_;
    GlobalTensor<uint8_t> outputGmOutputZ_;
    ...
    LocalTensor<uint8_t> bufferIn0_;
    LocalTensor<uint8_t> bufferIn1_;
    LocalTensor<uint8_t> bufferOut0_;
    constexpr static PowerConfig pConfig_ = {PowerAlgo::INTRINSIC};
};

}  // namespace Pow
#endif  // ASCENDC_POW_U8_NDDMA_WITHOUT_LOOPS_H_
