...
/*!
 * \file pow_s8_nddma_without_loops.h
 * \brief
 */
#ifndef ASCENDC_POW_S8_NDDMA_WITHOUT_LOOPS_H_
#define ASCENDC_POW_S8_NDDMA_WITHOUT_LOOPS_H_
...
class PowS8NddmaWithoutLoops
{
public:
    __aicore__ inline PowS8NddmaWithoutLoops(){};
                                ...
                                const PowTensorTensorTilingData* tilingDataPtr, TPipe* pipePtr)
    {
        ...
        inputGmInputX_.SetGlobalBuffer((__gm__ int8_t*)inputX);
        inputGmInputY_.SetGlobalBuffer((__gm__ int8_t*)inputY);
        outputGmOutputZ_.SetGlobalBuffer((__gm__ int8_t*)outputZ);
        constexpr int64_t DOUBLE_BUFFER = 2;
        int64_t BUFFER_SIZE_0 = tilingDataPtr_->elemNum * sizeof(int8_t);
...
private:
    __aicore__ inline void CopyIn0(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                   int64_t ubLoopIdx)
    {
        bufferIn0_ = queIn0_.AllocTensor<int8_t>();
                                   ...
                                   int64_t ubLoopIdx)
    {
        ...
        queIn1_.EnQue<int8_t>(bufferIn1_);
    }

    __aicore__ inline void Compute2(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        bufferIn0_ = queIn0_.DeQue<int8_t>();
        bufferIn1_ = queIn1_.DeQue<int8_t>();
        bufferOut0_ = queOut0_.AllocTensor<int8_t>();
        Power<int8_t, false, pConfig_>(bufferOut0_, bufferIn0_, bufferIn1_, 
        ...
        queOut0_.EnQue<int8_t>(bufferOut0_);
    }

    __aicore__ inline void CopyOut3(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        bufferOut0_ = queOut0_.DeQue<int8_t>();
        AscendC::DataCopyExtParams dataCopyExtParams;
        dataCopyExtParams.blockCount = 1;
        dataCopyExtParams.blockLen =
            ubSplitSize * tilingDataPtr_->outputStrides[tilingDataPtr_->ubSplitAxis] * sizeof(int8_t);
...
private:
    ...
    GlobalTensor<int8_t> inputGmInputX_;
    GlobalTensor<int8_t> inputGmInputY_;
    GlobalTensor<int8_t> outputGmOutputZ_;
    ...
    LocalTensor<int8_t> bufferIn0_;
    LocalTensor<int8_t> bufferIn1_;
    LocalTensor<int8_t> bufferOut0_;
    constexpr static PowerConfig pConfig_ = {PowerAlgo::INTRINSIC};
};

}  // namespace Pow
#endif  // ASCENDC_POW_S8_NDDMA_WITHOUT_LOOPS_H_
