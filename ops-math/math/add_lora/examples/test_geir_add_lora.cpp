...
#include "nn_other.h"
#include "../op_graph/add_lora_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::AddLora("add1");
    std::vector<int64_t> xShape = {1, 16};
    std::vector<int64_t> yShape = {1, 16};
    std::vector<int64_t> weightBShape = {1, 1, 16, 16};
    std::vector<int64_t> indicesShape = {1};
    std::vector<int64_t> weightAShape = {1, 1, 16, 16};
    ADD_INPUT(1, y, inDtype, yShape);
    ADD_INPUT(2, x, inDtype, xShape);
    ADD_INPUT(3, weightB, inDtype, weightBShape);
    ADD_INPUT(4, indices, DT_INT32, indicesShape);
    ADD_INPUT(5, weightA, inDtype, weightAShape);
    
    ADD_INPUT_ATTR(layer_idx, 0);
    ADD_INPUT_ATTR(scale, 1e-3);
    ADD_INPUT_ATTR(y_offset, 0);
    ADD_INPUT_ATTR(y_slice_size, -1);

    ADD_OUTPUT(1, y_out, inDtype, yShape);
