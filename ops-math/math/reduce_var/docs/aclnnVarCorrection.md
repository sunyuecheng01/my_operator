...
- **参数说明：**
  ...
  - self（aclTensor*, 计算输入）：公式中的输入`self`，shape支持0到8维，self与out的数据类型满足数据类型推导规则（参见[互推导关系](../../../docs/zh/context/互推导关系.md)），支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品/Ascend 950PR/Ascend 950DT</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT。
  ...
  - correction（int64_t，入参）：公式中的输入$\delta N$，修正值，数据类型为int64_t。
  - keepdim（bool，入参）：reduce轴的维度是否保留。数据类型为bool。
  - out（aclTensor*, 计算输出）：公式中的输入`out`，self与out的shape满足计算公式中的推导规则，self与out的数据类型满足数据类型推导规则（参见[互推导关系](../../../docs/zh/context/互推导关系.md)），支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT。
...
- **参数说明：**
  ...
  - workspaceSize（uint64_t, 入参）：在Device侧申请的workspace大小，由第一段接口aclnnVarCorrectionGetWorkspaceSize获取。
