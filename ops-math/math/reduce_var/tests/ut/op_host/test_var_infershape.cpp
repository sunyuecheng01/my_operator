    ...
    ge::graphStatus testCaseResult = ge::GRAPH_SUCCESS)
{
    ...
    auto inferShapeFunc = spaceRegistry->GetOpImpl("ReduceVar")->infer_shape;
...
TEST_F(ReduceVar, ReduceVar_infershape_case_0)
{
    // 用vector存储同类型参数（顺序与原参数列表一致）
    std::vector<gert::StorageShape> inputShapes = {{{25, 3}, {25, 3}}};

    std::vector<ge::DataType> dtypes = {ge::DT_FLOAT, ge::DT_FLOAT};

    std::vector<std::vector<int64_t>> expectResult = {{25, 1}, {25, 1}};
