...
/*!
 * \file reduce_var_infershape.cpp
 * \brief
 */
...
static ge::graphStatus InferShape4ReduceVar(gert::InferShapeContext* context) {
    ...
    int64_t inputDimNum = inShape->GetDimNum();
    ...
    if (keepDims) {
        inferStat = ReduceDimsWithKeepDims<int64_t>(inShape, &axes[0], static_cast<int32_t>(axes.size()), varShape);
    } else {
        inferStat = ReduceDimsWithoutKeepDims<int64_t>(inShape, &axes[0], static_cast<int32_t>(axes.size()), varShape);
    }
    ...
    if (inferStat == ge::GRAPH_SUCCESS) {
        ...
        *meanShape = *varShape;
        OP_LOGD(
            context->GetNodeName(), "inShape:%s reduce axes:%s keepDims:%d, get infer varShape:%s meanShape:%s.",
            Ops::Base::ToString(*inShape).c_str(), ss.str().c_str(), keepDims, Ops::Base::ToString(*varShape).c_str(),
...
IMPL_OP_INFERSHAPE(ReduceVar).InferShape(InferShape4ReduceVar);
}  // namespace ops
