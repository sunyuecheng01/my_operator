...
#include "nn_other.h"
#include "../op_graph/axpy_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    do {                                                                                                            \
        ...
        axpy1.set_input_##intputName(placeholder##intputIndex);                                              \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    ...
    axpy1.update_output_desc_##outputName(outputName##outputIndex##_desc);                 
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto axpy1 = op::AxpyV2("axpy1");
    std::vector<int64_t> x1Shape = {1, 1, 1, 1};
    std::vector<int64_t> x2Shape = {1, 1, 1, 1};
    std::vector<int64_t> alphaShape = {1, 1, 1, 1};
    std::vector<int64_t> yShape = {1, 1, 1, 1};

    ADD_INPUT(1, x1, inDtype, x1Shape);
    ADD_INPUT(2, x2, inDtype, x2Shape);
    ADD_INPUT(3, alpha, inDtype, alphaShape);
    ADD_OUTPUT(1, y, inDtype, yShape);

    outputs.push_back(axpy1);
    // 添加完毕
    return SUCCESS;
}
