...
#include "nn_other.h"
#include "../op_graph/cumsum_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::Cumsum("cumsum");
    vector<vector<int64_t>> shapes = {{2, 2, 2}, {2, 2}, {2, 2, 2}};

    ADD_INPUT(1, x, inDtype, shapes[0]);
    ADD_INPUT(2, axis, inDtype, shapes[1]);
    ADD_OUTPUT(3, y, inDtype, shapes[2]);
    ADD_INPUT_ATTR(exclusive, false);
    ADD_INPUT_ATTR(reverse, false);
