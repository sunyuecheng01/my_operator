...
#include "nn_other.h"
#include "../op_graph/search_sorted_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                     \
    ...
    search_sorted1.set_input_##inputName(placeholder##inputIndex);                                                   \
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    search_sorted1.set_input_##inputName(placeholder##inputIndex);                                                   \
    search_sorted1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                    \
    inputs.push_back(placeholder##inputIndex);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    ...
    auto search_sorted1 = op::SearchSorted("search_sorted");
    std::vector<int64_t> sortedSeqShape = {4, 4};
    std::vector<int64_t> valuesShape = {4, 2};

    ADD_INPUT(1, sorted_sequence, DT_INT32, sortedSeqShape);
    ADD_INPUT(2, values, DT_INT32, valuesShape);

    outputs.push_back(search_sorted1);
    // 添加完毕
    return SUCCESS;
}
