 ...
 * @param [in] self: npu device侧的aclTensor，数据类型支持FLOAT16、FLOAT32、INT32、INT64、INT16、INT8、UINT8。支持非连续的Tensor，数据格式支持ND
 * @param [in] bins: 直方图bins的数量，数据类型INT64
 * @param [in] min: 直方图统计下限（包括），host侧的aclScalar，数据类型需要可转换成FLOAT的数据类型。
 * @param [in] max: 直方图统计上限（包括），host侧的aclScalar，数据类型需要可转换成FLOAT的数据类型。
 * @param [in] out: npu device侧的aclTensor，数据类型支持FLOAT16、FLOAT32、INT32、INT64、INT16、INT8、UINT8。且数据类型是self可转化的数据类型。数据格式支持ND
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnHistcGetWorkspaceSize(
