...
#include "nn_other.h"
#include "../op_graph/non_finite_check_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::NonFiniteCheck("add1").create_dynamic_input_tensor_list(1);
    std::vector<int64_t> xShape = {4, 4};
    ADD_DYNAMIC_INPUT(0, tensor_list, inDtype, xShape, 0);
    ADD_OUTPUT(1, found_flag, DT_FLOAT, xShape);
