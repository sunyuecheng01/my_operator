...
#include "array_ops.h"
#include "nn_other.h"
#include "../op_graph/sort_with_index_proto.h"
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    ...
    sortWithIndex.update_output_desc_##outputName(outputName##outputIndex##_desc)
    ...
    Graph& graph)
{
    ...
    std::vector<int64_t> xShape = {2, 2048};
    ADD_INPUT(1, x, dataType, xShape);
    ADD_INPUT(2, index, indexType, xShape);
...
int InitializeAndSetupGraph(Graph& graph, std::vector<ge::Tensor>& input, DataType dataType,  DataType indexType)
{
    ...
    ret = CreateOppInGraph(dataType, indexType, input, inputs, outputs, graph);
    if (ret != SUCCESS) {
...
{
    // 1、创建图对象
    const char* graph_name = "tc_ge_irrun_sort_with_index_test";
    ...
    // 初始化和设置图
    if (InitializeAndSetupGraph(graph, input, dataType, indexType) != SUCCESS) {
        return FAILED;
    }
