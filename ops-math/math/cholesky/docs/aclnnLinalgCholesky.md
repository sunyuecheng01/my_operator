...
```Cpp
aclnnStatus aclnnLinalgCholesky(
  void               *workspace, 
  uint64_t           workspaceSize, 
  aclOpExecutor      *executor, 
...
## aclnnLinalgCholeskyGetWorkspaceSize
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_linalg_cholesky.h"
...
int main() {
  ...
  std::vector<float> selfHostData = {6, -3, 1, -3, 2, 0, 1, 0, 4};
  ...
  aclOpExecutor* executor;
  // 调用aclnnLinalgCholesky第一段接口
  ret = aclnnLinalgCholeskyGetWorkspaceSize(self, false, out, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLinalgCholeskyGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnLinalgCholesky第二段接口
  ret = aclnnLinalgCholesky(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLinalgCholesky failed. ERROR: %d\n", ret); return ret);
