...
#include "nn_other.h"
#include "../op_graph/squared_difference_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                         \
    ...
    squaredDifference.set_input_##intputName(placeholder##intputIndex);                                     \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    squaredDifference.set_input_##intputName(placeholder##intputIndex);                                     \
    squaredDifference.update_input_desc_##intputName(placeholder##intputIndex##_desc);                      \
    inputs.push_back(placeholder##intputIndex)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    squaredDifference.update_output_desc_##outputName(outputName##outputIndex##_desc)
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto squaredDifference = op::SquaredDifference("squaredDifference");
    ...
    outputs.push_back(squaredDifference);
    // 添加完毕
    return SUCCESS;
}
...
{
    // 1、创建图对象
    const char* graph_name = "tc_ge_irrun_square_test";
