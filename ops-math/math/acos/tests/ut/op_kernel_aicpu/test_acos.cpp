...
const std::string ktestcaseFilePath =
    "../../../../math/acos/tests/ut/op_kernel_aicpu/";
...
inline void RunKernelAcos(std::shared_ptr<aicpu::NodeDef> node_def,
...
void CreateAndRunKernelAcosParamInvalid(
    const std::vector<std::int64_t> &dims_in,
    const std::vector<std::int64_t> &dims_out, Tin *input0, Tout *output) {
  CreateAndRunKernelAcos(dims_in, dims_out, input0, output,
                         aicpu::KERNEL_STATUS_PARAM_INVALID);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAcosParamInvalid(const std::vector<std::int64_t> &dims,
                                        Tin *input0, Tout *output) {
  CreateAndRunKernelAcosParamInvalid(dims, dims, input0, output);
}

template <typename Tin, typename Tout>
void CreateAndRunKernelAcosInnerError(const std::vector<std::int64_t> &dims_in,
...
template <typename Tin, typename Tout>
void CreateAndRunKernelAcosInnerError(const std::vector<std::int64_t> &dims,
                                      Tin *input0, Tout *output) {
...
void RunTestAcos() {
  ...
  std::uint64_t dim[1];
  std::string data_dim_path{ktestcaseFilePath + "acos/data/acos_data_" +
                            data_name + "_dim.txt"};
  EXPECT_EQ(ReadFile(data_dim_path, dim, 1), true);

  std::uint64_t shape[dim[0]];
  std::string data_shape_path{ktestcaseFilePath + "acos/data/acos_data_" +
  ...
  std::string data_path0{ktestcaseFilePath + "acos/data/acos_data_input_0_" +
                         data_name + ".bin"};
...
TEST_F(TEST_ACOS_UT, DATA_TYPE_DT_FLOAT16) { RunTestAcos<Eigen::half>(); }

TEST_F(TEST_ACOS_UT, DATA_TYPE_DT_FLOAT) { RunTestAcos<std::float_t>(); }

TEST_F(TEST_ACOS_UT, DATA_TYPE_DT_DOUBLE) { RunTestAcos<std::double_t>(); }
// exception instance
...
TEST_F(TEST_ACOS_UT, NO_OUTPUT_EXCEPTION) {
  ...
  RunKernelAcos(node_def, aicpu::DeviceType::HOST,
                aicpu::KERNEL_STATUS_PARAM_INVALID);
}
