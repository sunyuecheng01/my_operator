...
#include "nn_other.h"
#include "../op_graph/square_sum_v1_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto squareSumV1 = op::SquareSumV1("squareSumV1");
    std::vector<int64_t> xShape = {4, 8};
    std::vector<int64_t> yShape = {8};
    ADD_INPUT(1, x, inDtype, xShape);
    
    ADD_INPUT_ATTR(axis, {1});
    ADD_INPUT_ATTR(keep_dims, false);

    ADD_OUTPUT(1, y, inDtype, yShape);

    outputs.push_back(squareSumV1);
    // 添加完毕
    return SUCCESS;
}
