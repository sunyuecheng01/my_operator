...
// 检查输入是否是空指针
inline static bool CheckNotNull(const aclScalar *start, const aclScalar *end, const aclScalar *step, const aclTensor *out) {
...
inline static aclnnStatus CheckParamsLogic(const aclScalar *start, const aclScalar *end, const aclScalar *step,
  ...
  OP_CHECK(dim_num != 0,
           OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected out a 1d tensor, but got %zu.", dim_num),
           return ACLNN_ERR_PARAM_INVALID);
