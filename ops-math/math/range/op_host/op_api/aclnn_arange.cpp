...
static bool CheckDtypeValid(const aclScalar* start, const aclScalar* end, const aclScalar* step, const aclTensor* out) {
...
inline static aclnnStatus CheckStepCorrect(const aclScalar *start, const aclScalar *end,
                                           const aclScalar *step, const aclTensor *out)
{
...
// 检查输入是否是空指针
static bool CheckNotNull(const aclScalar* start, const aclScalar* end, const aclScalar* step, const aclTensor* out) {
  // 检查start是否是空指针
...
static aclnnStatus CheckParams(const aclScalar* start, const aclScalar* end, const aclScalar* step,
                               const aclTensor* out) {
  // 1. 检查参数是否为空指针
  ...
  // 3. 检查输入的数据的值是否合理
  CHECK_RET(CheckStepCorrect(start, end, step, out) == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID);
  return ACLNN_SUCCESS;
}

aclnnStatus aclnnArangeGetWorkspaceSize(const aclScalar* start, const aclScalar* end, const aclScalar* step,
                                        aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor) {
  L2_DFX_PHASE_1(aclnnArange, DFX_IN(start, end, step), DFX_OUT(out));
  ...
  // 执行L0算子
  static const bool isClosed = false;
  auto arangeOutRet = l0op::Arange(start, end, step, out, isClosed, uniqueExecutor.get());
  CHECK_RET(arangeOutRet != nullptr, ACLNN_ERR_INNER_NULLPTR);
  // 将结果转换成输出类型
  auto arangeOutCast = l0op::Cast(arangeOutRet, out->GetDataType(), uniqueExecutor.get());
  CHECK_RET(arangeOutCast != nullptr, ACLNN_ERR_INNER_NULLPTR);
  // 将计算结果拷贝到输出out上，out可能是非连续的tensor
  auto viewCopyResult = l0op::ViewCopy(arangeOutCast, out, uniqueExecutor.get());
