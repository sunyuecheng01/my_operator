    ...
    gert::StorageShape& outStorageShape, ge::graphStatus testCaseResult = ge::GRAPH_SUCCESS)
{
    ...
    auto inferShapeFunc = spaceRegistry->GetOpImpl("DiagPart")->infer_shape;
...
TEST_F(DiagPart, DiagPart_infershape_case_0)
{
    // 用vector存储同类型参数（顺序与原参数列表一致）
    std::vector<gert::StorageShape> inputShapes = {{{16, 16}, {16, 16}}};
    std::vector<ge::DataType> dtypes = {
        ge::DT_FLOAT, // input1Dtype
        ge::DT_FLOAT};

    std::vector<int64_t> expectResult = {16};
