    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::Log("log");    
    std::vector<std::vector<int64_t>> shapes = {{5, 4, 3}, {5, 4, 3}};

    ADD_INPUT(1, x, inDtype1, shapes[0]);
    ADD_OUTPUT(2, y, inDtype2, shapes[1]);
    ADD_INPUT_ATTR(base, -1.0);
    ADD_INPUT_ATTR(scale, 1.0);
    ADD_INPUT_ATTR(shift, 0.0);
