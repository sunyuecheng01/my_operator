...
aclnnStatus aclnnLgammaGetWorkspaceSize(const aclTensor *self, aclTensor *out,
                                     uint64_t *workspaceSize, aclOpExecutor **executor) {
  L2_DFX_PHASE_1(aclnnLgamma, DFX_IN(self), DFX_OUT(out));
  ...
  // 调用l0算子Lgamma进行计算
  auto lgammaResult = l0op::Lgamma(selfContiguous, uniqueExecutor.get());
  CHECK_RET(lgammaResult != nullptr, ACLNN_ERR_INNER_NULLPTR);

  // 如果出参out是非连续Tensor，需要把计算完的连续Tensor转非连续
  auto viewCopyResult = l0op::ViewCopy(lgammaResult, out, uniqueExecutor.get());
