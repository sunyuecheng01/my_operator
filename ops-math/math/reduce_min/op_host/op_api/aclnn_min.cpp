...
aclIntArray *GetMinDimListForTensor(const aclTensor *self, aclOpExecutor *executor) {
...
aclnnStatus aclnnMinGetWorkspaceSize(const aclTensor *self, aclTensor *out, uint64_t *workspaceSize,
                                     aclOpExecutor **executor) {
  L2_DFX_PHASE_1(aclnnMin, DFX_IN(self), DFX_OUT(out));
  ...
  auto dim = GetMinDimListForTensor(self, uniqueExecutor.get());
