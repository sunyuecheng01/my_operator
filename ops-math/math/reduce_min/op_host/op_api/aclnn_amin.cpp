...
static const std::initializer_list<op::DataType> ASCEND910B_DTYPE_DTYPE_SUPPORT_LIST = {
    op::DataType::DT_FLOAT, op::DataType::DT_FLOAT16, op::DataType::DT_UINT8, op::DataType::DT_INT32,   // AiCore
    op::DataType::DT_INT64, op::DataType::DT_INT16,   op::DataType::DT_INT8,  op::DataType::DT_DOUBLE,  // AiCpu
    op::DataType::DT_BOOL, op::DataType::DT_BF16};  // AiCore(cast to float)
...
static void AminInferShape(const op::Shape& selfShape, const aclIntArray* dim, bool keepDim, op::Shape& reduceShape) {
  bitset<MAX_MASK_LEN> dimMask = bitset<MAX_MASK_LEN>();
  if (dim->Size() == 0) {
    dimMask.flip();
  }
  for (size_t i = 0; i < dim->Size(); i++) {
    int64_t index = GetPosDim((*dim)[i], selfShape.GetDimNum());
    // 前序已检查， 此处如果dim不会重复
    dimMask.set(index);
  }
  ...
  for (size_t i = 0; i < dim->Size(); i++) {
    if ((*dim)[i] >= selfDimNum || (*dim)[i] < (-selfDimNum)) {
      OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Provided dim %ld must be in the range of [%ld, %ld].",
              (*dim)[i], -selfDimNum, selfDimNum - 1);
      return false;
    }
    uint64_t index = GetPosDim((*dim)[i], selfDimNum);
    // 非标量reduce的dim的size不能为0
...
static bool CheckShape(const aclTensor* self, const aclIntArray* dim, const bool keepDim, const aclTensor* out) {
  ...
  AminInferShape(self->GetViewShape(), dim, keepDim, reduceShape);
...
aclnnStatus aclnnAminGetWorkspaceSize(const aclTensor* self, const aclIntArray* dim, bool keepDim, aclTensor* out,
                                      uint64_t* workspaceSize, aclOpExecutor** executor) {
  L2_DFX_PHASE_1(aclnnAmin, DFX_IN(self, dim, keepDim), DFX_OUT(out));
  // 创建OpExecutor
  ...
  // 调用min算子kernel
  auto minResult = l0op::ReduceMin(selfCasted, dim, keepDim, uniqueExecutor.get());
  ...
  auto viewCopyResult = l0op::ViewCopy(castMinOut, out, uniqueExecutor.get());
  CHECK_RET(viewCopyResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
