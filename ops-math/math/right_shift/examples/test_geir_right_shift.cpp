...
#include "nn_other.h"
#include "../op_graph/right_shift_proto.h"
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                     \
    ...
    right_shift1.set_input_##inputName(placeholder##inputIndex);                                                     \
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                               \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                            \
    right_shift1.set_input_##inputName(placeholder##inputIndex);                                                     \
    right_shift1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                      \
    inputs.push_back(placeholder##inputIndex);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    ...
    auto right_shift1 = op::RightShift("right_shift1");
    std::vector<int64_t> xShape = {2, 3};
    std::vector<int64_t> yShape = {3};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, y, inDtype, yShape);
     
    outputs.push_back(right_shift1);
    // 添加完毕
    return SUCCESS;
}
