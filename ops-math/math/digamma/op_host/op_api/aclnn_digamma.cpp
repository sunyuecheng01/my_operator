...
static constexpr std::initializer_list<DataType> ASCEND_DTYPE_DTYPE_SUPPORT_LIST = {
...
static inline bool CheckDtypeValid(const aclTensor *self, const aclTensor *out) {
  ...
  auto supportList = ASCEND_DTYPE_DTYPE_SUPPORT_LIST;
...
aclnnStatus aclnnDigammaGetWorkspaceSize(const aclTensor *self, aclTensor *out,
                                     uint64_t *workspaceSize, aclOpExecutor **executor) {
  L2_DFX_PHASE_1(aclnnDigamma, DFX_IN(self), DFX_OUT(out));
  ...
  // 调用l0算子Digamma进行计算
  auto digammaResult = l0op::Digamma(selfContiguous, uniqueExecutor.get());
  CHECK_RET(digammaResult != nullptr, ACLNN_ERR_INNER_NULLPTR);

  // 如果出参out是非连续Tensor，需要把计算完的连续Tensor转非连续
  auto viewCopyResult = l0op::ViewCopy(digammaResult, out, uniqueExecutor.get());
