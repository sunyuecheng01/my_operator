...
class l2_slogdet_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "slogdet_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "slogdet_test TearDown" << endl; }
};

// self == nullptr
TEST_F(l2_slogdet_test, ascend910B2_case_nullptr_self) {
    ...
    auto ut = OP_API_UT(aclnnSlogdet, INPUT(nullptr), OUTPUT(signOut, logOut));
...
// signOut == nullptr
TEST_F(l2_slogdet_test, ascend910B2_case_nullptr_sign_out) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto logOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);

    auto ut = OP_API_UT(aclnnSlogdet, INPUT(self), OUTPUT(nullptr, logOut));
...
// logOut == nullptr3
TEST_F(l2_slogdet_test, ascend910B2_case_nullptr_log_out) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);

    auto ut = OP_API_UT(aclnnSlogdet, INPUT(self), OUTPUT(signOut, nullptr));
...
// 异常数据类型 self
TEST_F(l2_slogdet_test, ascend910B2_case_fp16_self) {
...
// 异常数据类型 signOut
TEST_F(l2_slogdet_test, ascend910B2_case_fp16_sign_out) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 异常数据类型 logOut
TEST_F(l2_slogdet_test, ascend910B2_case_fp16_log_out) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 异常数据类型 self是复数但signOut是实数
TEST_F(l2_slogdet_test, ascend910B2_case_dtype_can_not_promote) {
...
// 数据类型覆盖 float
TEST_F(l2_slogdet_test, ascend910B2_case_float) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto logOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 数据类型覆盖 double
TEST_F(l2_slogdet_test, ascend910B2_case_double) {
...
// 数据类型覆盖 complex64
TEST_F(l2_slogdet_test, ascend910B2_case_complex64) {
    auto self = TensorDesc({3, 5, 5}, ACL_COMPLEX64, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_COMPLEX64, ACL_FORMAT_ND).ValueRange(0, 2);
    auto logOut = TensorDesc({3}, ACL_COMPLEX64, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 数据类型覆盖 complex128
TEST_F(l2_slogdet_test, ascend910B2_case_complex128) {
    auto self = TensorDesc({3, 5, 5}, ACL_COMPLEX128, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_COMPLEX128, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 8维以上
TEST_F(l2_slogdet_test, ascend910B2_case_dim_10) {
    auto self = TensorDesc({3, 3, 3, 3, 3, 3, 3, 3, 5, 5}, ACL_COMPLEX128, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3, 3, 3, 3, 3, 3, 3, 3}, ACL_COMPLEX128, ACL_FORMAT_ND).ValueRange(0, 2);
...
// self不是一系列方阵
TEST_F(l2_slogdet_test, ascend910B2_case_self_not_equal_noise_1) {
...
// signOut不是self去掉最后两维
TEST_F(l2_slogdet_test, ascend910B2_case_log_out_shape_wrong) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// logOut不是self去掉最后两维
TEST_F(l2_slogdet_test, ascend910B2_case_sign_out_shape_wrong) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// format覆盖
TEST_F(l2_slogdet_test, ascend910B2_case_format) {
...
// 输入输出dtype不一致但可以cast
TEST_F(l2_slogdet_test, ascend910B2_case_dtype_inconsistent) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto logOut = TensorDesc({3}, ACL_DOUBLE, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 输入输出format不一致
TEST_F(l2_slogdet_test, ascend910B2_case_format_inconsistent) {
    auto self = TensorDesc({3, 5, 5}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
    auto signOut = TensorDesc({3}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(0, 2);
...
// 空tensor
TEST_F(l2_slogdet_test, ascend910B2_case_empty_tensor_training) {
...
// 非连续tensor
TEST_F(l2_slogdet_test, ascend910B2_case_not_contiguous_1) {
