...
#include "nn_other.h"
#include "../op_graph/grouped_bias_add_grad_proto.h"
...
#define ADD_INT_INPUT(intputIndex, intputName, intputDtype, inputShape)                      \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                           \
    auto placeholder##intputIndex = op::Data("placeholder" + intputIndex).set_attr_index(0); \
    TensorDesc placeholder##intputIndex##_desc =                                             \
        TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, intputDtype);     \
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::GroupedBiasAddGrad("add1");
    std::vector<int64_t> gradYShape = {100, 256};
    std::vector<int64_t> groupIdxShape = {3};

    ADD_INPUT(1, grad_y, inDtype, gradYShape);
    ADD_INT_INPUT(2, group_idx, DT_INT32, groupIdxShape);
    
    ADD_INPUT_ATTR(group_idx_type, 0);
