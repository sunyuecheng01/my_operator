...
#include "nn_other.h"
#include "../op_graph/assign_add_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto assignAdd1 = op::AssignAdd("assignAdd1");
    std::vector<int64_t> xShape = {1, 16};
    std::vector<int64_t> yShape = {1, 16};
    ADD_INPUT(1, ref, inDtype, yShape);
    ADD_INPUT(2, value, inDtype, xShape);
    
    ADD_INPUT_ATTR(use_locking, false);

    ADD_OUTPUT(1, ref, inDtype, yShape);

    outputs.push_back(assignAdd1);
    // 添加完毕
    return SUCCESS;
}
