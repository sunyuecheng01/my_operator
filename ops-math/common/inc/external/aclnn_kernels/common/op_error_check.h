...
template <typename T>
bool IsNullptr(const T* param, const char* name)
{
    if (param == nullptr) {
        const char* paramName = typeid(T).name();
        int32_t status = -1;
        char* readableName = abi::__cxa_demangle(paramName, nullptr, nullptr, &status);
        if (status == 0 && readableName != nullptr) {
            OP_LOGE(
                ACLNN_ERR_PARAM_NULLPTR, "Expected a value of type [%s] for argument [%s] but instead found nullptr.",
                readableName, name);
            free(readableName);
        } else {
            OP_LOGE(ACLNN_ERR_PARAM_NULLPTR, "Expected a value for argument [%s] but instead found nullptr.", name);
        }
        return true;
    }
    return false;
...
static inline bool CheckReduceOutShape(const aclTensor* inferOut, const aclTensor* out)
{
    ...
    if (xShape != yShape) {
        if (!(xShape.GetShapeSize() == 1 && yShape.GetShapeSize() == 1)) {
            OP_LOGE(
                ACLNN_ERR_PARAM_INVALID, "The out tensor's shape[%s] is not equal with inferOut shape[%s].",
...
static inline bool CheckNCDimValid(const aclTensor* self, const aclTensor* out)
{
    ...
    if (format == op::Format::FORMAT_NCHW) {
        ...
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Input and output format only support [NCHW, NHWC] format .");
        return false;
    }
    if ((selfDimN != outDimN) || (selfDimC != outDimC)) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "The selfDimN[%ld]/outDimN[%ld] or selfDimC[%ld]/outDimC[%ld] not equal .",
...
#define OP_CHECK_DTYPE_NOT_SUPPORT(tensor, supportList, retExpr)                                                  \
    do {                                                                                                          \
        if (!CheckType(tensor->GetDataType(), supportList)) {                                                     \
            OP_LOGE(                                                                                              \
                ACLNN_ERR_PARAM_INVALID, "Tensor %s not implemented for %s, should be in dtype support list %s.", \
...
#define OP_CHECK_DTYPE_NOT_MATCH(tensor, expectedDtype, retExpr)                                           \
    do {                                                                                                   \
        if (tensor->GetDataType() != expectedDtype) {                                                      \
            OP_LOGE(                                                                                       \
                ACLNN_ERR_PARAM_INVALID, "Tensor %s expected dtype is %s but found %s.", #tensor,          \
...
#define OP_CHECK_DTYPE_NOT_SAME(tensor1, tensor2, retExpr)                                                       \
    do {                                                                                                         \
        if (tensor1->GetDataType() != tensor2->GetDataType()) {                                                  \
            OP_LOGE(                                                                                             \
                ACLNN_ERR_PARAM_INVALID, "Expected both tensors to have same dtype, but found %s %s and %s %s.", \
                #tensor1, op::ToString(tensor1->GetDataType()).GetString(), #tensor2,                            \
...
#define OP_CHECK_BROADCAST(tensor1, tensor2, retExpr)                                                               \
    do {                                                                                                            \
        if (!CheckBroadcastShape(tensor1->GetViewShape(), tensor2->GetViewShape())) {                               \
            OP_LOGE(                                                                                                \
                ACLNN_ERR_PARAM_INVALID, "The size of tensor %s %s must match the size of tensor %s %s.", #tensor1, \
                op::ToString(tensor1->GetViewShape()).GetString(), #tensor2,                                        \
...
#define OP_CHECK_BROADCAST_WITH_SHAPE(tensor, shape, retExpr)                                         \
    do {                                                                                              \
        if (!CheckBroadcastShape(tensor->GetViewShape(), shape)) {                                    \
            OP_LOGE(                                                                                  \
                ACLNN_ERR_PARAM_INVALID, "The size of tensor %s %s must match the size %s.", #tensor, \
                op::ToString(tensor->GetViewShape()).GetString(), op::ToString(shape).GetString());   \
            retExpr;                                                                                  \
        }                                                                                             \
    } while (0)
    ...
    do {                                                                                      \
        if (tensor1->GetViewShape() != tensor2->GetViewShape()) {                             \
            OP_LOGE(                                                                          \
                ...
                "equal %s.",                                                                  \
                #tensor1, #tensor2, op::ToString(tensor1->GetViewShape()).GetString(),        \
                op::ToString(tensor2->GetViewShape()).GetString());                           \
            retExpr;                                                                          \
        }                                                                                     \
    } while (0)

#define OP_CHECK_SHAPE_AND_SCALAR_EQUAL(tensor1, tensor2, retExpr)                                               \
    do {                                                                                                         \
        if (tensor1->GetViewShape() != tensor2->GetViewShape()) {                                                \
            if (!(tensor1->GetViewShape().GetShapeSize() == 1 && tensor2->GetViewShape().GetShapeSize() == 1)) { \
                OP_LOGE(                                                                                         \
                    ACLNN_ERR_PARAM_INVALID,                                                                     \
                    "Expected tensor for %s to have same size as tensor for %s, but %s does not "                \
                    "equal %s.",                                                                                 \
...
#define OP_CHECK_SHAPE_NOT_EQUAL_WITH_EXPECTED_SIZE(tensor, shape, retExpr)                                      \
    do {                                                                                                         \
        if (tensor->GetViewShape() != shape) {                                                                   \
            OP_LOGE(                                                                                             \
                ACLNN_ERR_PARAM_INVALID, "Expected tensor for %s to have same size as %s, but got %s.", #tensor, \
...
#define OP_CHECK_WRONG_DIMENSION(tensor, expectedDimNum, retExpr)                                                \
    do {                                                                                                         \
        if (tensor->GetViewShape().GetDimNum() != expectedDimNum) {                                              \
            OP_LOGE(                                                                                             \
                ACLNN_ERR_PARAM_INVALID, "Expected %zu dimension input, but got %s with sizes %s.",              \
...
#define OP_CHECK_MAX_DIM(tensor, maxDim, retExpr)                                                        \
    do {                                                                                                 \
        if (tensor->GetViewShape().GetDimNum() > static_cast<size_t>(maxDim)) {                          \
            OP_LOGE(                                                                                     \
                ACLNN_ERR_PARAM_INVALID, "The %s tensor cannot be larger than %zu dimensions.", #tensor, \
                static_cast<size_t>(maxDim));                                                            \
            retExpr;                                                                                     \
        }                                                                                                \
    } while (0)
...
#define OP_CHECK_MIN_DIM(tensor, minDim, retExpr)                                                     \
    do {                                                                                              \
        if (tensor->GetViewShape().GetDimNum() < static_cast<size_t>(minDim)) {                       \
            OP_LOGE(                                                                                  \
                ACLNN_ERR_PARAM_INVALID, "The %s tensor must have at least %zu dimensions.", #tensor, \
                static_cast<size_t>(minDim));                                                         \
            retExpr;                                                                                  \
        }                                                                                             \
    } while (0)
