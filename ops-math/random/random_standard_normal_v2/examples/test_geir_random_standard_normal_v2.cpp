...
#include "nn_other.h"
#include "../op_graph/random_standard_normal_v2_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::RandomStandardNormalV2("add1");
    std::vector<int64_t> xShape = {2};
    std::vector<int64_t> offsetShape = {1};
    std::vector<int64_t> outShape = {1, 1};
    ADD_CONST_INPUT(1, shape, inDtype, xShape);
    ADD_INT_INPUT(2, offset, inDtype, offsetShape, 0);

    ADD_INPUT_ATTR(dtype, 0);
    ADD_INPUT_ATTR(seed, 10);
    ADD_INPUT_ATTR(seed2, 5);
    
    ADD_OUTPUT(1, y, ge::DT_FLOAT, outShape);
    ADD_OUTPUT(2, offset, ge::DT_INT64, offsetShape);
