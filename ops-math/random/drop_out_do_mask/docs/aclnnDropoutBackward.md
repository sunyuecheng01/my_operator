...
- **参数说明：**
  ...
  - gradOutput(aclTensor*, 计算输入)：表示待处理的张量，Device侧的aclTensor，不支持空tensor场景。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。	 
     - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Ascend 950PR/Ascend 950DT</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT32。	 
   - mask(aclTensor*, 计算输入)：bit类型并使用UINT8类型存储的mask数据，Device侧的aclTensor，不支持空tensor场景。数据类型支持UINT8，shape需要为(align(gradOutput的元素个数,128)/8)，表示比特数需要与128对齐，其中，align表示将input的元素个数向上对齐为128的倍数，如align(1111, 128)的结果为1152。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)。[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
  - scale(double, 计算输入)：Host侧DOUBLE类型，输入`scale`，输出数据缩放比例。数据类型支持DOUBLE。
  - out(aclTensor*, 计算输出)：输出`out`，Device侧的aclTensor，数据类型需要是gradOutput可转换的数据类型，shape需要与gradOutput一致，支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)。[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
    - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Ascend 950PR/Ascend 950DT</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT32。
...
- **返回值：**
  ...
  161001 (ACLNN_ERR_PARAM_NULLPTR): 1. 传入的gradOutput、mask、out为空指针。
  161002 (ACLNN_ERR_PARAM_INVALID): 1. gradOutput、mask、out的数据类型不在支持的范围之内。
                                    2. scale需要为0或者大于等于1。
                                    3. gradOutput维度超过8维，gradOutput和out的shape不一致。
                                    4. mask的shape不满足条件。
  ```
