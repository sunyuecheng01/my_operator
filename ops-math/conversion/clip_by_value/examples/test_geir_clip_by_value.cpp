...
#include "nn_other.h"
#include "../op_graph/clip_by_value_proto.h"
...
#define ADD_DYNAMIC_INPUT(inputIndex, inputDtype, inputShape, dataName)                                     \
    do {                                                                                                    \
        ...
        clipByValue1.UpdateDynamicInputDesc("x", inputIndex, desc##dataName);                               \
        clipByValue1.set_dynamic_input_x(inputIndex, placeholder##dataName);                                \
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto clipByValue1 = op::ClipByValue("clipByValue1");
    std::vector<int64_t> xShape = {(1, 1, 1, 1)};

    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, clip_value_min, inDtype, {1});
    ADD_INPUT(3, clip_value_max, inDtype, {1});

    outputs.push_back(clipByValue1);
    // 添加完毕
    return SUCCESS;
}
