...
/*!
 * \file clip_by_value_ass.h
 * \brief
 */
#ifndef CLIP_BY_VALUE_ASS_H
#define CLIP_BY_VALUE_ASS_H
...
template <typename T>
class ClipByValueAss {
public:
    __aicore__ inline ClipByValueAss(){};
...
template <typename T>
__aicore__ inline void ClipByValueAss<T>::Init(
...
template <typename T>
__aicore__ inline void ClipByValueAss<T>::CopyIn(const int64_t& gmOffset)
{
...
template <typename T>
__aicore__ inline void ClipByValueAss<T>::Compute(const int64_t& len)
{
    auto xLocal = inQueueX.template DeQue<T>();
    auto yLocal = outQueue.template AllocTensor<T>();
    Mins(yLocal, xLocal, maxValue, len);
    inQueueX.FreeTensor(xLocal);
    Maxs(yLocal, yLocal, minValue, len);
    outQueue.EnQue(yLocal);
}

template <typename T>
__aicore__ inline void ClipByValueAss<T>::CopyOut(const int64_t& gmOffset)
{
...
template <typename T>
__aicore__ inline void ClipByValueAss<T>::Process()
{
    if (blockIdx >= blockNum) {
        return;
    }
