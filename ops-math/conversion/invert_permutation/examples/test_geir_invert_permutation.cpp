...
#include "nn_other.h"
#include "../op_graph/invert_permutation_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    invertPermutation.set_input_##intputName(placeholder##intputIndex);                                   \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    invertPermutation.set_input_##intputName(placeholder##intputIndex);                                   \
    invertPermutation.update_input_desc_##intputName(placeholder##intputIndex##_desc);                    \
    inputs.push_back(placeholder##intputIndex)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);                          \
    invertPermutation.update_output_desc_##outputName(outputName##outputIndex##_desc)             
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto invertPermutation = op::InvertPermutation("invert_permutation");
    
    std::vector<int64_t> xShape = {4};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_OUTPUT(1, y, inDtype, xShape);

    outputs.push_back(invertPermutation);
    // 添加完毕
    return SUCCESS;
}
...
{
    // 1、创建图对象
    const char *graph_name = "tc_ge_irrun_invert_permutation_test";
