...
/*!
 * \file clip_by_value_v2_ub_broadcast.h
 * \brief
 */

#ifndef ASCENDC_CLIP_BY_VALUEUB_V2_BROADCAST_H_
#define ASCENDC_CLIP_BY_VALUEUB_V2_BROADCAST_H_
...
class ClipByValueV2UbBroadcast {
public:
    __aicore__ inline ClipByValueV2UbBroadcast(){};
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::Init(
    GM_ADDR x, GM_ADDR clipValueMin, GM_ADDR clipValueMax, GM_ADDR y, GM_ADDR workspace,
    const ClipByValueUbBrcTilingData* tilingDataPtr, TPipe* pipePtr)
{
    pipePtr_ = pipePtr;
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::Process()
{
    int64_t ubLoopNum =
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::CopyIn0(
    int64_t ubSplitSize, const int64_t (&axesIndices)[8], int64_t ubLoopIdx, uint32_t (&srcShape)[8],
    uint32_t (&dstShape)[8], int64_t input_ub_length, BroadcastTiling& broadcastTiling)
{
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::CopyIn1(
    int64_t ubSplitSize, const int64_t (&axesIndices)[8], int64_t ubLoopIdx, uint32_t (&srcShape)[8],
    uint32_t (&dstShape)[8], int64_t input_ub_length, BroadcastTiling& broadcastTiling)
{
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::CopyIn2(
    int64_t ubSplitSize, const int64_t (&axesIndices)[8], int64_t ubLoopIdx, uint32_t (&srcShape)[8],
    uint32_t (&dstShape)[8], int64_t input_ub_length, BroadcastTiling& broadcastTiling)
{
...
template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::Compute3(int64_t ubSplitSize, const int64_t (&axesIndices)[8])
{
    LocalTensor<T> bufferOut0_ = queOut0_.AllocTensor<T>();
    Max(bufferOut0_, broadcastTempBuf0.Get<T>(), broadcastTempBuf1.Get<T>(),
        ubSplitSize * tilingDataPtr_->outputStrides[tilingDataPtr_->ubSplitAxis]);
    Min(bufferOut0_, bufferOut0_, broadcastTempBuf2.Get<T>(),
        ubSplitSize * tilingDataPtr_->outputStrides[tilingDataPtr_->ubSplitAxis]);
    queOut0_.EnQue(bufferOut0_);
}

template <typename T, int64_t R>
__aicore__ inline void ClipByValueV2UbBroadcast<T, R>::CopyOut4(
...
#endif // ASCENDC_CLIP_BY_VALUE_V2_UB_BROADCAST_H_