...
#include "nn_other.h"
#include "../op_graph/clip_by_value_v2_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::ClipByValueV2("clip_by_value_v2");
    vector<vector<int64_t>> shapes = {{1, 4}, {1, 4}, {1, 4}, {1, 4}};

    ADD_INPUT(1, x, inDtype, shapes[0]);
    ADD_INPUT(2, clip_value_min, inDtype, shapes[1]);
    ADD_INPUT(3, clip_value_max, inDtype, shapes[2]);

    ADD_OUTPUT(4, y, inDtype, shapes[3]);
