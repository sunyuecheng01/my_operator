...
#include "nn_other.h"
#include "../op_graph/strided_slice_assign_v2_proto.h"
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::StridedSliceAssignV2("add1");
    std::vector<int64_t> varRefShape = {4, 3};
    std::vector<int64_t> inputValueShape = {2, 2};
    std::vector<int64_t> sliceShape = {2};
    std::vector<int64_t> attrShape = {2};

    ADD_INPUT(1, var, inDtype, varRefShape);
    ADD_INPUT(2, input_value, inDtype, inputValueShape);

    ADD_INT_INPUT(3, begin, DT_INT32, attrShape, {1, 0});
    ADD_INT_INPUT(4, end, DT_INT32, attrShape, {4, 2});
    ADD_INT_INPUT(5, strides, DT_INT32, attrShape, {2, 1});
    ADD_INT_INPUT(6, axes, DT_INT32, attrShape, {0, 1});

    ADD_OUTPUT(1, var, inDtype, varRefShape);
