...
#include "nn_other.h"
#include "../op_graph/as_strided_proto.h"
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    node.set_input_##intputName(placeholder##intputIndex);                                                  \
    node.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                   \
    inputs.push_back(placeholder##intputIndex);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto node = op::AsStrided("as_strided1");
    std::vector<int64_t> xShape = {3, 3};
    std::vector<int64_t> yShape = {2, 2};
    std::vector<int64_t> sizeShape = {2};
    std::vector<int32_t> sizeValue = {2, 2};
    std::vector<int64_t> strideShape = {2};
    std::vector<int32_t> strideValue = {1, 2};
    std::vector<int64_t> offsetShape = {1};
    std::vector<int32_t> offsetValue = {1};

    ADD_INPUT(1, x, inDtype, xShape);

    ADD_INT_INPUT(2, size, DT_INT32, sizeShape, sizeValue);
    ADD_INT_INPUT(3, stride, DT_INT32, strideShape, strideValue);
    ADD_INT_INPUT(4, storage_offset, DT_INT32, offsetShape, offsetValue);
