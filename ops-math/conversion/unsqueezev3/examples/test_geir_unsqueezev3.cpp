...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    unsqueezev3.set_input_##intputName(placeholder##intputIndex);                            \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    unsqueezev3.set_input_##intputName(placeholder##intputIndex);                            \
    unsqueezev3.update_input_desc_##intputName(placeholder##intputIndex##_desc);             \
    inputs.push_back(placeholder##intputIndex);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto unsqueezev3 = op::UnsqueezeV3("unsqueezev3");
    std::vector<int64_t> xShape = {4, 2};
    std::vector<int64_t> aShape = {0};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, axes, inDtype, aShape);
     
    outputs.push_back(unsqueezev3);
    // 添加完毕
    return SUCCESS;
}
