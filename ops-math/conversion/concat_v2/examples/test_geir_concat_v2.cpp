...
#include "nn_other.h"
#include "../op_graph/concat_v2_proto.h"
...
#define ADD_DYNAMIC_INPUT(inputIndex, inputDtype, inputShape, dataName)                                     \
    do {                                                                                                    \
        ...
        concat1.UpdateDynamicInputDesc("x", inputIndex, desc##dataName);                                    \
        concat1.set_dynamic_input_x(inputIndex, placeholder##dataName);                                     \
    ...
    Graph& graph)
{
    ...
    ADD_INT_INPUT(1, concat_dim, DT_INT32, {1}, 1);
    ADD_INPUT_ATTR(N, 2);

    vector<int64_t> yShape = {32, 8, 4, 4};
    ADD_OUTPUT(1, y, inDtype, yShape);

    outputs.push_back(concat1);
