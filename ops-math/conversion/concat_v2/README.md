...
- 计算流程：
  ...
  - 流程：
    ...
    3. 沿 concat_dim 维度依次拼接：
       y = ConcatV2(x[0], x[1], ..., x[N-1], axis = concat_dim)
  - 输出：拼接后的 Tensor y
...
| 调用方式 | 调用样例                                                    | 说明                                                           |
|--------------|---------------------------------------------------------|--------------------------------------------------------------|
| 图模式调用 | [test_geir_concat_v2](./examples/test_geir_concat_v2.cpp) | 通过[算子IR](./op_graph/concat_v2_proto.h)构图方式调用ConcatV2算子。 |
