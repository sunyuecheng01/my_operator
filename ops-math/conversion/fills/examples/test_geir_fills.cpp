...
#include "nn_other.h"
#include "../op_graph/fills_proto.h"
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::Fills("fills");
    vector<vector<int64_t>> shape = {3, 4};

    ADD_INPUT(1, x, inDtype, shape);
    ADD_OUTPUT(1, y, inDtype, shape);
    ADD_INPUT_ATTR(value, 1.5f);
