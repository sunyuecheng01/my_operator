...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::CopyGm2UB(
...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::CopyOut2Gm(
...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::CopyWs2UB(
...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::CopyOut2Workspace(const int32_t tIdx, const int64_t calCount)
{
    int64_t workspaceOffset;
...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::ImplTransposeAndCompute(
    const int64_t transCount, const int32_t flag)
{
    ...
    if constexpr (AscendC::IsSameType<T, half>::value) {
        ...
        if (flag == 0) {
            ...
            DataCopy(
                transposeData, transposeData[padLeft * ubFactorElement],
...
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::ComputeHGrad(const int32_t calCount)
{
    ...
    for (size_t i = 0; i < padTop; i++) {
        Add(xLocal[padTop * ubFactorElement], xLocal[i * ubFactorElement], xLocal[padTop * ubFactorElement], calCount);
    }
...
template <typename T>
__aicore__ inline void PadV3GradReplicateSmallHLargeW<T>::CopyIn(const int32_t copyCount, const int64_t workspaceOffset)
{
    DataCopyExtParams copyParams{1, (uint32_t)(copyCount * sizeof(T)), 0, 0, 0};
    DataCopyPadExtParams<T> padParams{true, 0, 0, (T)0};
    LocalTensor<T> xLocal = xInQueue.AllocTensor<T>();
