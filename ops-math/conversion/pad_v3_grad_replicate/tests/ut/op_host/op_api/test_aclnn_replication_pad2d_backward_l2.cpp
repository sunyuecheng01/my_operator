...
class replication_pad2d_backward_test : public testing::Test {
 protected:
  static void SetUpTestCase() { cout << "replication_pad2d_backward_test SetUp" << endl; }

  static void TearDownTestCase() { cout << "replication_pad2d_backward_test TearDown" << endl; }
};
...
TEST_F(replication_pad2d_backward_test, case_3) {
  ...
  auto ut = OP_API_UT(aclnnReplicationPad2dBackward, INPUT(nullptr, input_tensor_desc, padding_desc),
  ...
  auto ut_2 = OP_API_UT(aclnnReplicationPad2dBackward, INPUT(grad_output_tensor_desc, nullptr, padding_desc),
                                                OUTPUT(grad_input_desc));
  workspace_size = 0;
  aclRet = ut_2.TestGetWorkspaceSize(&workspace_size);
  EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

  auto ut_3 = OP_API_UT(aclnnReplicationPad2dBackward, INPUT(grad_output_tensor_desc, input_tensor_desc, nullptr),
                                                OUTPUT(grad_input_desc));
  workspace_size = 0;
