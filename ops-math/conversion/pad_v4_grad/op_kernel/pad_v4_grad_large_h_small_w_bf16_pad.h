...
/*!
 * \file pad_v4_grad_large_h_small_w_bf16_pad.h
 * \brief
 */
#ifndef _PAD_V4_GRAD_LARGE_H_SMALL_W_BF16_PAD_H_
#define _PAD_V4_GRAD_LARGE_H_SMALL_W_BF16_PAD_H_

#include "pad_v4_grad_base.h"

template <typename T>
class PadV4GradLargeHSmallWBf16 : public PadV4GradBase<T> {
public:
    __aicore__ inline PadV4GradLargeHSmallWBf16(){};
...
// init used buffer
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::InitBuffer(TPipe* inputPipe)
{
    pipe = inputPipe;
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::CopyGm2UB(const int32_t batchIdx, const int32_t flag)
{
    int64_t gmXOffset;
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::CopyOut2Gm(
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::CopyGmAndWs2UB1(const int32_t batchIdx)
{
    DataCopyExtParams copyParams{1, (uint32_t)(this->width * sizeof(T)), 0, 0, 0};
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::CopyGmAndWorkspace2UB2(
    const int32_t transBlkIdx, const int32_t transTimes, const int32_t cycles, const int32_t batchIdx)
{
    DataCopyExtParams copyParams{1, (uint32_t)(this->width * sizeof(T)), 0, 0, 0};
    DataCopyPadExtParams<T> padParams{true, 0, 0, (T)0};
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::CopyOut2Ws(const int64_t calCount, const int32_t flag)
{
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::implTransposeAndCompute(const int64_t transCount)
{
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::ComputeHGrad(const int32_t calCount, const int32_t flag)
{
    LocalTensor<T> xLocal = xInQueue.DeQue<T>();
    LocalTensor<T> yLocal = yOutQueue.AllocTensor<T>();
    Cast(floatTenosr, xLocal, RoundMode::CAST_NONE, this->ubFactorElement * SMALL_WIDTH_LIMIT);
    ...
    if (flag == 0) {
        ...
        Cast(yLocal, floatTenosr, RoundMode::CAST_RINT, this->ubFactorElement * SMALL_WIDTH_LIMIT);
        DataCopy(
            yLocal, yLocal[this->hPad1 * SMALL_WIDTH_LIMIT], (COPY_ROWS_AND_COLS - this->hPad1) * SMALL_WIDTH_LIMIT);
    } else {
...
template <typename T>
__aicore__ inline void PadV4GradLargeHSmallWBf16<T>::Process()
{
...
#endif // _PAD_V4_GRAD_LARGE_H_SMALL_W_BF16_PAD_H_